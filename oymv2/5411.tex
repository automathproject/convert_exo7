...
\reponse{La fonction $f~:~x\mapsto\ln x$ est deux fois dérivable sur $]0,+\infty[$ et, pour $x>0$, $f''(x)=-\frac{1}{x^2}<0$. Par suite, $f$ est concave sur $]0,+\infty[$. On en déduit que~:
...
\reponse{Soient $p$ et $q$ deux réels strictement positifs vérifiant $\frac{1}{p}+\frac{1}{q}=1$ (de sorte que l'on a même $\frac{1}{p}<\frac{1}{p}+\frac{1}{q}=1$ et donc $p>1$ et aussi $q>1$).
...
\reponse{Soient $A=\sum_{k=1}^{n}|a_k|^p$ et $B=\sum_{k=1}^{n}|b_k|^q$.
...
\reponse{Pour $p>1$, la fonction $x\mapsto x^p$ est deux fois dérivable sur $]0,+\infty[$ et $(x^p)''=p(p-1)x^{p-2}>0$. Donc, la fonction $x\mapsto x^p$ est strictement convexe sur $]0,+\infty[$ et donc sur $[0,+\infty[$ par continuité en $0$. Donc,
...
\reponse{Pour $p=q=2$, c'est l'inégalité de \textsc{Cauchy}-\textsc{Schwarz} démontrée dans une planche précédente.}
\end{enumerate}
}
