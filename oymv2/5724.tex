...
\reponse{Puisque $f$ est de classe $C^1$ sur $\Rr^+$, pour $x\geqslant0$,  $\int_{0}^{x}f'(t)\;dt =f(x)-f(0)$. Donc l'intégrale $\int_{0}^{+\infty}f'(t)\;dt$ converge en $+\infty$ si et seulement si $f$ a une limite réelle $\ell$ quand $x$ tend vers $+\infty$.
...
\reponse{Soit $x\geqslant0$. D'après la formule de \textsc{Taylor}-\textsc{Lagrange}, il existe un réel $\theta_x\in]x,x+1[$
...
\reponse{Soit $F~:~x\mapsto\int_{0}^{x}f(t)\;dt$. $F$ est de classe $C^3$ sur $\Rr^+$. De plus, $F(x) =\int_{0}^{x}f(t)\;dt$ tend vers $\int_{0}^{+\infty}f(t)\;dt$ et 
