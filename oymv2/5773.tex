...
\reponse{\textbullet~Soient $P$ et $Q$ deux polynômes. La fonction $t\mapsto P(t)Q(t)e^{-t}$ est continue sur $[0,+\infty[$ et est négligeable en $+\infty$ devant $\frac{1}{t^2}$ d'après un théorème de croissances comparées. Donc la fonction $t\mapsto P(t)Q(t)e^{-t}$ est intégrable sur $[0,+\infty[$ et $\varphi(P,Q)$ existe dans $\Rr$.
...
\reponse{Soit $n\in\Nn$. La formule de \textsc{Leibniz} permet d'écrire
...
\reponse{Soient $P\in E$ et $n\in\Nn^*$. Soit $A>0$. Les deux fonctions $t\mapsto(t^ne^{-t})^{(n-1)}$ et $P$ sont de classe $C^1$ sur le segment $[0,A]$. On peut donc effectuer une intégration par parties et on obtient
...
\reponse{Soit $n\in\Nn$. Puisque $\text{deg}(h_n)=n$ et $\text{dom}(h_n)=(-1)^n$, on a $h_n^{(n)}=(-1)^nn!$. La question précédente fournit alors
