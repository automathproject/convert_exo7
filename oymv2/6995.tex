...
\begin{enumerate}
    \item \question{$a=0$}
\reponse{L'équation différentielle $y'-e^xe^y=0$ est à variables séparées: 
...
\reponse{Posons $z(x)=x+y(x)$: $z$ a le m\^eme domaine de définition que $y$ et est dérivable 
...
\indication{1. C'est une équation à variables séparées.}
\end{enumerate}
}
