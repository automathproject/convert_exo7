...
\reponse{Pour tout réel $x$ de $[-1,1]$, $\sin(2\Arcsin x)=2\sin(\Arcsin x)\cos(\Arcsin x)=2x\sqrt{1-x^2}$.}
    \item \question{$\cos(2\Arccos x)$,}
\reponse{Pour tout réel $x$ de $[-1,1]$, $\cos(2\Arccos x)=2\cos^2(\Arccos x)-1=2x^2-1$.}
    \item \question{$\sin^2\left(\frac{\Arccos x}{2}\right)$,}
\reponse{Pour tout réel $x$ de $[-1,1]$,
...
\reponse{L'expression proposée est définie sur $\Rr^*$ et impaire. Soit alors $x>0$.
...
\reponse{L'expression proposée existe si et seulement si $2x^2-1\in[1,+\infty[$ ou encore $x^2\in[1,+\infty[$ ou enfin
