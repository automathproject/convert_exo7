...
\organisation{exo7}
\contenu{
  \texte{Soit $f\colon \R^2\rightarrow \R$ la fonction
d\'efinie par 
\begin{eqnarray*}
...
\end{enumerate}
\begin{enumerate}
  \item \reponse{Puisque $f(x,y)=\frac{x^2y +3y^3}{x^2+y^2}
...
Par cons\'equent la fonction $f$ est continue en $(0,0)$.}
  \item \reponse{Les d\'eriv\'ees partielles
\begin{align*}
...
\end{align*}
existent.}
  \item \reponse{Puisque $f(x,x)=\frac{4x^3}{2x^2}= 2x$, la d\'eriv\'ee
...
\end{align*}}
  \item \reponse{D'apr\`es \eqref{eqt}, cette \'equation s'\'ecrit
\[
...
\]
d'où $z=3y-x$.}
  \item \reponse{La fonction $F\colon \R^2 \to \R^2$ s'\'ecrit 
