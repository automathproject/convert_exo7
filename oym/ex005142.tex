...
\organisation{exo7}
\contenu{
  \texte{Cet exercice est consacré aux sommes de termes consécutifs d'une suite arithmétique ou d'une suite géométrique.}
\begin{enumerate}
...
$$\sum_{k=4}^{n+1}(3k+7)=\frac{(19+3n+10)(n-2)}{2}=\frac{1}{2}(3n+29)(n-2)=\frac{1}{2}(3n^2+23n-58).$$}
  \item \reponse{Soit $n\in\Nn^*$. Posons $u_n=1,\underbrace{11...1}_n$. On a
...
\end{center}}
  \item \reponse{Soit $n\in\Nn^*$. Posons $u_n=\underbrace{1-1+1-...+(-1)^{n-1}}_n$.  On a
...
on a immédiatement $S_{4n}=1$, $S_{4n+1}=S_{4n}+0=1$, $S_{4n+2}=S_{4n+1}-1=0$ et $S_{4n+3}=S_{4n+2}+0=0$.}
  \item \reponse{Soient $n\in\Nn$ et $\theta\in\Rr$. Posons $C_n=\sum_{k=0}^{n}\cos(k\theta)$ et
...
\begin{itemize}}
  \item \reponse{[\textbf{- 1er cas.}] Si $\theta\notin2\pi\Zz$, alors $e^{i\theta}\neq1$. Par suite,
...
n+iS_n)=\frac{\sin\frac{n\theta}{2}\sin\frac{(n+1)\theta}{2}}{\sin\frac{\theta}{2}}.$$}
  \item \reponse{[\textbf{- 2ème cas.}] Si $\theta\in2\pi\Zz$, on a immédiatement $C_n=n+1$ et $S_n=0$.
\end{itemize}
...
\end{center}}
  \item \reponse{Soient $x\in[0,1]$ et $n\in\Nn^*$. Puisque $-x\neq1$, on a
...
\end{center}}
  ...
  \item \reponse{Soit $n\in\Nn$. $u_{n+1}-3=2u_n-6=2(u_n-3)$. La suite $(u_n-3)_{n\in\Nn}$ est donc une suite géométrique, de
