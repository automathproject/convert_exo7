...
\contenu{
  \texte{}
  \question{(Hors programme) Convergence et calcul de $\int_{1}^{+\infty}\frac{(-1)^{E(x)}}{x}\;dx$.}
  \reponse{La fonction $f~:~x\mapsto\frac{(-1)^{E(x)}}{x}$ est continue par morceaux sur $[1,+\infty[$ et donc localement intégrable sur $[1,+\infty[$.
