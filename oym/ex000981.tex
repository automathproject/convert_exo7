...
\end{enumerate}
\begin{enumerate}
  \item \reponse{Pour montrer que la famille $\{ v_1, v_2, v_3\}$ est une base nous allons 
montrer que cette famille est libre et génératrice.


  \begin{enumerate}}
  \item \reponse{Montrons que la famille $\{ v_1, v_2, v_3\}$ est libre.
...
Ainsi les coefficients vérifient $a=b=c=0$, cela prouve que la famille est libre.}
  \item \reponse{Montrons que la famille $\{ v_1, v_2, v_3\}$ est génératrice. Pour n'importe quel
...
la relation $av_1+bv_2+cv_3=(x,y,z)=v$. Donc la famille $\{ v_1, v_2, v_3\}$ est génératrice.}
  \item \reponse{La famille est libre et génératrice  donc c'est une base.}
  \item \reponse{Pour écrire  $w=(1,1,1)$ dans la base $(v_1,v_2,v_3)$  on peut résoudre le système correspondant
