...
\organisation{exo7}
\contenu{
  \texte{Montrer que, pour $A\in\Cc^{n\times m}$}
\begin{enumerate}
...
\end{enumerate}
\begin{enumerate}
  \item \reponse{$\|A\|_2=\|U\Sigma V^*\|_2=\|\Sigma\|_2=\max |\sigma_j|=\sigma_1$}
  \item \reponse{$\|A\|^2_F =$tr$(A^*A)=$tr$(U^*A^*AU)=\|AU\|^2_F=$tr$(A^*U^*UA)=\|UA\|^2_F$ et donc
$$\|A\|_F=\|U\Sigma V^*\|_F =\|\Sigma\|_F=\sqrt{\sigma_1^2 + \cdots +\sigma_r^2}$$}
  \item \reponse{$A^*A=(V\Sigma^*U^*) (U\Sigma V^*)=V(\Sigma^*\Sigma )V^*$ et donc $A^*A$ est semblable \`a $\Sigma^*\Sigma$, les deux matrices ont donc les m\^emes valeurs propres. Les valeurs propres de $\Sigma^*\Sigma$ sont $\sigma_1^2, \cdots , \sigma_r^2$ plus $n-r$ valeurs propres nulles si $n>r$.}
  \item \reponse{$|\det A|=|\det (U\Sigma V^*)|= |\det U|\det|\Sigma||\det V^*|= |\det \Sigma|=\prod_{i=1}^r \sigma_i$}
  \item \reponse{Une matrice hermitienne \'etant diagonalisable a une base orthonormale de vecteurs propres
