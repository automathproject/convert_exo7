...
\organisation{exo7}
\contenu{
  \texte{On effectue un contrôle sur des pièces de un euro
...
\end{enumerate}
\begin{enumerate}
  \item \reponse{Soit $F$ l'événement <<la pièce est fausse>>; soit $U$ l'événement <<la pièce est un euro>>; 
soit $D$ l'événement <<la pièce est deux euros>>. Alors
$P(F)=P(F/U)P(U)+P(F/D)P(D)=2.9\%$.}
  \item \reponse{On cherche $P(U/F)=$($P(F/U)P(U))/P(F)=51.7\%$.}
  \item \reponse{$X$ la variable aléatoire <<nombre de pièces 
