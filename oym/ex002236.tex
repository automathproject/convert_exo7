...
\organisation{exo7}
\contenu{
  \texte{Soit $A$ une matrice hermitienne inversible d\'ecompos\'ee en $A=M-N$ o\`u $M$ est inversible. Soit $B=I-M^{-1}A$ la matrice de l'it\'eration:
$$x_{n+1}= Bx_n +c .$$
...
\end{enumerate}
\begin{enumerate}
  \item \reponse{Pour le membre de gauche on obtient
...
ce qui fini la d\'emonstration.}
  \item \reponse{$y=Bx=\lambda x\Rightarrow x-y=(1-\lambda )x$. En utilisant l'\'egalit\'e pr\'ec\'edente
...
it\'erative converge.}
  \item \reponse{D\'emonstration par absurde. Supposons que ce n'est pas vrai: $\exists x_0\neq 0\quad
...
d\'efinie positive}
  \item \reponse{Soit $A=D-E-F$ la d\'ecomposition usuelle de $A$. Comme $A$ est
