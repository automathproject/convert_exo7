...
\contenu{
  \texte{}
  \question{Trouver toutes les applications continues sur $\Rr$ vérifiant~:~$\forall(x,y)\in\Rr^2,\;f(x)f(y)=\int_{x-y}^{x+y}f(t)\;dt$.}
  \reponse{$f$ est continue sur $\Rr$ et admet donc des primitives sur $\Rr$. Soit $F$ une primitive donnée de $f$ sur $\Rr$. Notons $(*)$ la relation~:
