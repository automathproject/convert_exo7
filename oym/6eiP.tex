\uuid{6eiP}
\exo7id{2230}
\titre{Algorithme de Gram--Schmidt et Gram--Schmidt modifi\'e}
\theme{Factorisation QR. Transformations de Givens. Moindres carrés}
\auteur{matos}
\date{2008/04/23}
\organisation{exo7}
\contenu{
  \texte{Etant donn\'es $n$ vecteurs lin\'eairement ind\'ependants de $\Rr^m$, $\{a_1, \cdots , a_n\}$, on veut calculer une base orthonormale pour span$\{a_1, \cdots , a_n\}$.
...
\end{enumerate}
\begin{enumerate}
  \item \reponse{Pour $k=1,\cdots ,n\quad a_k=\sum_{i=1}^kr_{ik}q_i$ avec $r_{ik}=q_i^Ta_k$ par orthonormalit\'e des $q_i$.}
  \item \reponse{D\'ecoule imm\'ediatement de la question pr\'ec\'edente.}
  \item \reponse{Algorithme de Gram-Schmidt:
...
\hspace{1cm}$q_k=z_k/r_{kk}$}
  ...
  \item \reponse{$$\sum_{i=k}^nq_ir_i^T =[q_k \cdots q_n]\left(\begin{array}{c}r_k^T\\ \vdots \\ r_n^T\end{array}\right) = q_k \cdots q_n]\left(\begin{array}{ccccccc} 
...
$$A^{(k)}e_k = z= [q_k \cdots q_n]\left(\begin{array}{c}r_{kk}\\ 0\\ \vdots \\ 0\end{array}\right)=r_{kk}q_k \Rightarrow r_{kk}=\|z\|_2, q_k=z/r_{kk}$$}
  \item \reponse{$$q_k^TA^{(k)} =[q_k^Tz, q_k^TB]=[1 , 0, \cdots , 0]\left(\begin{array}{c}r_k^T\\ \vdots \\r_n^T\end{array}\right)=r_k^T$$
et donc
$$[r_{k,k+1}, \cdots , r_{kn}]=q_k^TB$$}
  \item \reponse{$$[0,\cdots , 0, A^{(k+1)}] = \sum_{i=k+1}^n q_ir_i^T= [0,\cdots , 0, A^{(k)}] - q_kr_k^T= [0,\cdots , 0,A^{(k)} - q_k(r_{kk}, \cdots , r_{kn})]$$
$$[ 0,\cdots , 0, z-q_kr_{kk}, B-q_k(r_{k,k+1}, \cdots , r_{kn})] \Rightarrow A^{(k+1)}=B-q_k (r_{k,k+1}, \cdots , r_{kn})$$}
  \item \reponse{Donn\'ees: $A\in\Rr^{m\times n}, $ rank$(A)=n$
...
\hspace{2cm}  $a_{ij}\leftarrow a_{ij}-a_{ik}r_{kj}$}
  \item \reponse{complexit\'e: $mn^2$ flops.}
\end{enumerate}
}