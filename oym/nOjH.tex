...
\end{enumerate}
\begin{enumerate}
  \item \reponse{Montrons par récurrence que~:~$\forall n\geq1,\;\sum_{k=1}^{n}k=\frac{n(n+1)}{2}$.
...
\begin{itemize}}
  \item \reponse{[\textbf{1ère demonstration.}] Pour $k\geq1$, $(k+1)^2-k^2=2k+1$ et donc
...
$2\sum_{k=1}^{n}k=n^2+n$ ou enfin $\sum_{k=1}^{n}k=\frac{n(n+1)}{2}$.}
  \item \reponse{[\textbf{2ème demonstration.}]   On écrit
...
$$2S=\sum_{k=1}^{n}k+\sum_{k=1}^{n}(n+1-k)=\sum_{k=1}^{n}(k+n+1-k)=\sum_{k=1}^{n}(n+1)=n(n+1).$$}
  \item \reponse{[\textbf{3ème demonstration.}] On compte le nombre de points d'un rectangle ayant $n$ points de large et $n+1$
...
\end{array}
$$}
  \item \reponse{[\textbf{4ème démonstration.}] Dans le triangle de \textsc{Pascal}, on sait que pour $n$ et
...
\end{itemize}}
  \item \reponse{Pour $k\geq1$, $(k+1)^3-k^3=3k^2+3k+1$. Donc, pour $n\geq1$~:
...
\end{center}}
  \item \reponse{Soit $p$ un entier naturel. Pour $k\geq1$,
