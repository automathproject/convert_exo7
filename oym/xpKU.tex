\uuid{xpKU}
\exo7id{2212}
\titre{Quelques propri\'et\'es des normes matricielles}
...
\end{enumerate}
\begin{enumerate}
  \item \reponse{$\|A\|_2^2 =\rho (A^*A)$ rayon spectral de la matrice $A^*A$. D'un autre cot\^e on a:
...
o\`u tr est la trace de la matrice et $\lambda_i$ ses valeurs propres.}
  \item \reponse{$$\|A\|_2 \leq \|A\|_F =\left(\sum_{i,j}|a_{ij}|^2\right)^{1/2} \leq (mn\max_{i,j}|a_{ij}|^2 )^{1/2}=\sqrt{mn} \max_{i,j}|a_{ij}|$$
...
$\|Ax\|^2_2=\sum_{i=1}^n|a_{i,j_0}|^2 \geq \max |a_{ij}|^2 \Rightarrow \mbox{sup} \|Ax\|^2_2 \geq \max |a_{ij}|^2$}
  \item \reponse{On rappelle que $\|A\|_{\infty} =\sum_{j=1}^n |a_{i_0j}|$ pour un certain $i_0$. Alors
...
ce qui implique $\|A\|_2 \geq \|A\|_{\infty} /\sqrt{n}$}
  \item \reponse{M\^eme d\'emonstration que pr\'ec\'edemment ou alors constater que $\|A\|_1=\|A^T\|_{\infty}$.}
  \item \reponse{$\|E\|^2_F =\sum_{i,j} u_i^2v_j^2\sum_{i=1}^m\sum_{j=1}^n u_i^2v_j^2=\|u\|^2_2\|v\|^2_2$
