...
\organisation{exo7}
\contenu{
  \texte{On considère la famille de  plans $(P_m)_{m\in \Rr}$ définis par les équations cartésiennes :
$$ m^2x+(2m-1)y+mz=3$$}
\begin{enumerate}
...
\begin{enumerate}
  ...
  \item \reponse{Un point $A$ appartient à un plan d'équation $ax+by+cz+d=0$ si et seulement si 
$ax_A+by_A+cz_A+d=0$.
...
Donc $A$ appartient aux plans $P_1$ et $P_{-4}$ et pas aux autres.}
  \item \reponse{Un plan d'équation $ax+by+cz+d=0$ a pour vecteur normal $\vec n = (a,b,c)$.
...
Le seul plan ayant $\vec n$ pour vecteur normal est $P_{-2}$.}
  \item \reponse{Un vecteur est directeur du plan $P$ si et seulement si le produit scalaire
