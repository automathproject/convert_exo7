\uuid{UVKD}
\exo7id{2222}
\titre{Taille des \'el\'ements dans l'\'elimination de Gauss}
\theme{Méthode de Gauss. Factorisation LU et de Cholesky}
\auteur{matos}
\date{2008/04/23}
\organisation{exo7}
\contenu{
  \texte{Notons $\tilde{A}_k$ la matrice carr\'ee d'ordre $(n-k+1)$ form\'ee des \'el\'ements $a_{ij}^k , k\leq i,j \leq n$ de la matrice $A_k=(a_{ij}^k)$ obtenue come r\'esultat de la $(k-1)$--\`eme \'etape de l'\'elimination de Gauss. On suppose $A=A_1$ sym\'etrique d\'efinie positive.}
\begin{enumerate}
...
\end{enumerate}
\begin{enumerate}
  \item \reponse{A la k-\`eme \'etape de l'\'elimination de Gauss, l'\'el\'ement $a_{ij}^{k+1}$ est donn\'e par
...
$$(\tilde{A}_{k+1}v',v') +\frac{1}{a_{kk}^k} [a_kk^kv_k +\sum_{i=k+1}^na_{ik}^kv_i]^2$$}
  \item \reponse{Faisons un raisonnement par r\'ecurrence
\begin{itemize}}
  \item \reponse{$\tilde{A}_1$ est sym\'etrique d\'efinie positive;}
  \item \reponse{Par hypoth\`ese supposons que $\tilde{A}_k$ est d\'efinie positive;}
  \item \reponse{Supposons par absurde que $\tilde{A}_{k+1}$ ne soit pas d\'efinie positive: alors $\exists v'\neq 0:\quad (\tilde{A}_{k+1}v',v')\leq 0$. On d\'efinit le vecteur $v\in\Rr^{n-k+1}$ par:
\begin{itemize}}
  \item \reponse{$v_i=v'_i,\quad k+1\leq i\leq n$}
  \item \reponse{$v_k$ est solution de $a_{kk}^k +\sum_{i=k+1}^n a_{ik}^k v_i=0$
\end{itemize}
Alors $(\tilde{A}_kv,v)=0$ et $v\neq 0$; donc $\tilde{A}_k$ n'est pas d\'efinie positive, ce qui contredit l'hypoth\`ese de r\'ecurrence.
\end{itemize}}
  \item \reponse{Premi\`ere in\'egalit\'e: en utilisant la relation d'\'elimination on obtient: $a_{ii}^{k+1}=a_{ii}^k -\frac{\left|a_{ki}^k\right|^2}{a_{kk}^2}$
\begin{itemize}}
  \item \reponse{une matrice d\'efinie positive a tous ses \'el\'ements diagonaux strictement positifs, donc $a_{ii}^{k+1}>0$}
  \item \reponse{$\left|a_{ki}^k\right|^2/\left|a_{kk}^k\right|^2 \geq 0, \quad k+1\leq i\leq n$
\end{itemize}
