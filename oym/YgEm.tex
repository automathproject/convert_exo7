...
\exo7id{6981}
\titre{Exercice 6981}
\theme{Courbes planes, Courbes d'équation $y=f(x)$}
\auteur{blanc-centi}
\date{2015/07/04}
\organisation{exo7}
\contenu{
  \texte{Représenter les courbes d'équation cartésienne $y=f(x)$, donner l'équation de 
...
\end{enumerate}
\begin{enumerate}
  \item \reponse{Pour $f(x)=\sin^2x+\cos x$, le domaine de définition de $f$ est $\R$, et 
...
\ &1 &\ &\ &\ &-1 \\
\end{array}$$}
  \item \reponse{Tangentes horizontales\\
...
\end{tikzpicture}
\end{center}}
  \item \reponse{Pour $f(x)=x+\ln(1+e^x)$, le domaine de définition de $f$ est $\R$ et $f$ est de classe $\mathcal{C}^\infty$.
\begin{itemize}}
  \item \reponse{Variations de $f$\\
Comme $f'(x)=1+\frac{e^x}{1+e^x}$, pour tout $x$, $f'(x)>1$. En particulier $f$ est strictement croissante sur $\R$.}
  \item \reponse{Allure du graphe en $+\infty$\\
...
a en $-\infty$ une asymptote, d'équation $y=x$, et reste au-dessus de cette asymptote.}
  \item \reponse{Tangente au point d'abscisse $0$\\
