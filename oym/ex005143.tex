\uuid{5143}
\titre{Sommes télescopiques}
\theme{}
\auteur{rouget}
\date{2010/06/30}
\organisation{exo7}
\contenu{
  \texte{\label{exo:suprou7}
...
\end{enumerate}
\begin{enumerate}
  \item \reponse{Pour tout naturel non nul $k$, on a $\frac{1}{k(k+1)}=\frac{(k+1)-k}{k(k+1)}=\frac{1}{k}-\frac{1}{k+1}$,
et donc
...
\begin{itemize}}
  \item \reponse{[\textbf{- Calcul de} $\bf{S_1}$.] Posons $P_1=aX^2+bX+c$. On a
...
$$\sum_{k=1}^{n}k=\sum_{k=1}^{n}(P_1(k+1)-P_1(k))=P_1(n+1)-P_1(1)=\frac{n(n+1)}{2}.$$}
  \item \reponse{[\textbf{- Calcul de} $\bf{S_2}$.] Posons $P_2=aX^3+bX^2+cX+d$. On a
...
$$\sum_{k=1}^{n}k^2=\sum_{k=1}^{n}(P_2(k+1)-P_2(k))=P_2(n+1)-P_2(1)=\frac{n(n+1)(2n+1)}{6}.$$}
  \item \reponse{[\textbf{- Calcul de} $\bf{S_3}$.] Posons $P_3=aX^4+bX^3+cX^2+dX+e$. On a
...
$$\sum_{k=1}^{n}k^3=\sum_{k=1}^{n}(P_3(k+1)-P_3(k))=P_3(n+1)-P_3(1)=\frac{n^2(n+1)^2}{4}.$$}
  \item \reponse{[\textbf{- Calcul de} $\bf{S_4}$.] Posons $P_4=aX^5+bX^4+cX^3+dX^2+eX+f$. On a
