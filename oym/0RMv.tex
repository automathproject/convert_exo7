...
\organisation{exo7}
\contenu{
  \texte{Déterminer si les formes différentielles suivantes sont exactes et dans ce cas, les intégrer :}
\begin{enumerate}
...
\end{enumerate}
\begin{enumerate}
  \item \reponse{Pour $\omega_1$, on pose $P(x,y)=2xy$ et $Q(x,y)=x^2$. Comme $\omega_1$ est définie
...
o\`u $c$ est une constante réelle.}
  \item \reponse{Pour $\omega_2$, on pose $P(x,y,z)=xy$, $Q(x,y,z)=-z$ et $R(x,y,z)=xz.$ 
On constate que 
$ \frac{\partial P}{\partial y}=x$ alors que $ \frac{\partial Q}{\partial x}=0$. La forme
$\omega_2$ n'est donc pas exacte.}
  \item \reponse{Pour $\omega_3$, on pose $P(x,y)=2xe^{x^2-y}$ et $Q(x,y)=-2e^{x^2-y}$.
...
que $ \frac{\partial Q}{\partial x}=-4xe^{x^2-y}$ ; $\omega_3$ n'est donc pas exacte.}
  \item \reponse{Pour $\omega_4$, posons
$P(x,y,z)=yz^2$, $Q(x,y,z)=xz^2+z$, $R(x,y,z)=2xyz+2z+y$. On constate
que 
\begin{enumerate}}
  \item \reponse{$ \frac{\partial P}{\partial y}= \frac{\partial Q}{\partial x}= z^2$}
  \item \reponse{$ \frac{\partial P}{\partial z}= \frac{\partial R}{\partial x}=2zy$}
  \item \reponse{$ \frac{\partial Q}{\partial z}= \frac{\partial R}{\partial y}= 2xz+1$.}
\end{enumerate}
}