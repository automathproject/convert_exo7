...
\organisation{exo7}
\contenu{
  \texte{Calculer les déterminants suivants :}
\begin{enumerate}
...
\end{enumerate}
\begin{enumerate}
  \item \reponse{Sans modifier la valeur de $\text{det}A$, on effectue les transformations :$\forall j\in\llbracket1,n\rrbracket$, $C_j\leftarrow C_j+C_{2n+1-j}$.
...
\end{center}}
  \item \reponse{Ce déterminant a deux colonnes égales et est donc nul.}
  \item \reponse{On retranche la première colonne à toutes les autres et on obtient un déterminant triangulaire : $D_n=(-1)^{n-1}$.
...
$D_n=(-1)^{n-1}(n-1)$.}
  \item \reponse{On ajoute les $n-1$ dernières colonnes à la première puis on met $a+(n-1)b$ en facteur de la première colonne. On obtient
