\uuid{3192}
\titre{Polyn{\^o}me positif (Ens Ulm MP$^*$ 2003)}
\theme{}
\auteur{quercia}
\date{2010/03/08}
\organisation{exo7}
\contenu{
  \texte{}
  \question{Soit~$P\in\R[X]$. Montrer~:

($\forall\ x\ge 0,\ P(x) > 0$) $\Leftrightarrow$ ($\exists\ \ell \in\N$  tq $(X+1)^\ell P(X)$ est {\`a} coefficients strictement positifs).}
  \reponse{Le sens $\Leftarrow$ est trivial. Pour le sens $ \Rightarrow $, il suffit de v{\'e}rifier
