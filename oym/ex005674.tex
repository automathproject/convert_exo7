...
\organisation{exo7}
\contenu{
  \texte{$E =C^0(\Rr,\Rr)$. Pour $f$ élément de $E$, $\varphi(f)$ est l'application définie par :
...
\end{enumerate}
\begin{enumerate}
  \item \reponse{Soit $f\in C^0(\Rr,\Rr)$. Soit $F$ une primitive de $f$ sur $\Rr$. Pour tout $x\in\Rr^*$, on a $(\varphi(f))(x)=\frac{F(x)-F(0)}{x-0}$.
...
\end{center}}
  \item \reponse{Si $f$ est dans $\text{Ker}(\varphi)$ alors $f(0) = 0$ et pour tout $x$ non nul, $\int_{0}^{x}f(t)\;dt=0$. Par dérivation on obtient $\forall x\in\Rr^*$, $f(x) = 0$ ce qui reste vrai pour $x=0$ et donc $f = 0$. Finalement $\text{Ker}(\varphi)=\{0\}$ et $\varphi$ est injective.
...
\end{center}}
  \item \reponse{On cherche $\lambda\in\Rr$ et $f$ continue sur $\Rr$ et non nulle telle que $\forall x\in\Rr,\;(\varphi(f))(x) =\lambda f(x)$. D'après la question précédente, $0$ n'est pas valeur propre de $\varphi$ et donc nécessairement $\lambda\neq0$.
