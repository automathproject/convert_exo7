...
\organisation{exo7}
\contenu{
  \texte{$A$ et $B$ sont des parties d'un ensemble $E$. Montrer que :}
\begin{enumerate}
...
\end{enumerate}
\begin{enumerate}
  \item \reponse{Si $A=B=\varnothing$ alors $A\Delta B=\varnothing=A\cap B$.
...
qui est absurde. Donc $A=B=\varnothing$. Finalement, $A\Delta B=A\cap B\Leftrightarrow A=B=\varnothing$.}
  \item \reponse{Par distributivité de $\cap$ sur $\cup$,
...
\end{align*}}
  \item \reponse{$A\Delta B=(A\setminus B)\cup(B\setminus A)=(B\setminus A)\cup(A\setminus B)=B\Delta A$.}
...
deux ensembles peuvent donc se noter une bonne fois pour toutes $A\Delta B\Delta C$.}
  \item \reponse{$A=B\Rightarrow A\setminus B=\varnothing$ et $B\setminus A=\varnothing\Rightarrow A\Delta B =\varnothing$.
...
x\in B))\Rightarrow\exists x\in E/\;x\in(A\setminus B)\cup(B\setminus A)=A\Delta B\Rightarrow A\Delta B\neq\varnothing$.}
  ...
  \item \reponse{[$\Leftarrow$] Immédiat.}
  \item \reponse{[$\Rightarrow$] Soit $x$ un élément de $A$.
