...
\end{enumerate}
\begin{enumerate}
  \item \reponse{L'aire $\mathcal{A}$ du parallélogramme construit sur les vecteurs $\vec{u} = \left(\begin{array}{c}a\\c\end{array}\right)$ et 
...
où $\text{abs}$ désigne la fonction valeur absolue.}
  \item \reponse{Le volume  du  parall\'el\'epip\`ede construit sur trois vecteurs de $\Rr^3$ est la valeur absolue du déterminant
...
où l'on a développé par rapport à la première ligne.}
  \item \reponse{Si un parall\'el\'epip\`ede est construit sur trois vecteurs de $\Rr^3$
