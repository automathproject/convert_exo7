...
\organisation{exo7}
\contenu{
  \texte{Soit $E$ un $\Kk$-espace vectoriel et $f$ un élément de $\mathcal{L}(E)$.}
\begin{enumerate}
...
\end{enumerate}
\begin{enumerate}
  \item \reponse{On a toujours $\mbox{Ker}f\subset\mbox{Ker}f^2$.
...
$\mbox{Im}f=\mbox{Im}f^2$.}
  \item \reponse{$Id-p\;\mbox{projecteur}\Leftrightarrow(Id-p)^2=Id-p\Leftrightarrow Id-2p+p^2=Id-p\Leftrightarrow p^2=p\Leftrightarrow p\;\mbox{projecteur}$.
...
\end{align*}}
  \item \reponse{$p\circ q+q\circ p=0\Rightarrow p\circ q=(p\circ p)\circ q=p\circ(p\circ q)=-p\circ (q\circ p)$ et de même,
