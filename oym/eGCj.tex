\uuid{eGCj}
\exo7id{4261}
\titre{Fonctions affines}
\theme{Exercices de Michel Quercia, Intégrale de Riemann}
\auteur{quercia}
\date{2010/03/12}
\organisation{exo7}
\contenu{
  \texte{Soit $E = \mathcal{C}([a,b])$, et $F=\{f\in \mathcal{C}^2([a,b]),\text{ tel que } f(a)=f'(a)=f(b)=f'(b)=0\}$.}
\begin{enumerate}
...
\end{enumerate}
\begin{enumerate}
  \item \reponse{Il existe toujours une unique fonction $g$ de classe $\mathcal{C}^2$
    telle que $g'' = f$, $g(a)=g'(a)=0$~: $g(x) =  \int_{t=a}^x(x-t)f(t)\,d t$
    (Taylor-Intégral).}
  \item \reponse{Soient $\lambda,\mu\in \R$ tels que $f_1\ :\ x \mapsto f(x) - \lambda-\mu x$
