...
\end{enumerate}
\begin{enumerate}
  \item \reponse{\textbullet~Puisque $f$ est impaire, $f(0)=0$. Puisque $f$ est impaire et $2\pi$-périodique, $-f(\pi)=f(-\pi)=f(\pi)$ et donc $f(\pi)=0$. Puisque $f$ est $2\pi$-périodique, pour $k\in\Zz$, $f(2k\pi)=f(0)=0$ et $f((2k+1)\pi)=f(\pi)=0$. Finalement, $\forall k\in\Zz$, $f(k\pi)=0$.
...
\end{center}}
  \item \reponse{\textbullet~Soit $x\in[-\pi,0]$. Puisque $f$ est paire, $f(x)=f(-x)=\sin\left(-\frac{x}{2}\right)=\sin\left(\left|\frac{x}{2}\right|\right)$ et donc $\forall x\in[-\pi,\pi]$, $f(x)=\sin\left(\left|\frac{x}{2}\right|\right)$.
