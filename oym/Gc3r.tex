...
\end{enumerate}
\begin{enumerate}
  \item \reponse{En posant $y=x+1$, on a
...
\end{itemize}}
  \item \reponse{On a $K[x]/(f^{n}g^{m})\simeq K[x]/(f^{m})\times K[x]/(g^{m})$. On en
...
\end{array}\right.
$$}
  \item \reponse{Les idéaux de $K[x]/(f^{n})$ sont les idéaux engendrés par les
  ...
  \item \reponse{Les inversibles de $K[x]/(f^{n})$ sont les (classes des) polynômes
   ...
   de $f$ et $g$.}
  \item \reponse{Plus généralement, si les $f_{i}$ sont des polynômes irréductibles
