...
\organisation{exo7}
\contenu{
  \texte{\'Ecrire en coordonnée complexe :}
\begin{enumerate}
  \item \question{la rotation d'angle $\pi/4$ et de centre d'affixe $2+3i$;}
  \item \question{la réflexion d'axe d'équation  $y=2x+1$.}
\end{enumerate}
\begin{enumerate}
  \item \reponse{Par le cours, la rotation d'angle $\theta$ et de centre d'affixe $\omega$ est représentée par l'application 
...
\[ \C\to \C,\: z\mapsto e^{i\pi/4}(z-(2+3i)) + 2+3i = e^{i\pi/4}z +\frac{4 + \sqrt 2 + i(6-5\sqrt 2)}{2}.\]}
  \item \reponse{La réflexion d'axe d'équation  $y=2x+1$ est représentée par une application de la forme
