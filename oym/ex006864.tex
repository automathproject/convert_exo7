...
\organisation{exo7}
\contenu{
  \texte{Calculer les primitives suivantes par intégration par parties.}
\begin{enumerate}
...
\end{align*}}
  \item \reponse{$\int x \arctan x \, dx$
...
\end{align*}}
  \item \reponse{$\int \ln x \, dx$ puis $\int (\ln x)^2 \, dx$
...
Pour obtenir la dernière ligne on a utilisé la primitive calculée précédemment.}
  \item \reponse{Notons $I=\int \cos x\exp x \, dx$.
