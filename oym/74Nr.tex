...
\organisation{exo7}
\contenu{
  \texte{Calculez la norme des op\'erateurs suivants:}
\begin{enumerate}
...
\end{enumerate}
\begin{enumerate}
  \item \reponse{Soit $x$ une suite, on a
...
$$||T||=||g||_\infty.$$}
  \item \reponse{Soit $f \in {\mathcal C}([0,1])$, on a
...
deuxi\`eme in\'egalit\'e et on obtient $||u||=||g||_\infty$.}
  \item \reponse{si on prend $(x_n)=(a_n)$ on obtient $$u((a_n))=\sum a_n^2=
...
||(a_n)||_2$ d'o\`u l'\'egalit\'e $$||u||=||(a_n)||_2.$$}
  \item \reponse{Pour tout $j \in \mathbb{N}$ on a $|x_j| \leq ||(x_n)||_\infty$ et
