...
\organisation{exo7}
\contenu{
  \texte{Soit $X$ un ensemble. Pour $f \in \mathcal{F} (X, X)$, on d\'efinit $f^0 = id$ et
...
\end{enumerate}
\begin{enumerate}
  \item \reponse{Montrons la proposition demand\'ee par r\'ecurrence:
...
$$\forall \in \Nn \ \ f^n\circ f = f\circ f^n.$$}
  \item \reponse{On proc\`ede de m\^eme par r\'ecurrence:
