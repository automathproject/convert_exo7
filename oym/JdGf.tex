\uuid{JdGf}
\exo7id{5780}
\titre{*** I Matrices et déterminants de \textsc{Gram}}
\theme{Espaces préhilbertiens}
\auteur{rouget}
\date{2010/10/16}
\organisation{exo7}
\contenu{
  \texte{Soit $E$ un espace préhilbertien réel.
...
\end{enumerate}
\begin{enumerate}
  \item \reponse{Soit $F =\text{Vect}(x_1,...,x_n)$ et $m =\text{dim}F$. Soit $\mathcal{B}= (e_i)_{1\leqslant i\leqslant m}$ une base orthonormée de $F$ puis $M$ la matrice de la famille $(x_j)_{1\leqslant j\leqslant n}$ dans la base $\mathcal{B}$ .$M$ est une matrice rectangulaire de format $(m,n)$.
...
\end{center}}
  \item \reponse{\textbf{1ère solution.} Soit $x$ un vecteur de $E$ et $p_F(x)$ son projeté orthogonal sur $F$. Dans la première colonne de $\gamma(x,x_1,\ldots,x_n)$, le théorème de \textsc{Pythagore} permet d'écrire (puisque $x-p_F(x)\in F^\bot$)
