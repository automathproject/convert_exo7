...
\contenu{
  \texte{}
  \question{Soient $\Kk$ un sous-corps de $\Cc$ et $E$ et $F$ deux $\Kk$-espaces vectoriels de dimensions finies sur $\Kk$ et $u$ et $v$
deux applications linéaires de $E$ dans $F$. Montrer que ~:~
$|\mbox{rg}u-\mbox{rg}v|\leq\mbox{rg}(u+v)\leq\mbox{rg}u+\mbox{rg}v$.}
  \reponse{Par définition, $\mbox{rg }(u+v)=\mbox{dim }\left(\mbox{Im }(u+v)\right)$.
