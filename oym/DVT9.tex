...
\exo7id{2610}
\titre{Exercice 2610}
\theme{Sujets de l'année 2007-2008, Rattrapage}
\auteur{delaunay}
\date{2009/05/19}
\organisation{exo7}
\contenu{
  \texte{Soit $\alpha\in\R$ et $A_\alpha\in M_3(\R)$ la matrice suivante
...
\end{enumerate}
\begin{enumerate}
  \item \reponse{{\it Factorisons le polyn\^ome caract\'eristique $P_{A_\alpha}(X)$ en produit de facteurs du premier degr\'e.}
...
$$P_{A_\alpha}(X)=-(X+1)^2(X-\alpha+1).$$}
  \item \reponse{{\it D\'eterminons selon la valeur du param\`etre $\alpha$ les valeurs propres distinctes de $A_\alpha$ et leur multiplicit\'e.}
...
$\lambda_1=-1$ valeur propre double et $\lambda_2=\alpha-1$, valeur propre simple.}
  \item \reponse{{\it D\'eterminons les valeurs de $\alpha$ pour lesquelles la matrice $A_\alpha$ est diagonalisable.}
...
$(1,1,0)$, dans ce cas la matrice $A_\alpha$ n'est pas diagonalisable.}
  \item \reponse{{\it D\'eterminons selon la valeur de $\alpha$ le polyn\^ome minimal de $A_\alpha$.}
