...
\contenu{
  \texte{}
  \question{Soit $n\in\Nn$. Montrer que l'équation $y^{2n+1}+ y - x = 0$ définit implicitement une fonction $\varphi$ sur $\Rr$ telle que : $(\forall(x,y)\in\Rr^2),\;[y^{2n+1}+ y - x = 0\Leftrightarrow y =\varphi(x)]$.

Montrer que $\varphi$ est de classe $C^\infty$ sur $\Rr$ et calculer $\int_{0}^{2}\varphi(t)\;dt$.}
  \reponse{Soit $n\in\Nn$. Soit $x\in\Rr$. La fonction $f_x:y\mapsto y^{2n+1}+y-x$ est continue et strictement croissante sur $\Rr$ en tant que somme de fonctions continues et strictement croissantes sur $\Rr$. Donc la fonction $f_x$ réalise une bijection de $\Rr$ sur $]\lim_{y \rightarrow -\infty}f_x(y),\lim_{y \rightarrow +\infty}f_x(y)[=\Rr$. En particulier, l'équation $f_x(y)=0$ a une et une seule solution dans $\Rr$ que l'on note $\varphi(x)$.
