\uuid{4092}
\titre{$y'+ay = b$, $y(-\infty) = 0$}
\theme{}
\auteur{quercia}
\date{2010/03/11}
\organisation{exo7}
\contenu{
  \texte{Soit ${a,b} : \R \to \R$ continues telles que : $\forall\ x\in\R,\ a(x)\ge 1$
...
\end{enumerate}
\begin{enumerate}
  \item \reponse{$y =  \int_{t=\alpha}^x b(t)e^{A(t)-A(x)}\,d t + y(\alpha)e^{-A(x)}$
  ...
  \item \reponse{Comme $A(t)-A(x) \le t-x$ pour $t \le x$, l'intégrale
