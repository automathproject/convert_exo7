\uuid{GoLd}
\exo7id{7602}
\titre{Application entière dans $\mathbb{H}$}
...
\end{enumerate}
\begin{enumerate}
  \item \reponse{Soit $\Delta_r(a)$ le disque de centre $a\in\Cc$ et de rayon $r>0$. Supposons que $Im(f)$ ne rencontre pas $\Delta_r(a)$.
Alors,
...
Puisque $w\mapsto \frac{1}{w-a}$ est injective sur $\Cc-\{a\}$, en déduit que $f$ est constante.}
  \item \reponse{Une telle application entière ne rencontre par le disque $\Delta_1(-2i)$ : elle est donc constante,
d'après la question précédente.}
  \item \reponse{Soit $z\in\Cc-\{-i\}$,
\begin{eqnarray*}
...
= \frac{4 Im(z)}{|z+i|^2}.
\end{eqnarray*}}
  \item \reponse{Sur $\mathbb{H}$, $Im(z)>0$ et donc $|h(z)|<1$. L'image du demi-plan $\mathbb{H}$ par l'application $h$ est donc
incluse dans le disque unité $\Delta$ ; c'est donc une partie bornée de~$\Cc$.}
  \item \reponse{Soit $f :\Cc\to\mathbb{H}$ une application holomorphe.
