...
\organisation{exo7}
\contenu{
  \texte{Soit  $f$  une application de  ${\Rr}$  dans
...
\end{enumerate}
\begin{enumerate}
  \item \reponse{Cette assertion se d\'ecompose de la mani\`ere suivante : ( Pour tout $x\in
...
existe $x\in {\Rr},  f(x)>1$".}
  \item \reponse{Rappelons comment se traduit l'assertion ``L'application $f$ est
...
\Rr$ tels que $x_1 \leq x_2$ et $f(x_1) > f(x_2)$".}
  \item \reponse{La n\'egation est : ``l'application $f$ n'est pas croissante ou n'est pas
...
$f(x_1) \geq f(x_2)$, ou il existe $x \in \Rr,
  f(x)<0$".}
  \item \reponse{Cette assertion se d\'ecompose de la mani\`ere suivante : ``(Il existe $x\in
  ...
  \item \reponse{Cette assertion se d\'ecompose de la mani\`ere suivante : 
