\uuid{5388}
\titre{****}
\theme{Limites. Continuité en un point}
\auteur{rouget}
\date{2010/07/06}
\organisation{exo7}
\contenu{
  \texte{}
  \question{Etudier l'existence d'une limite et la continuité éventuelle en chacun de ses points de la fonction définie sur $]0,+\infty[$ par $f(x)=0$ si $x$ est irrationnel et $f(x)=\frac{1}{p+q}$ si $x$ est rationnel égal à $\frac{p}{q}$, la fraction $\frac{p}{q}$ étant irréductible.}
  \reponse{Soit $a$ un réel strcitement positif. On peut déjà noter que $\lim_{x\rightarrow a,\;x\in\Rr\setminus\Qq}f(x)=0$. Donc, si $f$ a une limite quand $x$ tend vers $a$, ce ne peut être que $0$ et $f$ est donc discontinue en tout rationnel strictement positif.
