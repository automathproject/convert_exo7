...
\organisation{exo7}
\contenu{
  \texte{Soit $E$ un espace euclidien de dimension~$n$ et $p$ endomorphismes
...
\end{enumerate}
\begin{enumerate}
  \item \reponse{$u_1+\dots+u_p$ est l'endomorphisme autoadjoint associé à
         $q_1+\dots+q_p$.}
  \item \reponse{$\Im(u_1)+\dots+\Im(u_p) \supset\Im(u_1+\dots+u_p) = E$ et la somme
         des dimensions est égale à~$\dim E$ donc la somme des sous-espaces est directe.}
  \item \reponse{On a $\mathrm{Ker}(u_1) = \{x\in E\text{ tq }x = u_2(x) + \dots + u_p(x)\}
