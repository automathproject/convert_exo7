...
\organisation{exo7}
\contenu{
  \texte{Soit $f:[a,b]\rightarrow \R$ une fonction continue sur $[a,b]$ ($a<b$).}
\begin{enumerate}
...
\end{enumerate}
\begin{enumerate}
  \item \reponse{\'Ecrivons la continuité de $f$ en $x_0$ avec $\epsilon = \frac {f(x_0)}{2} > 0$ :
...
$f$ est identiquement nulle.}
  \item \reponse{Soit $f$ est tout le temps positive, soit elle tout le temps négative, soit elle change 
...
le théorème des valeurs intermédiaires affirme qu'il existe $c$ tel que $f(c)=0$.}
  \item \reponse{Posons $g(x) = f(x)-x$. Alors $\int_0^1 g(x) dx = \int_0^1 \big( f(x) - x \big)  dx= \int_0^1 f(x) dx - \frac 12 = 0$.
