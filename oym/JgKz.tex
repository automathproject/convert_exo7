...
\end{enumerate}
\begin{enumerate}
  \item \reponse{Vrai. $\text{Vect}\{(1,1,0,0),(-1,1,-4,2)\}$ est inclus dans $\text{Vect} \{v_1,v_2,v_3\}$, car
...
(autrement dit : comme un plan est inclus dans un autre alors ils sont égaux).}
  \item \reponse{Vrai. On a $(1,1,0,0) = v_1+v_2$ donc  $(1,1,0,0)\in \text{Vect}\{v_1,v_2\}$, or 
$\text{Vect}\{v_1,v_2\}=\text{Vect}\{v_2,v_3\}\subset \text{Vect}\{v_2,v_3,v_4\}$. 
Donc $(1,1,0,0) \in \text{Vect}\{v_1, v_2\} \cap    \text{Vect}\{v_2, v_3, v_4\}$.}
  \item \reponse{Faux. Toujours la m\^eme relation nous donne que 
...
C'est donc un plan vectoriel et pas une droite.}
  \item \reponse{Faux. Encore une fois la relation donne que 
$\text{Vect}\{v_1, v_2\}+\text{Vect}\{v_2, v_3, v_4\} = \text{Vect}\{v_1, v_2, v_4\}$, or $3$ vecteurs ne
   peuvent engendrer $\Rr^4$ qui est de dimension $4$.}
  \item \reponse{Vrai.  Faire le calcul : l'intersection est $\{0\}$ et la somme est $\Rr^4$.}
\end{enumerate}
}