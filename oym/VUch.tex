...
\organisation{exo7}
\contenu{
  \texte{Soit $(X,d)$ un espace m\'etrique compact et $f:X\to X$ une application
v\'erifiant
...
\end{enumerate}
\begin{enumerate}
  \item \reponse{Si $f$ a deux points fixes $x\neq y$, alors $d(x,y) = d(f(x),f(y)) < d(x,y)$. Ce qui est absurde. Donc $f$ a au plus un point fixe.}
  \item \reponse{$f$ est continue et $X$ compact donc $X_1 = f(X)$ est compact,
...
$f(X_1) \subset f(X)$ soit $X_2 \subset X_1$, etc. Par récurrence $X_n \subset X_{n-1} \subset \cdots \subset X_1 \subset X$. Comme chaque $X_n$ est non vide alors $Y$ n'est pas vide (voir l'exercice \ref{exo 2}).}
  \item \reponse{Montrons d'abord que $f(Y) \subset Y$. Si $y\in Y$, alors pour tout $n\ge 0$ on a $y \in X_n$ donc $f(y) \in f(X_n)=X_{n+1}$ pour tout $n\ge 0$.
...
Soit $y \neq y'\in Y$ tel que $d(y,y')= \mathrm{diam}\,Y > 0$. Comme $Y=f(Y)$ alors il existe $x,x' \in Y$ tel que $y=f(x)$ et $y'=f(x')$. Or $d(y,y')=d(f(x),f(x')) < d(x,x')$. On a trouvé deux élements de $Y$ tel $d(x,x')$ est strictement plus grand que le diamètre de $Y$ ce qui est absurde. Donc $y=y'$ et le diamètre est zéro.}
  \item \reponse{Comme le diamètre est zéro alors $Y$ est composé d'un seul point $\{ p \}$ et comme $f(Y)=Y$ alors $f(p)=p$. Donc $p$ a un point fixe et nous savons que c'est le seul. Par la construction de $Y$ pour tout point $x_0 \in X$ la suite $x_n = f^n(x_0)$ converge vers $p$.}
\end{enumerate}
}