...
\contenu{
  \texte{}
  \question{Soient $a_1$,..., $a_n$, $b_1$,..., $b_n$ $2n$ nombres complexes deux à deux distincts tels que les sommes $a_i+b_j$ soient toutes non nulles. Résoudre le système $\sum_{j=1}^{n}\frac{x_j}{a_i+b_j}=1$, pour tout $i=1,...,n$ (en utilisant la décomposition en éléments simples de $R=\sum_{j=1}^{n}\frac{x_j}{X+b_j}$).}
  \reponse{Soit $(x_1,...,x_n)\in\Rr^n$ et $F=\sum_{k=1}^{n}\frac{x_k}{X+b_k}$.
