...
\contenu{
  \texte{}
  \question{Soit $f$ une application de classe $C^2$ sur $\Rr$ à valeurs dans $\Rr$ telle que $\forall x\in\Rr$, $f(x)+f''(x)\geqslant 0$. Montrer que $\forall x\in\Rr$, $f(x)+f(x+\pi)\geqslant 0$.}
  \reponse{On pose $g=f+f''$. Par hypothèse, la fonction $g$ est une application continue et positive sur $\Rr$ et de plus, la fonction $f$ est solution sur $\Rr$ de l'équation différentielle $y''+y=g$ sur $\Rr$. Résolvons cette équation différentielle, notée $(E)$, sur $\Rr$.
