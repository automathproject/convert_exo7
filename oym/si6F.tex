...
\organisation{exo7}
\contenu{
  \texte{Déterminer les développements limités à l'ordre demandé au voisinage des points indiqués~:}
\begin{enumerate}
...
\end{center}}
  \item \reponse{\textbf{Remarques.}

  \begin{enumerate}}
  \item \reponse{Pour $x\in\left]-\frac{\pi}{2},\frac{\pi}{2}\right[\setminus\{0\}$, on a $0<\frac{x}{\tan x}<1$ et donc la fonction $x\mapsto\Arccos\left(\frac{x}{\tan x}\right)$ est définie sur $\left]-\frac{\pi}{2},\frac{\pi}{2}\right[\setminus\{0\}$ (qui est un voisinage pointé de $0$).\rule[-2mm]{0mm}{6mm}}
  \item \reponse{Quand $x$ tend vers $0$, $\frac{x}{\tan x}\rightarrow1$ et donc $\Arccos\left(\frac{x}{\tan x}\right)=o(1)$ (développement limité à l'ordre $0$).}
  \item \reponse{La fonction $x\mapsto\Arccos x$ n'est pas dérivable en $1$ et n'admet donc pas en $1$ de développement limité d'ordre supérieur ou égal à $1$ (donc à priori, c'est mal parti).}
  \item \reponse{La fonction proposée est paire et, si elle admet en $0$ un développement limité d'ordre $3$, sa partie régulière ne contient que des exposants pairs.}
\end{enumerate}
}