\uuid{3597}
\titre{Matrice bloc}
\theme{}
\auteur{quercia}
\date{2010/03/10}
\organisation{exo7}
\contenu{
  \texte{Soit $A \in \mathcal{M}_n(K)$ et $M = \begin{pmatrix} A &0 \cr A &A \cr\end{pmatrix} \in \mathcal{M}_{2N}(K)$.}
\begin{enumerate}
  \item \question{Comparer les valeurs propres de $A$ et $M$.}
  \item \question{Soit $P \in  K[X]$ et $Q = XP'$.
    Montrer que $P(M) = \begin{pmatrix} P(A) & 0 \cr Q(A) & P(A) \cr \end{pmatrix}$.}
  \item \question{A quelle condition sur $A$, $M$ est-elle diagonalisable ?}
\end{enumerate}
\begin{enumerate}
  \item \reponse{Par récurrence pour $P = X^k$, puis par linéarité.}
  \item \reponse{$A=0$.}
\end{enumerate}
}