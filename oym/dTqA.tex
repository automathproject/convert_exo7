...
\organisation{exo7}
\contenu{
  \texte{D\'ecrire la boule de centre
...
\end{enumerate}
\begin{enumerate}
  \item \reponse{On a par définition $B(0,1)=\{x \in \mathbb{R}; |x-0|=|x| <
1\}=[-1,1].$}
  \item \reponse{C'est la norme euclidienne sur $\mathbb{R}^2$,
$B_1(0,1)=\{(x,y)\in \mathbb{R}^2; \sqrt{x^2+y^2}=1\}$ c'est le
disque de centre l'origine et de rayon $1$.}
  \item \reponse{$B_2(0,1)=\{(x,y); |x|<1 et |y|<1 \}.$ C'est un carr\'e.}
  \item \reponse{$B_3(0,1)=\{(x,y); |x|+|y|<1\}.$ Dans le quart de plan
