...
\contenu{
  \texte{}
  \question{Pour $x\in]-1,1[$, on pose $f(x)=\sum_{n=1}^{+\infty}x^{n^2}$. Trouver un équivalent simple de $f$ en $1$.}
  \reponse{Soit $x\in]-1,1[$. Pour $n\in\Nn^*$, $\left|x^{n^2}\right|=|x|^{n^2}\leqslant|x|^n$. Puisque la série numérique de terme général $|x|^n$ converge, on en déduit que la série de terme général $x^{n^2}$ est absolument convergente et en particulier convergente. Donc, $f$ est bien définie sur $]-1,1[$.
