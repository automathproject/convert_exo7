...
\organisation{exo7}
\contenu{
  \texte{On se donne une m\'etrique $d$ sur $X=[0,1]$ telle que
...
\end{enumerate}
\begin{enumerate}
  \item \reponse{Soit $K$ un compact pour $|.|$. Soit $U_i$, $i\in I$ tels
...
Donc compact pour $d$, donc fermé pour $d$.}
  \item \reponse{Si $U$ est un ouvert pour $d$ alors $U$ est un ouvert pour $|.|$.
