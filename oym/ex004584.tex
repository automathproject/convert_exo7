\exo7id{4584}
\titre{Série des traces (Centrale MP 2003)}
\theme{}
\auteur{quercia}
\date{2010/03/14}
\organisation{exo7}
...
\end{enumerate}
\begin{enumerate}
  \item \reponse{$\chi_A(\lambda) = -\lambda^3 + 2\lambda^2 + \lambda - 1$.
    $\chi_A(-1)>0$, $\chi_A(0) < 0$, $\chi_A(1)>0$, $\chi_A(2)>0$, $\chi_A(3)<0$
    donc $\chi_A$ admet une racine dans chacun des intervalles
    $]-1,0[$, $]0,1[$ et $]2,3[$.}
  \item \reponse{Cayley-Hamilton~: $t_n = 2t_{n-1}+t_{n-2}-t_{n-3}$.}
  \item \reponse{Soient $-1<\alpha<0<\beta<1<2<\gamma<3$ les valeurs propres de~$A$.
