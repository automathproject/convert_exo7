\uuid{44ft}
\exo7id{3575}
\titre{$\frac12(p\circ u + u\circ p)$ (Mines MP 2003)}
\theme{Exercices de Michel Quercia, Réductions des endomorphismes}
\auteur{quercia}
\date{2010/03/10}
\organisation{exo7}
\contenu{
  \texte{Soit~$E$ un espace vectoriel de dimension $n$ finie, $p$ un projecteur de rang~$r$
...
\end{enumerate}
\begin{enumerate}
  \item \reponse{Oui, les applications $u \mapsto p\circ u$ et $u \mapsto u\circ p$ le sont
    (ce sont des projecteurs) et elles commutent.}
  \item \reponse{Soit $\cal B$ une base de~$E$ obtenue par concaténation d'une
