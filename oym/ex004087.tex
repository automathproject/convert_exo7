...
\organisation{exo7}
\contenu{
  \texte{Soit $\lambda\in\R$ et $({\cal E})\Leftrightarrow \frac{d^2 u(x)}{d x^2} + (1-\lambda)x^2u(x) = 0$.}
\begin{enumerate}
...
\end{enumerate}
\begin{enumerate}
  \item \reponse{Il suffit de démontrer que les solutions de~$({\cal E})$ sont développables en série entière.
  ...
  \item \reponse{On doit avoir $H''(x) - 2xH'(x) + ((2-\lambda)x^2 - 1)H(x) = 0$.
