\exo7id{6952}
\titre{Calcul d'espérance et de variance}
\theme{}
\auteur{ruette}
\date{2013/01/24}
\organisation{exo7}
...
\end{enumerate}
\begin{enumerate}
  \item \reponse{$E(X)=\displaystyle \int_0^{+\infty}x\lambda e^{-\lambda x}dx=\frac{1}{\lambda}$ par intégration par parties.}
  \item \reponse{$E(X)=\displaystyle\sum_{k\geq 0} e^{-\lambda}\frac{\lambda^k}{k!}k
