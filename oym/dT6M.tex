...
\exo7id{5970}
\titre{Exercice 5970}
\theme{Compléments d'intégration, Séparabilité des $L^{p}
\auteur{tumpach}
\date{2010/11/11}
\organisation{exo7}
\contenu{
  \texte{\textbf{Th\'eor\`eme.}
...
\end{enumerate}
\begin{enumerate}
  \item \reponse{Soit $E$ un espace de Banach. On suppose qu'il existe une
famille $(O_{i})_{i\in I}$ telle que
\begin{itemize}}
  \item \reponse{Pour tout $i\in I$, $O_i$ est un ouvert non vide de
$E$.}
  \item \reponse{$O_i \cap O_j = \emptyset $ si $i\neq j$.}
  \item \reponse{$I$ n'est pas d\'enombrable.
\end{itemize}
...
d\'enombrable ce qui contredit (c).}
  \item \reponse{Pour tout $a\in \mathbb{R}^{n}$,  on pose $f_{a} =
\mathbf{1}_{\mathcal{B}(a, 1)}$ o\`u $\mathcal{B}(a, 1)$ est la boule de
