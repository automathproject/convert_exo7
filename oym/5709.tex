...
\contenu{
  \texte{}
  \question{Soit $p\in\Nn^*$, calculer $\sum_{p\in\Nn^*}^{}\left(\sum_{n\in\Nn^*,\;n\neq p}^{}\frac{1}{n^2-p^2}\right)$ et $\sum_{n\in\Nn^*}^{}\left(\sum_{p\in\Nn^*,\;p\neq n}^{}\frac{1}{n^2-p^2}\right)$. Que peut-on en déduire ?}
  \reponse{Soit $p\in\Nn^*$. Pour $n\in\Nn^*\setminus\{p\}$, $\frac{1}{n^2-p^2}=\frac{1}{2p}\left(\frac{1}{n-p}-\frac{1}{n+p}\right)$. Donc pour $N > p$,
