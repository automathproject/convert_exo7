...
\organisation{exo7}
\contenu{
  \texte{On consid\`ere la fonction $f : \R \to \R$ d\'efinie par
\begin{equation*}
f(t) =
\begin{cases}
  e^{1/t} & \mathrm{\ si\ } t<0\\
...
\end{enumerate}
\begin{enumerate}
  \item \reponse{$f$ est d\'erivable sur $\R_-^*$ en tant que compos\'ee de fonctions
...
$f''(0)=0$.}
  ...
  \item \reponse{On a d\'ej\`a trouv\'e que $f'(t)=-e^{1/t}/t^2$, donc  $f'(t)=P_1(t)/t^2
...
$n=2$ en posant $P_2(t)=1+2t$.}
  \item \reponse{Supposons que la formule est vraie au rang $n$.
