\uuid{4135}
\titre{Système autonome, ENS Ulm-Lyon-Cachan MP$^*$ 2006}
\theme{}
\auteur{quercia}
\date{2010/03/11}
\organisation{exo7}
\contenu{
  \texte{On considère le système différentiel : $$(V) \Leftrightarrow\left\{\begin{array}{l} x' = x(1-y) \\ y' = y(x-1).\\ \end{array}\right.$$
...
\end{enumerate}
\begin{enumerate}
  \item \reponse{$\frac{d}{d t}f(x,y) = x'\frac{\partial f}{\partial x} + y'\frac{\partial f}{\partial y}$ donc $f$ convient
...
d'où $f(x,y) = x+y-\ln(xy)$.}
  \item \reponse{D'après le théorème d'unicité de Cauchy-Lipschitz,
