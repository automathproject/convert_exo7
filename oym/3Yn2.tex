...
\exo7id{7410}
\titre{Exercice 7410}
\theme{Exercices de Christophe Mourougane, 106, 107, 108 - Algèbre linéaire}
\auteur{mourougane}
\date{2021/08/10}
\organisation{exo7}
\contenu{
  \texte{Dans $\Rr^4$ on considère les quatre vecteurs
...
\end{enumerate}
\begin{enumerate}
  \item \reponse{On observe que $v_3 = v_2 - v_1$ et que $v_4=v_1+v_2$ donc $V=Vect(v_1,v_2,v_3,v_4)=Vect(v_1,v_2)$. Comme de plus $v_1$ et $v_2$ sont linéairement indépendants, $(v_1,v_2)$ est une base de $V$. Donc $\dim(V)=2$. Le système n'est pas libre car par exemple $v_3=v_2-v_1$ est une relation de dépendance linéaire non triviale. Le système n'est pas générateur de $\Rr^4$ car $\dim(V)=2<4$ donc $Vect(v_1,v_2,v_3,v_4)=V\neq \Rr^4$.}
  \item \reponse{Comme nous l'avons vu dans la question 1, $(v_1,v_2)$ est une base de $V$. On échelonne la matrice dont les lignes sont les vecteurs $v_1$ et $v_2$.
...
\]
est de rang 4.}
  \item \reponse{$V=Vect(v_1,v_2)$. Maintenant soit $(x,y,z,t)\in \Rr^4$. $(x,y,z,t)\in V$ si et seulement si $\exists \lambda,\mu \in \Rr $ tels que $(x,y,z,t)=\lambda v_1 + \mu v_2$.
...
$$V=\{(x,y,z,t)\in\Rr^4 \ / \ \ x-2y+z=0 \ \ et \ \ -x+y+t=0 \}.$$}
  \item \reponse{$H$ est un sous-espace vectoriel de $\Rr^4$ car c'est l'ensemble des solutions d'une équation linéaire et homogène.}
  \item \reponse{\begin{eqnarray}
H &=& \{(x,y,z,t)\in\Rr^4 \ / \ \ -3x+y+2z-t=0 \} \nonumber \\
...
\]
Et donc $\dim(H)=3$.}
  \item \reponse{$v_3\in H$ car $-3\times 1 +1\times 1 + 2\times 1 -1\times 0 = 0$. Mais $v_1\notin H$ car $-3\times 1 +1\times 0 + 2\times -1 -1\times 1 = -6\neq 0$.
...
En utilisant la formule de Grassmann on obtient alors que $$\dim (V+H)=\dim (V)+\dim (H)-\dim (V\cap H)=2+3-1=4.$$}
  \item \reponse{On a vu que $\dim(V\cap H)=1$ et que $Vect(v_3)\in V\cap H$ donc on obtient $Vect(v_3)= V\cap H$ et donc $(v_3)$ est une base de $V\cap H$.}
\end{enumerate}
}