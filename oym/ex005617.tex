\uuid{5617}
\titre{***I Théorème de \textsc{Hadamard}}
\theme{}
\auteur{rouget}
\date{2010/10/16}
\organisation{exo7}
\contenu{
  \texte{}
  \question{Soit $A=(a_{i,j})_{1\leqslant i,j\leqslant n}\in\mathcal{M}_n(\Cc)$ telle que $\forall i\in\llbracket1,n\rrbracket$, $|a_{i,i}| >\sum_{j\neq i}^{}|a_{i,j}|$. Montrer que $A\in\mathcal{GL}_n(\Cc)$. (Une matrice à diagonale strictement dominante est inversible.)}
  \reponse{Montrons que $\text{Ker}A$ est réduit à $\{0\}$. Dans le cas contraire, on dispose d'un vecteur colonne non nul $X_0$ tel que $AX_0 = 0$. Posons $X_0=(x_i)_{1\leqslant i\leqslant n}$. Pour tout $i\in\llbracket1,n\rrbracket$,
