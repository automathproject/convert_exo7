...
\exo7id{5931}
\titre{Exercice 5931}
\theme{Théorème de Carathéodory, calcul d'aire et de volume, Aire de $\mathcal{S}
\auteur{tumpach}
\date{2010/11/11}
\organisation{exo7}
\contenu{
...
\end{enumerate}
\begin{enumerate}
  \item \reponse{Soit $I = \int_{-\infty}^{+\infty} e^{-x^2}\,dx$. On a~:
$$
...
Ainsi $I = \int_{-\infty}^{+\infty} e^{-x^2}\,dx  = \sqrt{\pi}$.}
  \item \reponse{\emph{Calcul de l'aire de la sph\`ere unit\'e de
...
\frac{n}{2}\right)}.
$$}
  \item \reponse{\emph{Calcul du volume de la boule unit\'e de
$\mathbb{R}^{n}$.} Soit $ \mathcal{B}_{n} = \{(x_1, \dots, x_n)
\in \mathbb{R}^n ,\, \sum_{i=1}^{n} x_{i}^2 \leq 1 \} $ la boule
ferm\'ee de rayon 1 dans $\mathbb{R}^n$. On note $\mathcal{V}_{n}$
son volume. On a~:
...
en utilisant l'identit\'e~: $\Gamma(s + 1) = s\Gamma(s)$.}
  \item \reponse{\emph{Application~:} L'aire de la sph\`ere de rayon $R$ dans
