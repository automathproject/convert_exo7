...
\organisation{exo7}
\contenu{
  \texte{Soit $E$ un espace euclidien et $s$ une symétrie de~$E$.}
\begin{enumerate}
...
\end{enumerate}
\begin{enumerate}
  \item \reponse{c'est un endomorphisme autoadjoint positif de déterminant~$1$.}
  \item \reponse{$X^n\det\Bigl(\frac{\mathrm{id}}X+s^*\circ s\Bigr) = \det(\mathrm{id}+Xs^*\circ s)
             =\det(s^*\circ(\mathrm{id}+Xs^*\circ s)\circ s) = \det(s^*\circ s + X\mathrm{id})$.}
  \item \reponse{$s^*\circ s$ est diagonalisable avec des valeurs propres $(\lambda_i)$
  ...
  \item \reponse{Se ramener au cas $A_4=I$ puis calculer $\det A$ par pivotage.}
\end{enumerate}
}