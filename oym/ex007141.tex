\exo7id{7141}
\titre{Puissance d'un point par rapport à un cercle}
\theme{}
\auteur{megy}
\date{2017/04/05}
\organisation{exo7}
\contenu{
  \texte{Soit $\mathcal C$ un cercle et $P$ un point du plan.}
\begin{enumerate}
...
\end{enumerate}
\begin{enumerate}
  \item \reponse{Soit $\mathcal D'$  une autre droite passant par $P$ et intersectant le cercle en deux points $A'$ et $B'$. 
...
\]}
  \item \reponse{On procède de la même manière, en utilisant la version tangentielle du théorème de l'angle inscrit.}
  \item \reponse{Avec les notations de la question précédente, on a par le théorème de Pythagore $PT^2+r^2=PO^2$, d'où le résultat.}
  \item \reponse{L'ensemble est donc $\{P\in \mathcal P,\: ||\overrightarrow{OP}||=\lambda + r^2\}$ Si $\lambda < -r^2$, l'ensemble est vide. Si $\lambda=-r^2$, c'est le centre du cercle, et si $\lambda > -r^2$, c'est un cercle de centre $O$ et de rayon $\sqrt{\lambda + r^2}$.}
  \item \reponse{Si les deux produits sont nuls, alors $P$ coïncide avec l'un des deux points $A$ et $B$, ainsi qu'avec l'un des deux points $C$ et $D$. On en déduit que $ABCD$ est en fait un triangle et qu'il est donc inscriptible.
