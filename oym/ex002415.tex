...
\organisation{exo7}
\contenu{
  \texte{Soient $(E,d)$ un espace m\'etrique et ${\cal H}$ une famille \'equicontinue d'appli\-cations
...
\begin{enumerate}
  ...
  \item \reponse{Montrons que $A$ est ouvert. Soit $x\in A$, alors $\mathcal{H}(x) = \{f(x) \mid f\in \mathcal{H}\}$ est bornée, notons $M$ une borne. \'Ecrivons l'équicontinuité pour $\epsilon = 1$, il existe $\eta >0$ tel que
...
Donc $B(x,\eta) \subset A$. Donc $A$ est ouvert.}
  \item \reponse{Montrons que $A$ est fermé. Soit $(x_n)$ une suite d'éléments de $A$
