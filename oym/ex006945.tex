\uuid{6945}
\titre{Fonctions génératrices}
\theme{}
\auteur{ruette}
\date{2013/01/24}
\organisation{exo7}
\contenu{
  \texte{Soit $X$ une variable aléatoire de loi binomiale $\mathcal{B}(n,p)$.}
\begin{enumerate}
...
\end{enumerate}
\begin{enumerate}
  \item \reponse{$\displaystyle G_X(z)=\sum_{k=0}^nC^k_np^k(1-p)^{n-k}z^k
=\sum_{k=0}^nC^k_n(pz)^k(1-p)^{n-k}$ donc $G_X(z)=(pz+1-p)^n$.}
  \item \reponse{$G_Y(z)=(pz+1-p)^m$ par a). Par indépendance,
