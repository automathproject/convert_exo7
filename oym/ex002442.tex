...
\organisation{exo7}
\contenu{
  \texte{Pour toute matrice carrée $A$ de dimension $n$, 
...
\end{enumerate}
\begin{enumerate}
  \item \reponse{Notons $C=AB$ et $D=BA$.
...
$$\textrm{tr}(BA) =\sum_{1\le k \le n} \sum_{1\le i \le n} b_{ki}a_{ik} =  \sum_{1\le i \le n}\sum_{1\le k \le n} a_{ik}b_{ki} =\textrm{tr}(AB)$$}
  \item \reponse{$M$ et $M'$ sont semblables donc il existe une matrice de passage $P$ telle que $M'=P^{-1}MP$ donc
$$\textrm{tr}\, M' = \textrm{tr}\big( P^{-1}(MP) \big) = \textrm{tr}\big( (MP)P^{-1} \big) = \textrm{tr} ( M I ) = \textrm{tr}\, M$$}
  \item \reponse{La trace a aussi la propriété évidente que 
