\uuid{3873}
\titre{Suite $(f(nx))$ ENS Cachan MP$^*$ 2004}
\theme{}
\auteur{quercia}
\date{2010/03/11}
\organisation{exo7}
\contenu{
  \texte{}
  \question{Soit $f : \R \to \R$ une fonction uniformément continue telle que pour
tout~$x>0$ la suite $(f(nx))_{n\in\N}$ est convergente. Que peut-on dire
de~$f$~?}
  \reponse{Soit pour $x>0$, $\ell(x) = \lim_{n\to+\infty} f(nx)$. On a $\ell(kx) = \ell(x)$
