...
\contenu{
  \texte{}
  \question{Dans $E=\Rr^n$, on considère l'hyperplan $H$ d'équation $x_1+...+x_n=0$ dans la base canonique $(e_i)_{1\leq i \leq n}$ de $E$. Pour $\sigma\in S_n$ donnée, on considère l'endomorphisme $f_\sigma$ de $E$ défini par~:~$\forall i\in E,\;f_\sigma(e_i)=e_{\sigma(i)}$.
...
On pose alors $p=\frac{1}{n!}\sum_{\sigma\in S_n}^{}f_\sigma$. Montrer que $p$ est une projection dont on déterminera l'image et la direction.}
  \reponse{Pour $(x_1,...,x_n)\in E$, on pose $\varphi((x_1,...,x_n))=x_1+...+x_n$. $\varphi$ est une forme linéaire non nulle sur $E$ et $H$ est le noyau de $\varphi$. $H$ est donc bien un hyperplan de $E$.
