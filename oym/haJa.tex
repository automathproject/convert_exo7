...
\end{enumerate}
\begin{enumerate}
  \item \reponse{Soit $f$ une fonction dérivable sur $\Rr$ à valeurs dans $\Rr$. Si $f$ est paire, alors, pour tout réel $x$,
...
\end{center}}
  \item \reponse{Soient $n\in\Nn^*$ et $f$ une fonction $n$ fois dérivable sur $\Rr$ à valeurs dans $\Rr$. Supposons $f$
...
De même, si $f$ est impaire et $n$ fois dérivable sur $\Rr$, $f^{(n)}$ a la parité contraire de celle de $n$.}
  \item \reponse{Soit $f$ une fonction continue sur $\Rr$ et impaire et $F$ une primitive de $f$. Montrons que $F$ est paire.
...
mais $F~:~x\mapsto x+1$ est une primitive de $f$ qui n'est pas impaire.}
  \item \reponse{On montre aisément en dérivant une ou plusieurs fois l'égalité~:~$\forall x\in\Rr,\;f(x+T)=f(x)$, que les
