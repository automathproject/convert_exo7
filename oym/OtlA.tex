\uuid{OtlA}
\exo7id{5766}
\titre{*** I (très long) Intégrale de \textsc{Poisson}}
\theme{Intégrales dépendant d'un paramètre}
\auteur{rouget}
\date{2010/10/16}
\organisation{exo7}
\contenu{
  \texte{Pour $x\in\Rr$, on pose $F(x)=\int_{-\pi}^{\pi}\ln(1-2x\cos\theta+x^2)\;d\theta$.}
\begin{enumerate}
...
\begin{enumerate}
  ...
  \item \reponse{\textbf{Parité de $F$.} Soit $x$ un réel du domaine de définition de $F$. En posant $t=\theta+\pi$, on obtient
...
\end{center}}
  \item \reponse{\textbf{Calcul de $F'(x)$.} Soit $x\in\Rr\setminus\{-1,1\}$. On pose $t=\tan\left(\frac{\theta}{2}\right)$. On a donc $\cos\theta=\frac{1-t^2}{1+t^2}$ et $d\theta=\frac{2dt}{1+t^2}$. On obtient
...
\end{center}}
  \item \reponse{\textbullet~$F$ est continue sur $[-1,1]$, dérivable sur $]-1,1[$ de dérivée nulle sur $]-1,1[$. Donc la fonction $F$ est constante sur l'intervalle $[-1,1]$. Par suite, pour tout réel $x\in[-1,1]$, $F(x)=F(0)=0$.
