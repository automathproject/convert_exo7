...
\exo7id{5917}
\titre{Exercice 5917}
\theme{Intégrale de Riemann, Propriétés de l'intégrale de Riemann}
\auteur{tumpach}
\date{2010/11/11}
\organisation{exo7}
\contenu{
  \texte{En utilisant la d\'efinition d'une fonction int\'egrable au sens
...
\end{enumerate}
\begin{enumerate}
  \item \reponse{Soit $\varepsilon>0$ donn\'e. Puisque $f$ est
...
g(x)\right)\,dx = \int_{a}^{b} f(x)\,dx + \int_{a}^{b} g(x)\,dx$.}
  \item \reponse{$\cdot$ Pour $\lambda = 0$ il n'y a rien a d\'emontrer. \\
 $\cdot$ Si $f$
...
En conclusion, $\lambda f$ est Riemann-int\'egrable et
$\int_{a}^{b} \lambda f(x)\,dx = \lambda \int_{a}^{b} f(x)\,dx$.}
  \item \reponse{Soient $f$ et $g$  deux fonctions Riemann-int\'egrables sur
...
c'est-\`a-dire $\int_{a}^{b} f(x)\,dx \leq \int_{a}^{b} g(x)\,dx$.}
  \item \reponse{Soit $\{f_{i}\}_{i\in\mathbb{N}}$ une suite de fonctions
