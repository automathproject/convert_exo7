...
\contenu{
  \texte{}
  \question{Soit $f$ une fonction définie sur un voisinage de $0$ telle que $\lim_{x\rightarrow 0}f(x)=0$ et $\lim_{x\rightarrow 0}\frac{f(2x)-f(x)}{x}=0$. Montrer que $\lim_{x\rightarrow 0}\frac{f(x)}{x}=0$. (Indication. Considérer $g(x)=\frac{f(2x)-f(x)}{x}$.)}
