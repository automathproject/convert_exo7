...
\organisation{exo7}
\contenu{
  \texte{Un industriel doit vérifier l'état de marche de ses machines et en
...
\end{enumerate}
\begin{enumerate}
  \item \reponse{30\% est la probabilité de l'événement Panne, noté $Pa$;
...
0.7=0.505$.}
  \item \reponse{La probabilité pour une machine hors d'usage de n'avoir jamais eu de
panne auparavant est 
$P(\text{non}\,Pa/HU)=P(HU/\text{non}\,Pa) P(\text{non}\,Pa)/P(HU)=0.4\cdot 0.7/
0.505\,=0.554\,46$.}
  \item \reponse{La loi de probabilité de $X$ est une loi binomiale, $n= 10$, $p=0.3$, espérance $3$.}
  \item \reponse{$P[X=5]=\binom{10}{5}(0.3)^{5}(0.7)^{5}=0.102\,92$}
\end{enumerate}
}