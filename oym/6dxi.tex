...
\exo7id{2605}
\titre{Exercice 2605}
\theme{Sujets de l'année 2007-2008, Partiel}
\auteur{delaunay}
\date{2009/05/19}
\organisation{exo7}
\contenu{
  \texte{Soit $u$ l'endomorphisme de $\R^3$ dont la matrice dans la base canonique est  
...
\end{enumerate}
\begin{enumerate}
  \item \reponse{{\it D\'eterminons et factorisons le polyn\^ome caract\'eristique de $A$.}
...
$$P_A(X)=-(X+2)[(-4-X)(3-X)+10]=-(X+2)(X^2+X-2)=-(X+2)^2(X-1).$$}
  \item \reponse{{\it D\'emontrons que les valeurs propres de $A$ sont $1$ et $-2$ et d\'eterminons les sous-espaces propres associ\'es}.
...
Le sous-espace propre $E_{-2}$ est donc le plan vectoriel d'équation $x+y+z=0$, dont une base est donnée, par exemple, par $\vec e_2=(1,-1,0)$ et $\vec e_3=(1,0,-1)$.}
  \item \reponse{{\it D\'emontrons que $A$ est diagonalisable et donnons une base de $\R^3$ dans laquelle la matrice de $u$ est diagonale.}
...
$$D=\begin{pmatrix}1&0&0 \\  0&-2&0 \\  0&0&-2\end{pmatrix}.$$}
  \item \reponse{{\it Trouvons une matrice $P$ telle que $P^{-1}AP$ soit diagonale.}
