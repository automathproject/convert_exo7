...
\end{enumerate}
\begin{enumerate}
  \item \reponse{Remarque: si $x\in F$, alors clairement $\bar{x}=x$ convient. \\
  ...
  \item \reponse{On voit que $\mathcal{M}_2 (\mathbb{R}) \simeq
  ...
  \item \reponse{La suite de matrices 
    $\displaystyle M_n = \begin{pmatrix} 1 & n \\ 0 & 1 \end{pmatrix}$ 
    est dans $\text{SL}_2(\mathbb{R})$ sans être bornée ($\| M_n \| = \sqrt{2+n^2}$).}
  \item \reponse{\begin{enumerate}}
  \item \reponse{C'est une conséquence directe du (1) en prenant pour $x$ la matrice nulle.}
  \item \reponse{Écrivons $f (M) = \sqrt{a^2 + b^2 + c^2 + d^2}$,
  ...
  \item \reponse{En un extremum on doit avoir $\nabla f = \lambda \nabla g$ (extrema liés) donc
