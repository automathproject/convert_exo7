...
\end{enumerate}
\begin{enumerate}
  \item \reponse{$\epsilon(x)=\displaystyle{\frac{ f (x)-f (c)}{ x-c}-f '(c)}.$
...
L'unicit\'e est \'evidente.}
  \item \reponse{Pour tout $  n \geq 1  ,   S_{n+1}-S_n=\displaystyle{\frac{1}{ 2n+2}}+
...
0  $) donc elle converge.}
  \item \reponse{Pour tout $  0\leq k\leq n  ,  \displaystyle{\frac{1}{ 2n}}\leq \displaystyle{\frac{1}{ n+k}}\leq
...
l'in\'egalit\' e $
 \displaystyle{\frac{1}{ 2}}\leq S \leq 1  .$}
  \item \reponse{Soit $  \epsilon   $ l'application de $  \rbrack -1, 1
...
s'annulle en $  0  $).}
  \item \reponse{Des \' egalit\' es $  \displaystyle { \hbox{log }\Bigl( 1+\frac{1}{n+k}
...
.$}
  \item \reponse{Par les deux questions qui pr\' ec\' edent il est imm\' ediat que $  \displaystyle{ \lim_{n\rightarrow \infty }\sigma _n =\hbox{log }(2)}  .$}
  \item \reponse{Soit $f  :  \rbrack -1, 1\lbrack \rightarrow   {\R}  $ une application continue, d\'erivable en $
