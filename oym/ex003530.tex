...
\organisation{exo7}
\contenu{
  \texte{On considère la matrice de $M_n(\C)$, $A=\begin{pmatrix}0      & a      & \dots  & a\cr
...
\end{enumerate}
\begin{enumerate}
  \item \reponse{$\det(M+(t))$ est une fonction affine de~$t$.}
  \item \reponse{$|\lambda+a| = k|\lambda+b|$ et $\lambda=x+iy  \Rightarrow 
