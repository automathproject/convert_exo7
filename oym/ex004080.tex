\exo7id{4080}
\titre{Ulm-Lyon-Cachan MP$^*$ 2000}
\theme{}
\auteur{quercia}
\date{2010/03/11}
\organisation{exo7}
\contenu{
  \texte{Soient $u,v,w$ trois applications bornées et de classe~$\mathcal{C}^1$ sur~$\R$,
...
\end{enumerate}
\begin{enumerate}
  \item \reponse{$u_n-u(t_n) =  \frac1{2\pi} \int_{t=t_n}^{t_n+2\pi} \int_{x=t_n}^t u'(x)\,d x\,d t$
et on majore l'intégrale interne par Cauchy-Schwarz.}
  \item \reponse{$w+w''=u'$ donc $w(t) =  \int_{x=t_n}^t\sin(t-x)u'(x)\,d x + \alpha\cos t + \beta\sin t$
puis 
