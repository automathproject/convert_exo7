\uuid{BFcU}
\exo7id{2231}
\titre{Rotation de Givens}
\theme{Factorisation QR. Transformations de Givens. Moindres carrés}
\auteur{matos}
\date{2008/04/23}
\organisation{exo7}
\contenu{
  \texte{Soient $p,q : $ $1\leq p < q\leq n$, $c,s\in\Rr :$ $c^2+s^2=1$.\\
...
\end{enumerate}
\begin{enumerate}
  \item \reponse{$G_{p,q}(c,s)= I +(c-1)e_pe_p^T + s e_qe_p^T -s e_qe_q^T + (c-1)e_pe_q^T$ avec $e_i$ les vecteurs de la base canonique.}
  \item \reponse{On montre que $e_i^TG^TGe_j=\delta_{ij} \quad \forall i,j=1, \cdots ,n$ et donc $G^TG=I$ ce qui permet de conclure que $G$ est inversible d'inverse $G^T$ et donc orthogonale.}
  \item \reponse{$e_i^TGA =e_i^TA= a_i^T$ pour $i\neq p,q$

$e_p^TGA= ca_p^T-sa_q^T$, $e_q^TGA= sa_p^T+ca_q^T$, et donc $G$ change seulement les lignes $p$ et $q$}
  \item \reponse{On pose $\alpha =a_{pj}$ et $\beta = a_{qj}$ . On a donc \`a r\'esoudre dans le premier cas  le syst\`eme
