...
\contenu{
  \texte{}
  \question{Pour $A=(a_{i,j})_{1\leq i,j\leq n}\in\mathcal{M}_n(\Rr)$, $N(A)=\mbox{Tr}(^tAA)$. Montrer que $N$ est une norme vérifiant de plus $N(AB)\leq N(A)N(B)$ pour toutes matrices carrées $A$ et $B$. $N$ est-elle associée à un produit scalaire~?}
  \reponse{Posons $\varphi~:~(A,B)\mapsto\mbox{Tr}({^t}AB)$. Montrons que $\varphi$ est un produit scalaire sur $\mathcal{M}_n(\Rr)$. 
