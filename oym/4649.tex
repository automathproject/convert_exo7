\uuid{4649}
\titre{X MP$^*$ 2001}
\theme{Exercices de Michel Quercia, Séries de Fourier}
\auteur{quercia}
\date{2010/03/14}
\organisation{exo7}
\contenu{
  \texte{}
  \question{Soit $a>0$ et $f$ continue sur $[0,a]$ à valeurs réelles. On suppose que pour tout $x\in\R$
on a $ \int_{t=0}^a f(t)\cos(xt)\,d t=0$. Montrer que $f$ est nulle.}
  \reponse{On pose $g(t) = f(a|t|/\pi)$ pour $t\in{[-\pi,\pi]}$, prolongée par $2\pi$-périodicité.
Alors $g$ est paire, continue, et tous ses coefficients de Fourier sont nuls donc $g=0$.}
}