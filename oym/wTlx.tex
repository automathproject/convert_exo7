...
\end{enumerate}
\begin{enumerate}
  \item \reponse{Soit $x\in[0,1[$. Pour tout réel $t$ de $[0,x]$, on a $\frac{1}{1-t}=\sum_{n=0}^{+\infty}t^n$. Maintenant, pour tout réel $t\in[0,x]$ et tout entier naturel $n$, on a $|t|^n\leqslant x^n$. Puisque la série numérique de terme général $x^n$ converge, on en déduit que la série de fonctions de terme général $t\mapsto t^n$ converge normalement et donc uniformément sur le segment $[0,x]$. D'après le théorème d'intégration terme à terme sur un segment, on peut affirmer que
...
\end{center}}
  \item \reponse{Par suite, pour $t\in]0,1[$,
