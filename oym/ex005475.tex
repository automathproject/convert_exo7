...
\organisation{exo7}
\contenu{
  \texte{Pour $n$ entier naturel, on pose $In=\int_{0}^{\pi/4}\tan^nx\;dx$.}
\begin{enumerate}
...
\end{enumerate}
\begin{enumerate}
  \item \reponse{$I_0=\int_{0}^{\pi/4}dx=\frac{\pi}{4}$ et $I_{1}=\int_{0}^{\pi/4}\frac{\sin x}{\cos x}\;dx=\left[-\ln|\cos x|\right]_{0}^{\pi/4}=\frac{\ln2}{2}$.
...
De même, $\sum_{k=1}^{n}\frac{(-1)^{k-1}}{2k}=I_1-(-1)^nI_{2n+1}$ et donc, $\forall n\in\Nn^*,\;I_{2n+1}=\frac{(-1)^n}{2}\left(\ln2-\sum_{k=1}^{n}\frac{(-1)^{k-1}}{k}\right)$.}
  \item \reponse{Soient $\varepsilon\in]0,\frac{\pi}{2}[$ et $n\in\Nn^*$.
