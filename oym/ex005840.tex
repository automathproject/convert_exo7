...
\end{enumerate}
\begin{enumerate}
  \item \reponse{Puisque $p>0$ et $q>0$, $1= \frac{1}{p}+ \frac{1}{q}> \frac{1}{p}$ et donc $p>1$. De même, $q>1$. D'autre part, $q= \frac{p}{p-1}$.

  \begin{enumerate}}
  \item \reponse{L'inégalité est immédiate quand $y=0$. Soit $y > 0$ fixé.
...
\end{center}}
  \item \reponse{Posons $A=\sum_{k=1}^{n}|a_k|^p$ et $B =\sum_{k=1}^{n}|b_k|^q$.
...
\end{center}}
  \item \reponse{Soit $((a_k)_{1\leqslant k\leqslant n},(b_k)_{1\leqslant k\leqslant n})\in(\Rr^n)^2$. D'après l'inégalité de \textsc{Hölder}, on a
