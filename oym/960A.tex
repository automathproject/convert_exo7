...
\organisation{exo7}
\contenu{
  \texte{Soit $P(X)$ un polyn\^ome de $\C[X]$, soit $A$ une matrice de $M_n(\C)$. On note $B$ la matrice : $B=P(A)\in M_n(\C)$.}
\begin{enumerate}
...
\end{enumerate}
\begin{enumerate}
  \item \reponse{{\it On d\'emontre que si $\vec x$ est un vecteur propre de $A$ de valeur propre $\lambda$, alors $\vec x$ est un vecteur propre de $B$ de valeur propre $P(\lambda)$.}
...
ce qui prouve que $\vec x$ est un vecteur propre de la matrice $B=P(A)$ pour la valeur propre $P(\lambda)$.}
  \item \reponse{Le but de cette question est de d\'emontrer que les valeurs propres de $B$ sont toutes de la forme $P(\lambda)$, avec $\lambda$ valeur propre de $A$.

 Soit $\mu\in\C$, on d\'ecompose le polyn\^ome $P(X)-\mu$ en produit de facteurs de degr\'e $1$ :
$$P(X)-\mu=a(X-\alpha_1)\cdots(X-\alpha_r).$$
   \begin{enumerate}}
  \item \reponse{{\it On d\'emontre que} 
$\det(B-\mu I_n)=a^n\det(A-\alpha_1I_n)\cdots\det(A-\alpha_rI_n).$
...
car le d\'eterminant est une forme multilin\'eaire (d'o\`u le $a^n$) et le d\'eterminant d'un produit de matrices est \'egal au produit de leurs d\'eterminants.}
  \item \reponse{{\it On en d\'eduit que si $\mu$ est valeur propre de $B$, alors il existe une valeur propre $\lambda$ de $A$ telle que $\mu=P(\lambda)$.}
