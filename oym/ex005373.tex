\exo7id{5373}
\titre{***I Dérivée d'un déterminant}
\theme{}
\auteur{rouget}
\date{2010/07/06}
\organisation{exo7}
\contenu{
  \texte{Soient $a_{i,j}$ ($(i,j)$ élément de $\{1,...,n\}^2$) $n^2$ fonctions de $\Rr$ dans $\Rr$, dérivables sur $\Rr$ et $A=(ai,j)_{1\leq i,j\leq n}$. Calculer la dérivée de la fonction $x\mapsto\mbox{det}(A(x))$.
...
\end{enumerate}
\begin{enumerate}
  \item \reponse{Soit $\Delta_n(x)=\left|\begin{array}{ccccc}
x+1&1&\ldots& &1\\
1&x+1&\ddots& &\vdots\\
\vdots&\ddots&\ddots&\ddots&\vdots\\
\vdots& &\ddots&\ddots&1\\
