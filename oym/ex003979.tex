\exo7id{3979}
\titre{$f' = f^{-1}$ (Ens Cachan MP$^*$ 2003)}
\theme{}
\auteur{quercia}
\date{2010/03/11}
\organisation{exo7}
\contenu{
  \texte{On note~$E$ l'ensemble des fonctions~$f$ de classe~$\mathcal{C}^1$ bijectives
...
\end{enumerate}
\begin{enumerate}
  \item \reponse{$m=\frac{1+\sqrt5}2$, $c=m^{-1/m}$.}
  \item \reponse{$f$ et $f'$ ont des limites nulles en $0^+$ et infinies en~$+\infty$
  ...
  \item \reponse{On note~$a$ le point fixe de~$f$, $b$ celui de~$g$ et on suppose
