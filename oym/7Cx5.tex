\uuid{7Cx5}
\exo7id{5412}
\titre{***I Polynômes de \textsc{Legendre}}
\theme{Fonctions réelles d'une variable réelle dérivables (exclu études de fonctions)}
\auteur{rouget}
\date{2010/07/06}
\organisation{exo7}
\contenu{
  \texte{Pour $n$ entier naturel non nul donné, on pose $L_n=((X^2-1)^n)^{(n)}$.}
\begin{enumerate}
...
\end{enumerate}
\begin{enumerate}
  \item \reponse{$(X^2-1)^n$ est de degré $2n$ et donc, $L_n$ est de degré $2n-n=n$. Puis, $\mbox{dom}(L_n)=\mbox{dom}((X^{2n})^{(n)})=\frac{(2n)!}{n!}$.}
  \item \reponse{$1$ et $-1$ sont racines d'ordre $n$ de $A_n$ et donc racines d'ordre $n-k$ de $A_n^{(k)}$, pour tout $k$ élément de $\{0,...,n\}$.
