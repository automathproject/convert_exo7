...
\contenu{
  \texte{}
  \question{Soit $G$ un sous-groupe de $GL(E)$ avec $\text{dim}E=n$ et $\text{card}G=p$.
Soit $F=\{x\in E/\;\forall g\in G,\;g(x)=x\}$.

Montrer que $\text{dim}F= \frac{1}{p}\sum_{g\in G}^{}\text{Tr}g$.}
  \reponse{Par la même méthode qu'au \ref{ex:rou32}, on voit que $f=\frac{1}{p}\sum_{g\in G}^{}g$ est un projecteur et donc $\frac{1}{p}\sum_{g\in G}^{}\text{Tr}g=\text{rg}f$. Maintenant, si $x$ est un élément de $F$ alors pour tout $g$ dans $G$, $g(x)=x$ et donc $f(x) = x$. Ainsi, un élément $x$ de $F$ est dans $\text{Im}f$. 
