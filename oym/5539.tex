...
\contenu{
  \texte{}
  \question{Pour $\lambda\in\Rr$, on note $(\Gamma_\lambda)$ la courbe d'équation $y=\lambda xe^{-x}$. Quel est le lieu des centres de courbure $C_\lambda$ en $O$ à $(\Gamma_\lambda)$ quand $\lambda$ décrit $\Rr$.}
  \reponse{Soit $\lambda\in\Rr$. $\mathcal{C}_\lambda$ est le support de l'arc paramétré $t\mapsto\left(
\begin{array}{c}
t\\
\lambda te^{-t}
\end{array}
