...
\organisation{exo7}
\contenu{
  \texte{Soient $f : {\R^2} \to \R, {(t,x)} \mapsto {f(t,x)}$ de classe $\mathcal{C}^1$,
...
\end{enumerate}
\begin{enumerate}
  \item \reponse{Qu'il en existe et qu'il y en a une unique maximale, son intervalle de définition est ouvert.}
  \item \reponse{Soit $(]\alpha,\beta[,x)$ une solution maximale. Si $t_0\in{]\alpha,\beta[}$ est tel que $x(t_0)=a$
  ...
  \item \reponse{Soit $\varphi : {[a,b]} \to {[a,b]},y \mapsto {x(T).}$ Comme deux courbes intégrales
