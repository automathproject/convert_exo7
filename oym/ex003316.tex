\exo7id{3316}
\titre{Commutants itérés}
\theme{}
\auteur{quercia}
\date{2010/03/09}
\organisation{exo7}
\contenu{
  \texte{Soit $u\in\mathcal{L}(E)$. On pose pour $v\in\mathcal{L}(E)$~: $\varphi(v) = v\circ u - u\circ v$,
...
\end{enumerate}
\begin{enumerate}
  \item \reponse{$\varphi(v\circ w) = \varphi(v)\circ w + v\circ \varphi(w)$.}
  \item \reponse{Par récurrence $\varphi^n(v\circ w) = \sum_{k=0}^n C_n^k\varphi^k(v)\circ\varphi^{n-k}(w)$
