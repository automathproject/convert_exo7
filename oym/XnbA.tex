\uuid{XnbA}
\exo7id{7820}
\titre{Produit de formes linéaires}
\theme{Exercices de Christophe Mourougane, 328.00 - Formes bilinéaires}
\auteur{mourougane}
\date{2021/08/11}
\organisation{exo7}
\contenu{
  \texte{}
\begin{enumerate}
  \item \question{Montrer que le polyn{ô}me quadratique à coefficients complexes avec $a\not =0$
$$ax^2+a'y^2+a''z^2+2byz+2b'zx+2b''xy$$ est le produit
de deux polyn{ô}mes linéaires si et seulement si
$$aa'-{b''}^2=aa''-{b'}^2=ab-b'b''=0.$$}
  \item \question{L'analogue à coefficients réels est-il vrai ?}
\end{enumerate}
\begin{enumerate}

\end{enumerate}
}