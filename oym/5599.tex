...
\contenu{
  \texte{}
  \question{Soient $E$ un $\Cc$-espace vectoriel non nul de dimension finie $n$ et $f$ un endomorphisme de $E$ tel que $\forall x\in E$, $\exists p\in\Nn^*$ tel que $f^p(x)=0$. Montrer que $f$ est nilpotent.}
  \reponse{(Ne pas confondre : ($\forall x\in E,\; \exists p\in\Nn^*/\;f^p(x)=0$) et ($\exists p\in\Nn^*/\;\forall x\in E,\;f^p(x) = 0)$. Dans le deuxième cas, $p$ est indépendant de $x$ alors que dans le premier cas, $p$ peut varier quand $x$ varie).
