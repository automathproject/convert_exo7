...
\organisation{exo7}
\contenu{
  \texte{Soit $C$ une courbe régulière plane fermée simple convexe paramétrée 
...
\end{enumerate}
\begin{enumerate}
  \item \reponse{La fonction courbure est continue et non constante sur le compact $[0,\ell]$.
 ...
 Ce sont donc des coins.}
  \item \reponse{On utilise la formule de Frenet $\dot{n}(t)=-\kappa(t)\dot{c}(t)$.
 \begin{eqnarray*}
  \int_0^\ell \dot{\kappa}c(t)&=-&\int_0^\ell \kappa\dot{c}(t)=\int_0^\ell \dot{n}(t)
 =n(\ell)-n(0)=0.
 \end{eqnarray*}}
  \item \reponse{Par hypothèse, l'ordonnée de $\dot{\kappa}(t)c(t)$ serait de signe constant sur les deux
 parties de la courbe délimitées par $P$ et $Q$. Ceci contredit l'annulation de l'intégrale.}
  \item \reponse{S'il n'y a que trois coins, la courbe est partagée en trois parties.
