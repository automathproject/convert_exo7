\uuid{7631}
\titre{Applications sur le disque}
\theme{}
\auteur{mourougane}
\date{2021/08/10}
\organisation{exo7}
\contenu{
  \texte{Soit $f$ une fonction holomorphe sur le disque unité $\Delta$. 
...
\end{enumerate}
\begin{enumerate}
  \item \reponse{Cette application est continue en $i$ : elle ne tend donc pas vers $+\infty$ en module quand $z$ tend vers $i$.
  Elle ne tend donc pas vers $+\infty$ en module quand $|z|$ tend vers $1$.}
  \item \reponse{Par le principe du maximum appliqué à la fonction holomorphe $\Delta\to\Cc$, $z\mapsto 1/f(z)$,
 ...
 Il n'existe donc pas de telle suite. Donc, dans le cas où $f$ n'a pas de zéro, $|f(z)|$ ne tend pas vers $+\infty$ quand $|z|$ tend vers $1$.}
  \item \reponse{Comme $|f(z)|$ tend vers $+\infty$ quand $|z|$ tend vers $1$, il existe $r\in ]0,1[$ tel que pour tout $z\in\Delta$ avec $|z|>r$, $|f(z)|>1$. L'application $f$ ne s'annule donc pas sur $\Delta-\overline{\Delta_r}$.
