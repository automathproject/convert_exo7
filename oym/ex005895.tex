...
\contenu{
  \texte{}
  \question{Soit $\begin{array}[t]{cccc}
...
\end{array}$. Montrer que $f$ est différentiable en tout point de $M_n(\Rr)\setminus\{0\}$ et déterminer sa différentielle.}
  \reponse{On munit $\mathcal{M}_n(\Rr)$ d'une norme sous-multiplicative $\|\;\|$. Soit $A\in GL_n(\Rr)$. On sait que $GL_n(\Rr)$ est un ouvert de $\mathcal{M}_n(\Rr)$ et donc pour $H\in\mathcal{M}_n(\Rr)$ de norme suffisamment petite, $A+H\in GL_n(\Rr)$. Pour un tel $H$
