...
\organisation{exo7}
\contenu{
  \texte{Notons $T=\{0\}\times [-1,1]\cup [-1,1]\times \{0\}$ muni de la
...
\end{enumerate}
\begin{enumerate}
  \item \reponse{$T$ est compact car c'est un fermé borné de $\Rr^2$.
...
$T$ est connexe donc $f(T)$ est connexe. Donc $f(T)$ est un compact connexe de $\Rr$ c'est donc un segment compact.}
  \item \reponse{Ce sont les quatre points cardinaux $N=(0,1)$, $S=(0,-1)$, $E=(1,0)$, $W=(-1,0)$.}
  \item \reponse{Par l'absurde, supposons que $T$ soit homéomorphe à une partie $I$ de $\Rr$, alors
