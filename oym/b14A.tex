...
\end{enumerate}
\begin{enumerate}
  \item \reponse{Le polyn\^ome caract\'eristique associ\'e \`a $E$ est : $p(x) = x^2+2x+4$ ; son discriminant
...
$$ y(x) = e^{-x}(a\cos{\sqrt3 x} + b\sin{\sqrt3 x})$$ obtenues lorsque $a,b$ d\'ecrivent $\R$.}
  \item \reponse{Le second membre est de la forme $ e^{\lambda x}Q(x) $ avec $\lambda = 1$ et $Q(x)=x$. On
...
$$ y(x)= e^{-x}(a\cos{\sqrt3 x} + b\sin{\sqrt3 x}) +\frac{1}{7}(x-\frac{4}{7})e^x \; ;\; a,b \in \R.$$}
  \item \reponse{Soit $h$ une solution de $E$. Les conditions $h(0)=1$, $h(1)=0$ sont r\'ealis\'ees ssi
$$a=\frac{53}{49}\qquad \hbox{et} \qquad b=-\frac{53\cos\sqrt3+3e^2}{49\sin\sqrt3}.$$}
  \item \reponse{\begin{enumerate}}
  \item \reponse{On a : $g^\prime(x) =e^xf^\prime(e^x)$ et
...
donc $g$ est solution de $E$.}
  \item \reponse{R\'eciproquement pour $f(t)= g(\log t)$ o\`u $g$ est une
