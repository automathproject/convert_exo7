...
\end{enumerate}
\begin{enumerate}
  \item \reponse{Les composantes de l'application $c$ sont de classe $\mathcal{C}^\infty$.
 La dérivée de la dernière composante n'est jamais nulle.
 La courbe $c$ est donc régulière.}
  \item \reponse{Pour tout paramètre $t$, $\|\dot{c}(t)\|=\sqrt{2+t^2}\geq \sqrt{2}$.
 Donc,
 $$L[c_{\mid [a,b]}]=\int_a^b \|\dot{c}(t)\| dt\geq\sqrt{2}(b-a).$$}
  \item \reponse{Soit $t\in\Rr $,
 $$(t\cos t)^2+(t\sin t)^2=t^2=(t)^2.$$
 Par conséquent, $\mathcal{C}$ est dans la quadrique d'équation $x^2+y^2=z^2$.}
  \item \reponse{Soit $t\in\Rr $, $c(-t)= \left(\begin{array}{c}-t\cos t \\ t\sin t\\- t\end{array}\right)$ et se déduit donc de $c(t)$
 par le demi-tour autour de l'axe des ordonnées (d'équation $x=z=0$).}
  \item \reponse{$$ \includegraphics[scale=.4]{images/img-mour-449}$$
 %faire du gif avec wims2
 %convert wims.gif wims.png}
  \item \reponse{$$c(t)=\left(\begin{array}{c}t\cos t \\ t\sin t\\ t\end{array}\right) ; 
