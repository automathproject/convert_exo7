...
\organisation{exo7}
\contenu{
  \texte{Dans un espace euclidien $E$, on considère un vecteur unitaire $a$, et
...
\end{enumerate}
\begin{enumerate}
  \item \reponse{$<u_{k}(x),a>=k<x,a><a,a>+<x,a>=(k+1)<x,a>$ donc
    $x=\frac{-k}{k+1}<u_{k}(x),a>a+u_{k}(x)$. On en déduit que $u_{k}$
    est inversible, et que $u_{k}^{-1}=u_{\frac{-k}{k+1}}$.}
  \item \reponse{L'adjoint d'un endomorphisme $u$ est l'unique endomorphisme $v$ qui
    satisfait~: $\forall(x,y)\in E^{2}, <u(x),y>=<x,u(y)>$. Or
    $<u_{k}(x),y>=k<x,a><y,a>+<x,y>=<x,u_{k}(y)>$. Donc $u_{k}$ est égal
    à son adjoint.}
  \item \reponse{Si $u_{k}$ est orthogonal, on doit avoir $\Vert u_{k}(a)\Vert=\Vert
  ...
  \item \reponse{Si $k=0$, 1 est la seule valeur propre et $E_{1}=E$
