...
\organisation{exo7}
\contenu{
  \texte{Soit $\R[X]$ l'ensemble de polynômes dans $\R$ et soit $\R_{n}[X]$ l'ensemble des polynômes de degré au plus $n$.
...
\end{enumerate}
\begin{enumerate}
  \item \reponse{Soit $\lambda,\,\mu\in\R$ et $P,\, Q\in \R[X]$. Comme la dérivation est une application linéaire on a bien :
\begin{equation*}
...
donc $\Phi(X^{j})\in\R_{2}[X]$ pour $j\in\{0,1,2\}$. C'est bien un endomorphisme.}
  \item \reponse{En appliquant la question précédente, on a bien :
\begin{equation*}
[\Phi]_{\mathcal{B}}^{\mathcal{B}}=\left[[\Phi(1)]_{\mathcal{B}}\,|\,[\Phi(X)]_{\mathcal{B}}\,|\,[\Phi(X^2)]_{\mathcal{B}}\right]=A
\end{equation*}}
  \item \reponse{$rg(A)=rg(\Phi)$. On applique une méthode de Gauss à la matrice $A$:
\begin{equation*}
...
\end{pmatrix}=3
\end{equation*}}
  \item \reponse{Comme $rg\,A=3=dim\R_{2}[X]$, l'application $\Phi$ est surjective. La matrice $A$ est carrée donc en appliquant le théorème du rang, elle est aussi injective donc bijective.}
  \item \reponse{Le noyau est donnée par :
...
Ainsi $Ker\, (A-I_{3})=\left\{(x,0,-x),\, x\in\R\right\}=vect\left((1,0,-1)\right)$. $(-1,0,1)$ est une base de $Ker\, (A-I_{3})$. Une base de $Ker\,(\Phi-I_{d})$ est donc : $X^2-1$.}
  \item \reponse{Par le théorème du rang : $dim\ Im(\Phi-Id)=dim \R_{2}[X]-dim\ Ker\,(\Phi-Id)=3-1=2$. Il reste à déterminer une base de l'image. 
\begin{align*}
...
La famille $((1,0,1),\, (0,1,0))$ est une famille génératrice de l'image qui est de dimension $2$. C'est donc une base. On en déduit qu'une base de $Ker\,(\Phi-Id)$ est $((X^2+1),X)$.}
  \item \reponse{On a l'égalité ensembliste :
\begin{align*}
