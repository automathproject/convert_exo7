...
\contenu{
  \texte{}
  \question{Soit $A=(a_{i,j})_{1\leq i,j\leq n+1}$ définie par $a_{i,j}=0$ si $i>j$ et $a_{i,j}=C_{j-1}^{i-1}$ si $i\leq j$.
...
Montrer que $A$ est inversible et déterminer son inverse. (Indication~:~considérer l'endomorphisme de $\Rr_n[X]$ qui à un polynôme $P$ associe le polynôme $P(X+1)$).}
  \reponse{Soit $f$ l'endomorphisme de $\Rr_n[X]$ qui, à un polynôme $P$ de degré inférieur ou égal à $n$, associe le polynôme $P(X+1)$.
