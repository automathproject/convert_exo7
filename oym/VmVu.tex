\uuid{VmVu}
\exo7id{4573}
\titre{Développer peut être dangereux}
\theme{Exercices de Michel Quercia, Séries entières}
\auteur{quercia}
\date{2010/03/14}
\organisation{exo7}
\contenu{
  \texte{Pour $n\in\N$ et $x\in\R$ on pose $u_n(x) = \Bigl(\frac{x(1-x)}2\Bigr)^{4^n}$.}
\begin{enumerate}
...
\begin{enumerate}
  \item \reponse{$]-1,2[$.}
  \item \reponse{Pour $0\le k\le 4^n$, on a $|a_k| \le C_{4^n}^{4^n/2}\Bigm/2^{4^n}$
