...
\organisation{exo7}
\contenu{
  \texte{Soit $f : [a,b] \to \R$ une fonction continue. On veut d\'emontrer
que
...
\end{enumerate}
\begin{enumerate}
  \item \reponse{Pour tout $x \in ]a,b[$, on a $x \in [a,b]$ donc $f(x) \leq \sup_{a\leq
  ...
  \item \reponse{$f$ est continue sur un intervalle ferm\'e et born\'e, donc elle est born\'ee
  et elle atteint ses bornes. Soit $x_0$ le r\'eel o\`u le maximum est atteint : 
$f(x_0)=\sup_{a\leq x \leq b} f(x)$.
  \begin{itemize}}
  \item \reponse{si $x_0=a$, consid\'erons la suite $a_n=a+1/n$. Pour $n\geq
  ...
  \item \reponse{si $x_0=b$ on obtient le r\'esultat de mani\`ere identique en consid\'erant
    la suite $b_n=b-1/n$.}
  \item \reponse{si $a<x_0<b$ : $f(x_0)$ est major\'e par le sup de $f$ sur $]a,b[$, donc
$$f(x_0) \leq \sup_{a<x<b} f(x) \leq  \sup_{a\leq x \leq b} f(x)=f(x_0)$$
donc $f(x_0) = \sup_{a<x<b} f(x)$.
  \end{itemize}}
  \item \reponse{Avec la fonction $g$, on a $\sup_{0<x<1} g(x)=0$ car pour chaque $x \in
