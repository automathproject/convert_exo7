\uuid{5867}
\titre{** I}
\theme{Suites et séries de matrices}
\auteur{rouget}
\date{2010/10/16}
\organisation{exo7}
\contenu{
  \texte{}
  \question{On munit $\mathcal{M}_p(\Cc)$ d'une norme sous-multiplicative notée $\|\;\|$. Soit $A$ un élément de $\mathcal{M}_p(\Rr)$ tel que $\|A\|<1$. Montrer que la série de terme général $A^n$, $n\in\Nn$, converge puis que $\sum_{n=0}^{+\infty}A^n=(I-A)^{-1}$.

En déduire que $\|(I-A)^{-1}-(I+A)\|\leqslant \frac{\|A\|^2}{1-\|A\|}$.}
  \reponse{Soit $A\in\mathcal{M}_p(\Cc)$ telle que $\|A\|<1$. Pour tout entier naturel $n$, on a $\|A^n\|\leqslant\|A\|^n$. Puisque $\|A\|<1$, la série numérique de terme général $\|A\|^n$, $n\in\Nn$, converge. Il en est de même de la série de terme général $\|A^n\|$ et donc la série de terme général $A^n$, $n\in\Nn$, converge absolument. 
