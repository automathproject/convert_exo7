...
\organisation{exo7}
\contenu{
  \texte{Soit $f$ l'application de $\Rr_n[X]$  dans  $\Rr[X]$
...
\end{enumerate}
\begin{enumerate}
  \item \reponse{Il est facile de voir que $f(\lambda P + \mu Q) = \lambda f(P)+\mu f(Q)$ donc $f$ est linéaire,
de plus, $P$ étant un polynôme de degré $\le n$ alors $f(P)$ aussi.}
  \item \reponse{Pour $n=3$ on calcule l'image de chacun des éléments de la base :
...
jusqu'à l'élément diagonal (qui est nul).}
  \item \reponse{Nous savons que $f(1)=0$ et $f(X)=0$ donc $1$ et $X$ sont dans le noyau $\Ker f$.
...
$\Ker f =  \textrm{Vect}\{1,X\}$.}
  ...
  \item \reponse{Soit $Q \in \Im f$. Il existe donc $R\in\Rr_n[X]$ tel que $f(R)=Q$.
...
Donc notre polynôme $P$ convient.}
  \item \reponse{Montrons l'unicité. Soient $P$ et $\tilde P$ tels que $f(P)=f(\tilde P)=Q$
