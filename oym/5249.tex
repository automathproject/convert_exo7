...
\contenu{
  \texte{}
  \question{Calculer $\mbox{inf}_{\alpha\in]0,\pi[}(\mbox{sup}_{n\in\Nn}(|\sin(n\alpha)|))$.}
  \reponse{Pour $\alpha\in]0,\pi[$, posons $f(\alpha)=\mbox{sup}_{n\in\Nn}(|\sin(n\alpha)|)$. $\{(\sin(n\alpha),\;n\in\Nn\}$ est une partie non vide et majorée (par $1$) de $\Rr$. Donc, pour tout réel $\alpha$ de $]0,\pi[$, $f(\alpha)$ existe dans $\Rr$.
