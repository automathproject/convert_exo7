...
\contenu{
  \texte{}
  \question{Pour $x>0$, on pose $f(x) =\sum_{n=0}^{+\infty}e^{-x\sqrt{n}}$. Trouver un équivalent simple de $f$ en $0$ à droite.}
  \reponse{Soit $x>0$. Pour $n\in\Nn^*$, $n^2e^{-x\sqrt{n}}=e^{-x\sqrt{n}+2\ln n}\underset{n\rightarrow+\infty}{=}o(1)$ d'après un théorème de croissances comparées. On en déduit que $e^{-x\sqrt{n}}\underset{n\rightarrow+\infty}{=}o\left(\frac{1}{n^2}\right)$ et donc que la série de terme général $e^{-x\sqrt{n}}$ converge. Ainsi, $f$ est bien définie sur $]0,+\infty[$.
