...
\organisation{exo7}
\contenu{
  \texte{Soit $E = \R_n[X]$ et $(P\mid Q) =  \int_{t=0}^1 P(t)Q(t)\,d t$.}
\begin{enumerate}
...
\end{enumerate}
\begin{enumerate}
  \item \reponse{$ \int_{t=0}^1 t^kt^x\,d t = \frac 1{k+x+1}$.}
  \item \reponse{$\Phi$ a pour pôles au plus simples $-1,-2,\dots,-n-1$ et
             pour racines $0,1,\dots,n-1$. Comme $\Phi(x) \to 0$ lorsque $x\to\infty$,
             on a donc $\Phi(x) = \lambda\frac{x(x-1)\dots(x-n+1)}{(x+1)\dots(x+n+1)}$.}
  \item \reponse{$a_k =$ résidu de $\Phi$ en $-k-1 =
