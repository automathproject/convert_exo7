\exo7id{4802}
\titre{Fonction presque additive, Centrale MP 2001}
\theme{}
\auteur{quercia}
\date{2010/03/16}
\organisation{exo7}
\contenu{
  \texte{Soient $E,F$ deux $\R$-espaces vectoriels norm{\'e}s, $F$ {\'e}tant complet.
...
\end{enumerate}
\begin{enumerate}
  \item \reponse{$f(rx) = rf(x)$ pour tout $r\in\N$ par r{\'e}currence, puis pour
    ...
    dans $\C$.}
  \item \reponse{$\|f_{n+1}(x) - f_n(x)\|\le M2^{-n-1}$ donc la s{\'e}rie t{\'e}lescopique
    $\sum(f_{n+1}(x) - f_n(x))$ est uniform{\'e}ment convergente.}
  \item \reponse{$\|f_n(x+y) - f_n(x) - f_n(y)\|\le M2^{-n}$ donc
