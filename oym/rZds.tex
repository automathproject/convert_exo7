...
\exo7id{6989}
\titre{Exercice 6989}
\theme{Courbes planes, Courbes en polaires}
\auteur{blanc-centi}
\date{2015/07/04}
\organisation{exo7}
\contenu{
  \texte{\'Etudier les courbes d'équations polaires suivantes:}
\begin{enumerate}
...
\end{enumerate}
\begin{enumerate}
  \item \reponse{L'expression $r(\theta)=\frac{1}{\sqrt{\tan(2\theta)}}$ est bien définie 
sur $\mathcal{D}= ]0,\frac\pi4[$ (il faut $\tan(2\theta)$ 
bien défini et strictement positif).
\begin{itemize}}
  \item \reponse{Passages par l'origine\\
Puisque $r$ ne s'annule pas, la courbe ne passe pas par l'origine. Mais elle admet l'origine pour point limite: $r(\theta)\xrightarrow[\theta\to\frac{\pi}{4}^-]{}0^+$.}
  \item \reponse{Variations et signe de la fonction $r$\\
...
Cela signifie que la courbe tourne (dans le sens trigonométrique) en se rapprochant de l'origine.}
  \item \reponse{Tangente à l'origine\\
...
\end{tikzpicture}  
\end{center}}
  \item \reponse{L'expression $r(\theta)=\frac{\sin^2\theta}{\cos\theta}$ est bien définie sur $\mathcal{D}=]-\frac\pi2,\frac\pi2[$.
\begin{itemize}}
...
\end{tikzpicture}  
\end{center}}
  \item \reponse{L'expression $r(\theta)=\sqrt{\cos(2\theta)}$ est bien définie si $\cos(2\theta)$ est positif, \textsl{i.e.} sur $\mathcal{D}=\bigcup_{k\in\Z}[-\frac{\pi}{4}+k\pi;\frac{\pi}{4}+k\pi]$.
\begin{itemize}}
  \item \reponse{Réduction du domaine d'étude\\
