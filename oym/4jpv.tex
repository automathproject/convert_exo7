...
\organisation{exo7}
\contenu{
  \texte{Une application de $X$ dans $Y$ est dite {\it ouverte} si l'image de tout ouvert
...
\end{enumerate}
\begin{enumerate}
  \item \reponse{Soit $P$ un polynôme, et $F$ un fermé de $\Rr$.  Soit $(y_n)$
...
Donc $P(F)$ est fermé.}
  \item \reponse{Soit $X=Y=\Rr$ et  $H=(xy=1)$ est un fermé de $X\times Y$, mais
si $\pi(x,y)=x$ alors $\pi(H)=\Rr^*$ n'est pas un fermé de $X=\Rr$.}
  \item \reponse{A vérifier...}
\end{enumerate}
}