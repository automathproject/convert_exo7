...
\exo7id{2602}
\titre{Exercice 2602}
\theme{Sujets de l'année 2006-2007, Rattrapage}
\auteur{delaunay}
\date{2009/05/19}
\organisation{exo7}
\contenu{
...
\end{enumerate}
\begin{enumerate}
  \item \reponse{{\it Factorisons le polyn\^ome caract\'eristique de $A$.} (1 point)
...
Les valeurs propres de la matrice $A$ sont $\lambda_1=-1$, valeur propre simple et $\lambda_2=2$, valeur propre double.}
  \item \reponse{{\it D\'eterminons les sous-espaces propres et caract\'eristiques de $A$.} (2 points)
...
D'o\`u $\ker(A-2I)^2=\{(x,y,z)\R^3,\ x+2z=0\}$, c'est le plan vectoriel d'\'equation $x+2z=0$.}
  \item \reponse{{\it D\'emontrons qu'il existe une base de $\R^3$ dans laquelle la matrice de $f$ est} ( 1 point)
$$B=\begin{pmatrix}-1 &0& 0 \\  0&2&1 \\ 0&0&2\end{pmatrix}.$$
...
P^{-1}=\begin{pmatrix}-1 &0&-2 \\  1&0&1 \\ -1&1&-1\end{pmatrix}.$$}
  \item \reponse{{\it Ecrivons la d\'ecomposition de Dunford de $B$.} (1 point)
...
la matrice $D$ est diagonale, la matrice $N$ est nilpotente, $N^2=0$, et $ND=DN$, c'est donc bien la d\'ecomposition de Dunford de la matrice $B$.}
  \item \reponse{{\it Calculons $\exp B$}. (1 point)
