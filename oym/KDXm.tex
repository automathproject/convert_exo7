\uuid{KDXm}
\exo7id{2234}
\titre{M\'ethode de Givens Rapide}
\theme{Factorisation QR. Transformations de Givens. Moindres carrés}
\auteur{matos}
\date{2008/04/23}
\organisation{exo7}
\contenu{
  \texte{Soit $A\in\Rr^{m\times n}$. On veut construire une matrice $M\in \Rr^{m\times m}$ telle que
\begin{itemize}
...
\end{enumerate}
\begin{enumerate}
  \item \reponse{$MM^T=$diag$(d_1, \cdots , d_m)=\Delta^2$ avec $\Delta=$diag$ (\sqrt{d_1}, \cdots, \sqrt{d_m})$
...
Comme $\Delta^{-1}S$ est triangulaire sup\'erieure ona $ A=QR$ avec $Q=M^T\Delta^{-1}, R=\Delta^{-1}S$}
  ...
  \item \reponse{$$M_1x=\left(\begin{array}{c}\beta_1x_1 +x_2\\x_1+\alpha_1x_2\end{array}\right)  ,\quad M_1DM_1^T=\left(\begin{array}{cc}d_2+\beta_1^2 d_1 & d_1\beta_1 +d_2\alpha_1\\ d_1\beta_1 +d_2\alpha_1 & d_1+\alpha_1^2d_2\end{array}\right)$$
Alors
\begin{itemize}}
  \item \reponse{$x_1+\alpha_1x_2=0\Leftrightarrow \alpha_1=-x_1/x_2$}
  \item \reponse{$d_1\beta_1 +d_2 (-x_1/x_2)=0\Leftrightarrow \beta_1=-\alpha_1 d_2/d_1=x_1d_2/(x_2d_1)$
\end{itemize}
...
$d_1+\alpha_1^2d_2 =d_1(1+\alpha_1^2d_2/d_1) = d_2(1+\gamma_1)$}
  \item \reponse{le m\^eme type de calcul nous donne
$$\beta_2=-x_2/x_1, \quad \alpha_2=-(d_1/d_2)\beta_2,\quad \gamma_2=-\alpha_2\beta_2 = (d_1/d_2)(x_2/x_1)^2$$}
  \item \reponse{on remarque que $\gamma_1 \gamma_2=1$ et donc soit $\gamma_1\leq 1$ , soit $\gamma_2\leq 1$}
\end{enumerate}
}