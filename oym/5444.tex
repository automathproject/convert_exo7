...
\contenu{
  \texte{}
  \question{Soient $f$ et $g$ deux fonctions continues et strictement positives sur $[a,b]$. Pour $n$ entier naturel non nul donné, on pose $u_n=\left(\int_{a}^{b}(f(x))^ng(x)\;dx\right)^{1/n}$.

Montrer que la suite $(u_n)$ converge et déterminer sa limite (commencer par le cas $g=1$).}
  \reponse{$f$ est continue sur le segment $[a,b]$ et admet donc un maximum $M$ sur ce segment. Puisque $f$ est strictement positive sur $[a,b]$, ce maximum est strictement positif.
