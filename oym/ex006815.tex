\uuid{6815}
\titre{Exercice 6815}
\theme{}
\auteur{gijs}
\date{2011/10/16}
\organisation{exo7}
\contenu{
  \texte{Soit $X$ un espace topologique, $A\subset X$ une partie
et $C\subset X$ une partie connexe telle que $C \cap A
\neq \emptyset$ et $C \cap (X\setminus A) \neq \emptyset$.}
\begin{enumerate}
  \item \question{Démontrer que $C$ contient des points de la
frontière de $A$.}
  \item \question{Pourquoi $A = \{(x,y,0) \mid x^2 + y^2 \le 1\}
\subset \Rr^3$ et $C = \{(0,0,z) \mid |z|\le1\}$
n'est-il pas un contre-exemple pour la propriété
énoncée dans 1. ?}
\end{enumerate}
\begin{enumerate}

\end{enumerate}
}