...
\contenu{
  \texte{}
  \question{Soient $F$ et $G$ deux sous-espaces vectoriels d'un espace vectoriel de dimension finie sur $\Kk$.

Démontrer que $\text{dim}(F+G)=\text{dim}F+\text{dim}G-\text{dim}(F\cap G)$.}
  \reponse{Soit $f$ l'application de $F\times G$ dans $E$ qui à un élément $(x,y)$ de $F\times G$ associe $x+y$.
