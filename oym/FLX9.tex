...
\organisation{exo7}
\contenu{
  \texte{Soit $(M_n)$ une suite de points dans le plan, de coordonnées $(x_n,y_n)$
...
\end{enumerate}
\begin{enumerate}
  \item \reponse{Diagonaliser ${}^tM  \Rightarrow  y_n -\frac 32x_n = \text{cste}$.}
  \item \reponse{$y_n-x_n = 2^n(y_0-x_0)$ donc si $y_0\ne x_0$ alors $M_n \to\infty$
sinon la suite est constante.}
  \item \reponse{$\frac32$ si $y_0\ne x_0$.}
\end{enumerate}
}