...
\exo7id{5954}
\titre{Exercice 5954}
\theme{Espaces $L^{p, Inégalités de Young et de Hölder}
\auteur{tumpach}
\date{2010/11/11}
\organisation{exo7}
\contenu{
...
\end{enumerate}
\begin{enumerate}
  \item \reponse{Soit $a, b \geq 0$ et soit $p, q \in(1, +\infty)$ tel que
...
%$a = b^{\frac{1}{p-1}}$.}
  \item \reponse{Soit $f\in L^{p}(\mu)$ et $g\in L^{q}(\mu)$. D'apr\`es la
...
i.e. $\|f g\|_{1}\leq \|g\|_{\infty} \|f\|_{1}$.}
  \item \reponse{Soient $p, p' \in [1, +\infty)$. On suppose $p< p'$. Soit
$p<r<p'$. On a
$$
...
donc $f$ appartient \`a $L^{r}(\mu)$.}
  \item \reponse{Supposons que  $\mu$ soit une mesure finie et soit $f\in
L^{\infty}(\mu)$. Alors
$$
|f(x)| \leq \|f\|_{\infty}
$$
...
\|f\|_{\infty}$.}
  \item \reponse{Posons $f_1:=f^r$ et $g_1:=g^r$. On a $f_1\in
