\uuid{5842}
\titre{*** I Topologie dans $\mathcal{M}_n(\Kk)$}
\theme{}
\auteur{rouget}
\date{2010/10/16}
\organisation{exo7}
...
\end{enumerate}
\begin{enumerate}
  \item \reponse{Soit $\begin{array}[t]{cccc}
...
\end{center}}
  \item \reponse{$\mathcal{M}_n(\Rr)\setminus GL_n(\Rr)$ est fermé en tant que complémentaire d'un ouvert.
...
\end{center}}
  \item \reponse{\textbullet~Montrons que $O_n(\Rr)$ est fermé. Posons $\begin{array}[t]{cccc}
...
\end{center}}
  \item \reponse{$S_n(\Rr)$ est un sous espace vectoriel de l'espace de dimension finie $\mathcal{M}_n(\Rr)$ et est donc un fermé de $\mathcal{M}_n(\Rr)$.
...
\end{center}}
  \item \reponse{Soit $A\in\mathcal{M}_n(\Rr)$ et $p$ un élément fixé de $\llbracket1,n-1\rrbracket$ (le résultat est clair si $p=0$ ou $p=n$).
...
$f_{I,J}~:~A\mapsto\text{det}(A_{I,J})$ est continue sur $\mathcal{M}_n(\Rr)$. L'ensemble des matrices $A$ telles que $\text{det}(A_{I,J}) = 0$ est donc un fermé de $\mathcal{M}_n(\Rr)$ (image réciproque du fermé $\{0\}$ de $\Rr$ par l'application continue $f_{I,J}$) et l'ensemble des matrices de rang inférieur ou égal à $p$ est un fermé de $\mathcal{M}_n(\Rr)$ en tant qu'intersection de fermés.}
  \item \reponse{Soit $A\in\mathcal{M}_n(\Cc)$. Posons $\text{Sp}(A)=(\lambda_i)_{1\leqslant i\leqslant n}$. On sait que toute matrice est triangulable dans $\Cc$ et donc il existe $P\in GL_n(\Cc)$ et $T\in\mathcal{T}_n(\Cc)$ avec $\forall i\in\llbracket1,n\rrbracket$, $t_{i,i}=\lambda_i$ telle que $A=PTP^{-1}$.
...
Par suite, aucune des matrices $A+E$ avec $\|E\|_\infty\leqslant \frac{1}{4}$ n'a de valeurs propres réelles et donc aucun donc diagonalisable dans $\Rr$. On a montré que l'ensemble des matrices réelles diagonalisables dans $\Rr$ n'est pas dense dans $\mathcal{M}_n(\Rr)$.}
  \item \reponse{La matrice de la forme quadratique $Q~:~(x,y)\mapsto ax^2+2bxy+cy^2$ dans la base canonique est $\left(
\begin{array}{cc}
a&b\\
b&c
\end{array}
...
L'ensemble des triplets considéré est l'intersection des images réciproques par ces applications de l'ouvert $]0,+\infty[$ de $\Rr$ et est donc un ouvert de $\Rr^3$.}
  \item \reponse{Notons $\mathcal{S}$ l'ensemble des matrices stochastiques.
...
\end{center}}
  \item \reponse{Soient $A$ et $B$ deux matrices réelles diagonalisables. Soient $\begin{array}[t]{cccc}
