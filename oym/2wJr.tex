...
\exo7id{5359}
\titre{***I}
\theme{Le groupe symétrique}
\auteur{rouget}
\date{2010/07/04}
\organisation{exo7}
\contenu{
  \texte{$\sigma$ étant une permutation de $\{1,...,n\}$ donnée, on définit la matrice notée $P_\sigma$, carrée d'ordre $n$ dont le terme ligne $i$ colonne $j$ est $\delta_{i,\sigma(j)}$ (où $\delta_{i,j}$ est le symbôle de \textsc{Kronecker}. On note $G$ l'ensemble des $P_\sigma$ où $\sigma$ décrit $S_n$.}
\begin{enumerate}
...
\begin{enumerate}
  ...
  \item \reponse{Soient $\sigma$ et $\sigma'$ deux éléments de $S_n$. Soit $(i,j)\in\{1,...,n\}^2$. Le coefficient ligne $i$, colonne $j$ de $P_\sigma P_{\sigma'}$ vaut
...
$$\forall(\sigma,\sigma')\in(S_n)^2,\;P_\sigma\times P_{\sigma'}=P_{\sigma\circ\sigma'}.$$}
  \item \reponse{Soit $\sigma\in S_n$. D'après a), $P_\sigma P_{\sigma^{-1}}=P_{\sigma\circ\sigma^{-1}}=P_{Id}=I_n=P_{\sigma^{-1}}P_\sigma$. On en déduit que toute matrice $P_\sigma$ est inversible, d'inverse $P_{\sigma^{-1}}$. Par suite, $G\subset GL_n(\Rr)$ (et clairement, $G\neq\emptyset$).
