...
\organisation{exo7}
\contenu{
  \texte{Une statue de hauteur $s$ est placée sur un 
...
\end{enumerate}
\begin{enumerate}
  \item \reponse{On note $x$ la distance de l'observateur au pied de la statue.
...
Donc la distance optimale de vision est  $x_0 = \sqrt{p(p+s)}$.}
  \item \reponse{Pour calculer l'angle maximum $\alpha_0$ correspondant, on pourrait 
...
on en déduit $\alpha_0 = \Arctan \frac{s}{2x_0} = \Arctan \frac{s}{2\sqrt{p(p+s)}}$.}
  \item \reponse{Pour la statue de la liberté, on a la hauteur de la statue $s=46$ mètres
