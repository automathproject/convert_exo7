...
\contenu{
  \texte{}
  \question{Soit $f$ de classe $C^1$ sur $\Rr$ vérifiant pour tout $x$ réel, $f\circ f(x)=\frac{x}{2}+3$. En remarquant que $f(\frac{x}{2}+3)=\frac{f(x)}{2}+3$, montrer que $f'$ est constante puis déterminer $f$.}
  \reponse{$$\forall x\in\Rr f(\frac{x}{2}+3)=f(f\circ f(x))=f\circ f(f(x))=\frac{f(x)}{2}+3.$$
