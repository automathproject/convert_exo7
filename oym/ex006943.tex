\uuid{6943}
\titre{La loi exponentielle est sans mémoire}
\theme{}
\auteur{ruette}
\date{2013/01/24}
\organisation{exo7}
...
\end{enumerate}
\begin{enumerate}
  \item \reponse{$P(A\mid B):= \frac{P(A\cap B)}{P(B)}$ donc
...
e^{-\lambda s}=P(X> s).$}
  \item \reponse{Soit $H(t)=1-F_X(t)=P(X>t)$. Par hypothèse, on a $H(t+s)=H(t)H(s)$.
