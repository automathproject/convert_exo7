\uuid{4819}
\titre{Th{\'e}or{\`e}me de Hahn-Banach (Polytechnique MP$^*$ 2003)}
\theme{}
\auteur{quercia}
\date{2010/03/16}
\organisation{exo7}
\contenu{
  \texte{Soit~$E$ un espace vectoriel norm{\'e} de dimension finie et~$F$ un hyperplan de~$E$.
...
\end{enumerate}
\begin{enumerate}
  \item \reponse{$f(x_1)+f(x_2) \le \|\kern-1.2pt|f\|\kern-1.2pt|\;\|x_1+x_2\|
                            \le \|\kern-1.2pt|f\|\kern-1.2pt|\;(\|x_1-\varepsilon\| + \|x_2+\varepsilon\|)$.}
  \item \reponse{Prendre $\alpha$ compris entre le sup du premier membre et l'inf du troisi{\`e}me.
    Le sup et l'inf sont dans cet ordre d'apr{\`e}s la question pr{\'e}c{\'e}dente.}
  \item \reponse{Rmq~: $\varphi$ est mal d{\'e}finie, il faut ajouter "$\varphi$ est lin{\'e}aire".
  ...
  \item \reponse{Si $u^k = (u^k_n)_{n\in\N}\in E$ et $(u^k)_{k\in\N}$ est une suite de Cauchy,
    ...
    de limites.}
  \item \reponse{Prendre~$F_n = \{u\in E\text{ tq }u_k=0\text{ si }k\ge n\}$.}
  \item \reponse{D'apr{\`e}s la question~{\bf 3)} on peut construire $f_n$, forme
