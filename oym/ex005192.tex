\exo7id{5192}
\titre{***I Le théorème des noyaux itérés}
\theme{}
\auteur{rouget}
\date{2010/06/30}
\organisation{exo7}
\contenu{
  \texte{\label{exo:suprou10}
...
\end{enumerate}
\begin{enumerate}
  \item \reponse{Soient $k$ un entier naturel et $x$ un élément de $E$.
...
On a montré que~:~$\forall k\in\Nn,\;I_{k+1}\subset I_k$.}
  ...
  \item \reponse{Soit $k$ un entier naturel. Supposons que $N_k=N_{k+1}$.
...
\end{align*}}
  \item \reponse{On a $\{0\}=N_0\subset N_1\subset N_2...$ Supposons que chacune de ces inclusions soient strictes. Alors,
...
que pour $k=p$, on a $N_k=N_p$.}
  \item \reponse{$0<\mbox{dim }N_1<...<\mbox{dim }N_p$ montre que pour $k\leq p$, on a $\mbox{dim }N_k=k$ et en particulier $p
\leq\mbox{dim }N_p=n$.}
\end{enumerate}
}