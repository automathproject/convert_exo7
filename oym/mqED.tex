...
\organisation{exo7}
\contenu{
  \texte{Soit $f$ un morphisme de l'anneau $A$ vers l'anneau $B$.}
\begin{enumerate}
...
\end{enumerate}
\begin{enumerate}
  \item \reponse{Soit $J\subset B$ un idéal premier de $B$. Soient $a,b\in A$ tels que
  ...
  \item \reponse{Prenons $A=\Zz$, $B=\Qq$, $f(k)=k$. $f(\Zz)=\Zz$ n'est pas un idéal
  ...
  \item \reponse{Soit $I$ un idéal maximal de $A$ et $J=f(I)$. Supposons $J\neq B$.
  ...
  \item \reponse{$(X+2)(X+3)=X^{2}+5X$ dans $\Zz_{6}[X]$, donc $(X+\bar{2})(X+\bar{3})\in
    (X)$, mais $(X+\bar{2})\notin(X)$ et $(X+\bar{3})\notin(X)$, donc
