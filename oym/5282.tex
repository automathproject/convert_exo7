...
\contenu{
  \texte{}
  \question{Quelle est la probabilité $p_n$ pour que dans un groupe de $n$ personnes choisies au hasard, deux personnes au moins aient le même anniversaire (on considèrera que l'année a toujours $365$ jours, tous équiprobables). Montrer que pour $n\geq23$, on a $p_n\geq\frac{1}{2}$.}
  \reponse{Si $n\geq366$, on a clairement $p_n=1$ (Principe des tiroirs~:~si $366$ personnes sont à associer à $365$ dates d'anniversaire, alors $2$ personnes au moins sont à associer à la même date d'anniversaire).
