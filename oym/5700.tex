...
\contenu{
  \texte{}
  \question{Soit $\alpha\in\Rr$. Nature de la série de terme général $u_n=\frac{1+(-1)^nn^\alpha}{n^{2\alpha}}$, $n\geqslant1$.}
  \reponse{Si $\alpha<0$, $u_n\underset{n\rightarrow+\infty}{\sim}n^{-2\alpha}$  et si $\alpha=0$, $u_n=1+(-1)^n$. Donc si $\alpha\leqslant0$, $u_n$ ne tend pas vers $0$. La série de terme général $u_n$ diverge grossièrement dans ce cas.
