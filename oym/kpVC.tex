...
\exo7id{2593}
\titre{Exercice 2593}
\theme{Sujets de l'année 2006-2007, Devoir à la maison}
\auteur{delaunay}
\date{2009/05/19}
\organisation{exo7}
\contenu{
  \texte{Soit $f$ l'endomorphisme de $\R^4$ dont la matrice dans la base canonique  est 
...
\end{enumerate}
\begin{enumerate}
  \item \reponse{{\it D\'emontrons que $1$ et $2$ sont des valeurs propres de $f$}.
...
$$\det(A-2I)=\begin{vmatrix}-10&-3&-3&1 \\  6&1&2&-1 \\ 26&7&8&-2 \\ 0&0&0&0\end{vmatrix}=0.$$ Ainsi, les r\'eels $1$ et $2$ sont bien valeurs propres de la matrice $A$.}
  \item \reponse{{\it D\'eterminons des vecteurs propres de $f$ associ\'es aux valeurs propres $1$ et $2$}.
...
$\vec u=(3,-2,-8,0).$}
  \item \reponse{{\it  On consid\`ere le vecteur $\vec u$ pr\'ec\'edent et on determine des vecteurs $\vec v$ et $\vec w$ tels que }
$$f(\vec v)=2\vec v+\vec u\ {\hbox{et}}\ f(\vec w)=2\vec w+\vec v.$$
...
le vecteur $\vec w=(1/2,0,-2,-1)$ convient.}
  \item \reponse{{\it Les vecteurs $\vec e$, $\vec u$, $\vec v$ et $\vec w$ sont ceux d\'efinis pr\'ec\'edemment. On d\'emontre que $(\vec e,\vec u,\vec v,\vec w)$ est une base de $\R^4$ et on  donne la matrice de $f$ dans cette base.}
...
$$B=\begin{pmatrix}1&0&0&0 \\ 0&2&1&0 \\ 0&0&2&1 \\ 0&0&0&2\end{pmatrix}.$$}
  \item \reponse{{\it La matrice $A$ est-elle diagonalisable ?}
