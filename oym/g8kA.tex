...
\exo7id{5965}
\titre{Exercice 5965}
\theme{Intersections des $L^p$ et convergences, Cas de la mesure de comptage sur $\mathbb{N}
\auteur{tumpach}
\date{2010/11/11}
\organisation{exo7}
\contenu{
  \texte{Soit $\Omega = \mathbb{N}$ muni de la mesure de comptage. Pour
...
\end{enumerate}
\begin{enumerate}
  \item \reponse{Montrons que si $q\leq p$, alors $\ell^{q}\subset \ell^{p}$.
...
\subset \ell^{\infty}.
$$}
  \item \reponse{La suite $u^{(\alpha)}_{n} = n^{-\alpha}$ appartient \`a
