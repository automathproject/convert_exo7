...
\organisation{exo7}
\contenu{
  \texte{Etude des suites $(u_n)=(\cos na)$ et $(v_n)=(\sin na)$ où $a$ est un réel donné.}
\begin{enumerate}
...
\end{enumerate}
\begin{enumerate}
  \item \reponse{Posons $a=\frac{2p\pi}{q}$ où $p\in\Zz$, $q\in\Nn^*$ et $\mbox{PGCD}(p,q)=1$. Pour tout entier naturel $n$, on a
...
Or, si $a=\frac{2p\pi}{q}$ où $p\in\Zz$, $q\in\Nn^*$, $\mbox{PGCD}(p,q)=1$ et $\frac{p}{q}\in\Zz$, alors $u_1\neq u_0$ et la suite $u$ n'est pas constante et donc diverge, et si $a\in2\pi\Zz$, la suite $u$ est constante et donc converge.}
  \item \reponse{(a) et b)) Pour tout entier naturel $n$, 
...
Le déterminant de ce système vaut $-\sin^2a-(\cos a-1)^2<0$ car $a\notin2\pi\Zz$. Ce système admet donc l'unique solution $\ell=\ell'=0$ ce qui contredit l'égalité $\ell^2+{\ell'}^2=1$. Donc, les suites $u$ et $v$ divergent.}
  ...
  \item \reponse{Soit $E'=\{na+2k\pi,\;n\in\Nn,\;k\in\Zz\}$. Supposons que $E'$ est dense dans $\Rr$ et montrons que $\{u_n,\;n\in\Nn\}$ et $\{v_n,\;n\in\Nn\}$ sont dense dans $[-1,1]$.
...
Il reste donc à démontrer que $E'$ est dense dans $\Rr$.}
  \item \reponse{Soit $E=\{na+2k\pi,\;n\in\Zz,\;k\in\Zz\}$. $E$ est un sous groupe non nul de $(\Rr,+)$ et donc est soit de la forme $\alpha\Zz$ avec $\alpha=\mbox{inf}(E\cap]0,+\infty[)>0$, soit dense dans $\Rr$ si $\mbox{inf}(E\cap]0,+\infty[)=0$.
...
Mais alors, $a$ étant aussi dans $E$, il existe un entier relatif $p$ tel que $a=p\alpha=\frac{2p\pi}{q}\in2\pi\Qq$. Ceci est exclu et donc, $E$ est dense dans $\Rr$.}
  \item \reponse{Soit $x$ dans $[-1,1]$. D'après ce qui précède, pour $\varepsilon>0$ donné, il existe $n\in\Zz$ tel que $|\cos(na)-x|<\varepsilon$ et donc $|u_{|n|}-x|<\varepsilon$, ce qui montre que $\{u_n,\;n\in\Nn\}$ est dense dans $[-1,1]$. De même, $\{v_n,\;n\in\Nn\}$ est dense dans $[-1,1]$.}
\end{enumerate}
}