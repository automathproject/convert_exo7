...
\exo7id{1956}
\titre{Exercice 1956}
\theme{Droites du plan ; droites et plans de l'espace, Droites dans le plan}
\auteur{liousse}
\date{2003/10/01}
\organisation{exo7}
\contenu{
  \texte{Soit $P$ un  plan muni d'un repère  $\mathcal{R}(O,\vec{i},\vec{j})$, les points et les vecteurs sont exprimés par leurs
...
\begin{enumerate}
  ...
  \item \reponse{Un vecteur directeur est $\overrightarrow{AB}$ dont les coordonnées sont $(x_B-x_A,y_B-y_A)=(-3,1)$.
...
L'équation est donc $x+3y-11=0$.}
  \item \reponse{On trouve $\vec v = \overrightarrow{AB}=(5,-3)$, $p=-\frac35$
...
l'équation $3x+5y+31=0$.}
  \item \reponse{On trouve $\vec v = \overrightarrow{AB}=(0,3)$, la droite est donc verticale (sa pente est infinie)
