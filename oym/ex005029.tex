\exo7id{5029}
\titre{Centre de courbure}
\theme{}
\auteur{quercia}
\date{2010/03/17}
\organisation{exo7}
\contenu{
  \texte{Déterminer les coordonnées du centre de courbure au point $M$ pour les courbes
suivantes :}
\begin{enumerate}
...
\end{enumerate}
\begin{enumerate}
  \item \reponse{$x=-4t^3$, $y=\frac{3+6t^2-3t^4}2$.}
  \item \reponse{$x=6\cos t-3\cos2t$,\quad $y=6\sin t+3\sin 2t$.}
  \item \reponse{$x=t+\sin t$,\quad $y=-1+\cos t$, $I_t = M_{t-\pi} + (\pi,-2)$.}
  \item \reponse{$x=a(\cos^3t + 3\cos t\sin^2t)$,\quad
             $y=a(\sin^3t + 3\sin t\cos^2t)$.
             $x\pm y = a(\cos t\pm \sin t)^3  \Rightarrow  $ similitude de centre $O$,
             rapport $2$, angle $\frac\pi4$.}
  \item \reponse{$x_I = \frac{3x^4+1}{2x^3}$,\quad $y_I = \frac{x^4+3}{2x}$.}
  \item \reponse{$x=\left(a-\frac {b^2}a\right)\cos^3 t$,\quad
             $y=\left(b-\frac {a^2}b\right)\sin^3 t$.}
  \item \reponse{$\rho = e^{\theta-\pi/2}$.}
  \item \reponse{$x=\frac{2+\cos\theta-\cos^2\theta}3$,\quad
