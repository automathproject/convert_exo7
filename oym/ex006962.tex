...
\end{enumerate}
\begin{enumerate}
  \item \reponse{Si $k\in\Zz$ est racine de $P$, alors $k^n+a_{n-1}k^{n-1}+\cdots+a_1k=-a_0$ 
ce qui donne $k(k^{n-1}+\cdots+a_1)=-a_0$, donc $k$ divise $a_0$.}
  \item \reponse{Si $X^3-X^2-109X-11$ a une racine $k\in\Zz$, nécessairement $k$ divise $11$, 
