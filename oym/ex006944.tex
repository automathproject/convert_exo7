\uuid{6944}
\titre{Loi d'un temps d'arrêt avec un jeu de pile ou face}
\theme{}
\auteur{ruette}
\date{2013/01/24}
\organisation{exo7}
\contenu{
  \texte{Soit $(X_n)_{n\geq 1}$ une suite de variables aléatoires indépendantes de même loi de
...
\end{enumerate}
\begin{enumerate}
  \item \reponse{$\{\omega\mid T_1(\omega)=+\infty\}=\{\omega\mid X_n(\omega) =0
...
est fini presque sûrement.}
  \item \reponse{Pour tout $k\in\Nn^*$, $\{T_1=k\}=\{X_1=0, X_2=0,\ldots, X_{k-1}=0, X_k=1\}$ donc
...
$E(T_1)=p f'(1-p)=\frac{1}{p}$.}
  \item \reponse{Montrons par récurrence que $T_n$ est finie presque sûrement
...
Conclusion : les variables aléatoires $T_1, T_2-T_1,\ldots, T_n-T_{n-1}$ sont indépendantes.}
  \item \reponse{Ce qui précède permet de voir que, pour avoir
