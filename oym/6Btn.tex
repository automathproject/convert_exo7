\uuid{6Btn}
\exo7id{2096}
\titre{Intégrales de Wallis}
\theme{Calculs d'intégrales}
\auteur{bodin}
\date{2008/02/04}
\organisation{exo7}
\contenu{
  \texte{Soit $\displaystyle I_n=\int_0^{\frac \pi 2}(\sin x)^n \, d x$ \ \ pour $n\in \N$.}
\begin{enumerate}
...
\begin{enumerate}
  ...
  \item \reponse{$$ I_{n+2}  = \int_0^{\frac \pi 2} \sin^{n+1} x \cdot \sin x \,  dx.$$
...
$$I_{n+2} = \frac{n+1}{n+2} I_n.$$}
  \item \reponse{Nous avons donc une formule de récurrence pour $I_n$ qui s'exprime en fonction de $I_{n-2}$
...
$$I_n = \frac{2 \cdot 4 \cdots (n-1)}{1 \cdot 3 \cdots n}.$$}
  \item \reponse{Pour calculer $\int_{-1}^1\left( 1-x^2\right) ^n d x$ nous allons nous ramener à une intégrale de Wallis.
