...
\contenu{
  \texte{}
  \question{Soient $p$ et $q$ deux projecteurs d'un $\Cc$-espace vectoriel $E$.
...
Dans le cas où $p+q$ est un projecteur, déterminer $\text{Ker}(p+q)$ et $\text{Im}(p+q)$.}
  \reponse{$\Rightarrow$/ Si $p+q$ est un projecteur alors l'égalité $(p+q)^2=p+q$ founit $pq+qp=0$. En composant par $p$ à droite ou à gauche , on obtient  $pqp+qp=0=pq+pqp$ et donc  $pq = qp$.
