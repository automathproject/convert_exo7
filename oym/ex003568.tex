\exo7id{3568}
\titre{$v \mapsto v\circ u$ (Centrale MP 2003)}
\theme{}
\auteur{quercia}
\date{2010/03/10}
\organisation{exo7}
\contenu{
  \texte{Soit~$E$ un espace vectoriel de dimension finie et $u\in\mathcal{L}(E)$. On considère l'application~$\Phi_u$ qui à $v\in\mathcal{L}(E)$
associe~$v\circ u$.}
\begin{enumerate}
...
\begin{enumerate}
  ...
  \item \reponse{Pour $p\in K[X]$ on a $P(\Phi_u) = v \mapsto v\circ P(u)$ donc $u$ et $\Phi_u$ ont
    mêmes polynômes annulateurs.}
  \item \reponse{$(\lambda\in\mathrm{Spec}(\Phi_u)) \Leftrightarrow (\exists\ v\ne 0\text{ tq }v\circ(u-\lambda\mathrm{id}_E) = 0)
