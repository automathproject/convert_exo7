...
\contenu{
  \texte{}
  \question{Pour $n\in\Nn^*$ et $t\in\Rr$, soit $f_n(t) =\frac{\Arctan(nt)}{n^2}$.
...
Etude complète de $f =\sum_{n=1}^{+\infty}f_n$ : domaine de définition, parité, limites, continuité, dérivabilité (vérifier que $f$ n'est pas dérivable en $0$), allure du graphe.}
  \reponse{\textbf{Domaine de définition.} Soit $t\in\Rr$. Pour chaque $n\in\Nn^*$, $f_n(t)$ existe et de plus $f_n(t)=\frac{\Arctan(nt)}{n^2}\underset{n\rightarrow+\infty}{=}O\left(\frac{1}{n^2}\right)$. Donc la série numérique de terme général $f_n(t)$, $n\geqslant1$, converge absolument et en particulier converge. On a montré que
