...
\exo7id{5535}
\titre{Exercice 5535}
\theme{Etude métrique des courbes}
\auteur{rouget}
\date{2010/07/15}
\organisation{exo7}
\contenu{
  \texte{Longueur $L$ de $(\Gamma)$ dans chacun des cas suivants~:}
\begin{enumerate}
...
\end{enumerate}
\begin{enumerate}
  \item \reponse{L'astroïde complète est obtenue quand $t$ décrit $[-\pi,\pi]$ et pour des raisons de symétrie, $L=4\int_{0}^{\pi}{2}\left\|\overrightarrow{\frac{dM}{dt}}\right\| dt$.
...
\end{center}}
  \item \reponse{$\overrightarrow{\frac{dM}{dt}}=\left(\begin{array}{c}
R(1-\cos t)\\
R\sin t
\end{array}
...
\end{center}}
  \item \reponse{Une représentation paramétrique de $\Gamma$ est $\left\{
\begin{array}{l}
x=t\\
y=\frac{t^2}{2p}
\end{array}
...
\end{center}}
  \item \reponse{La cardioïde complète est obtenue quand $\theta$ décrit $[-\pi,\pi]$.
