...
\organisation{exo7}
\contenu{
  \texte{Dans $\Rr^2$ on consid\`ere l'ensemble $A= \{ (x, \sin(
\frac{1}{x}))\; ; \; x >0 \}$.}
\begin{enumerate}
...
\end{enumerate}
\begin{enumerate}
  \item \reponse{Si $(x_1, \sin \frac{1}{x_1})$ et $(x_2, \sin \frac{1}{x_2})$
...
$\gamma : [x_1,x_2] \longrightarrow \Rr^2$ définie par $\gamma(t) = (t,\sin \frac 1t)$. Donc $A$ est connexe par arcs donc connexe.}
  \item \reponse{$\bar A = A \cup (\{0\}\times [-1,1])$. On peut utiliser l'exercice \ref{exocon} pour montrer que $\bar A$ est connexe. Ici nous allons le montrer directement.
