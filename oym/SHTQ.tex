\uuid{SHTQ}
\exo7id{5417}
\titre{**** Toute fonction dérivée vérifie le théorème des valeurs intermédiaires}
\theme{Fonctions réelles d'une variable réelle dérivables (exclu études de fonctions)}
\auteur{rouget}
\date{2010/07/06}
\organisation{exo7}
\contenu{
  \texte{Soit $f$ une fonction dérivable sur un intervalle ouvert $I$  à valeurs dans $\Rr$. Soient $a$ et $b$ deux points distincts de $I$ vérifiant $f'(a)<f'(b)$ et soit enfin un réel $m$ tel que $f'(a)<m<f'(b)$.}
\begin{enumerate}
...
\end{enumerate}
\begin{enumerate}
  \item \reponse{Soit $m$ un élément de $]f'(a),f'(b)[$. Puisque $\lim_{h\rightarrow 0}\frac{f(a+h)-f(a)}{h}=f'(a)$ et que 
...
$h>0$ est ainsi dorénavant fixé.}
  \item \reponse{La fonction $f$ est continue sur $I$ et donc, la fonction $g~:~x\mapsto\frac{f(x+h)-f(x)}{h}$ est continue sur $[a,b]$. D'après le théorème des valeurs intermédiaires, comme $g(a)<m<g(b)$, $\exists y\in[a,b]/\;g(y)=m$ ou encore $\exists y\in[a,b]/\;\frac{f(y+h)-f(y)}{h}=m$.
