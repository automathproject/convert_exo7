...
\organisation{exo7}
\contenu{
  \texte{Soit $f: {[0,1]} \to \R$ de classe $\mathcal{C}^2$ telle que $f(0)=f(1)=0$.}
\begin{enumerate}
...
\end{enumerate}
\begin{enumerate}
  \item \reponse{Immédiat. La fonction prolongée est $\mathcal{C}^1$ sur~$\R$ et $\mathcal{C}^2$ par morceaux.}
  \item \reponse{On décompose $f$ en série de Fourier~:
