...
\organisation{exo7}
\contenu{
  \texte{Soit $E$ un $\Kk$-espace vectoriel et soit $(u,v)\in(\mathcal{L}(E))^2$.}
\begin{enumerate}
...
\begin{enumerate}
  ...
  \item \reponse{[$\Leftarrow$] Soit $(u,v)((\mathcal{L}(E))^2$. On suppose qu'il existe $w\in\mathcal{L}(E)$ tel que $u=w\circ
...
$\mbox{Ker}u$. Donc $\mbox{Ker}v\subset{Ker}u$.}
  \item \reponse{[$\Rightarrow$] Supposons que $\mbox{Ker}v\subset\mbox{Ker}u$. On cherche à définir $w$, élément de
...
$$w(\lambda X)=w(\lambda y+\lambda z)=u(\lambda x)=\lambda u(x)=\lambda w(X).$$}
  \item \reponse{On applique 1) à $u=Id$.
