\exo7id{3231}
\titre{{\'E}quations fonctionnelles}
\theme{}
\auteur{quercia}
\date{2010/03/08}
\organisation{exo7}
\contenu{
  \texte{Trouver tous les polyn{\^o}mes $P \in {\C[X]}$ tels que \dots}
\begin{enumerate}
...
\end{enumerate}
\begin{enumerate}
  \item \reponse{si $P(x) = 0$, alors $P\bigl((x-1)^2\bigr) = P\bigl((x+1)^2\bigr) = 0$.
\\
...
Donc $P = 0$ ou $P = 1$.}
  \item \reponse{Si $x$ est racine, alors $x^2$ et $(x+1)^2$ le sont aussi.
\\
...
$ \Rightarrow  P = (X^2 + X + 1)^\alpha$.}
  \item \reponse{Seule racine possible : 1 $ \Rightarrow  P=-(X-1)^k$.}
\end{enumerate}
}