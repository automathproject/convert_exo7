\uuid{OK7d}
\exo7id{4176}
\titre{Non injectivité locale de l'exponentielle}
\theme{Exercices de Michel Quercia, Dérivées partielles}
\auteur{quercia}
\date{2010/03/11}
\organisation{exo7}
\contenu{
  \texte{Soit $f : {\mathcal{M}_n(\C)} \to {\mathcal{M}_n(\C)}, M  \mapsto {\exp(M).}$}
\begin{enumerate}
...
\end{enumerate}
\begin{enumerate}
  \item \reponse{$d f_M(H) = \sum_{k=0}^\infty \frac1{k!}(M^{k-1}H + \dots + HM^{k-1})$.}
  \item \reponse{les matrices $M_k = \begin{pmatrix}0&1/k\cr0&2i\pi\cr\end{pmatrix}$ sont toutes
semblables à~$M_\infty$ et ont même exponentielle $I$.}
  \item \reponse{$M_k=\begin{pmatrix}0&2\pi+1/k\cr-4\pi^2/(2\pi+1/k)&0\cr\end{pmatrix}$.}
\end{enumerate}
}