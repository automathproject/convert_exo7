...
\organisation{exo7}
\contenu{
  \texte{Soit $n\in\Nn^*$ et $P(X)=c(X-a_1)\cdots(X-a_n)$ 
...
\begin{enumerate}
  ...
  \item \reponse{Puisque $P(X)=c(X-a_1)\cdots(X-a_n)$ : 
\begin{eqnarray*}
...
$$\frac{P'}{P}=\sum_{k=1}^n\frac{1}{X-a_k}$$}
  \item \reponse{Puisque $\sum_{k=1}^n\frac{1}{(X-a_k)^2}$ est la dérivée de $-\sum_{k=1}^n\frac{1}{X-a_k}$, on obtient
  par dérivation de $-\frac{P'}{P}$ :
$$\frac{P'^2-PP''}{P^2}=\sum_{k=1}^n\frac{1}{(X-a_k)^2}$$}
  \item \reponse{On a remarqué que la dérivée de $P'$ est la somme de facteurs $c(X-a_1)\cdots(X-a_n)$
