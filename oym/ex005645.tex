...
\end{enumerate}
\begin{enumerate}
  \item \reponse{$d=\sum_{\sigma\in  S_n}^{}\varepsilon(\sigma)a_{\sigma(1),1}...a_{\sigma(n),n}$ est dérivable sur $\Rr$ en tant que combinaison linéaire de produits de fonctions dérivables sur $\Rr$ et de plus
...
\end{align*}}
  \item \reponse{\textbf{1 ère solution.} D'après ce qui précède, la fonction $d_n$ est dérivable sur $\Rr$ et pour $n\geqslant 2$ et $x$ réel, on a
