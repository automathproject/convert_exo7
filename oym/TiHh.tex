...
\organisation{exo7}
\contenu{
  \texte{Soit $X$ l'espace des suites r\'eelles nulles \`a partir d'un certain rang, et soit
...
\end{enumerate}
\begin{enumerate}
  \item \reponse{Notons $x^p$ la suite 
...
La suite $(x^p)$ est de Cauchy, mais ne converge pas dans $X$, donc $X$ n'est pas complet.}
  ...
  \item \reponse{Soit $Y$ l'espace de toutes les suites. Alors $X$ est dense dans
...
qui tend vers $0$ lorsque $p$ tend vers $+\infty$.}
  \item \reponse{Soit $(x^n)_n$ une suite de Cauchy de $Y$.
...
donc la suite $(x^n_k)_n$ est de Cauchy dans $\Rr$, donc converge, nous notons $x^\infty_k$ sa limite.}
  \item \reponse{Nous avons construit une suite $x^\infty=(x^\infty_1,x^\infty_2,\ldots)$. Comme $(x^n)$ est de Cauchy alors
$$\forall \epsilon >0 \quad \exists N \in \Nn \quad \forall p,q \ge N \quad
\rho(x^p,x^q) < \epsilon,$$
...
donc $(x^n)$ converge vers $x^\infty$ pour la distance $\rho$.}
  \item \reponse{Bien évidemment $x^\infty \in Y$ donc
