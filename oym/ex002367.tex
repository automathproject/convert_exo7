...
\end{enumerate}
\begin{enumerate}
  \item \reponse{Si $L(a) = 0$ alors $a\in H$ donc $\mathrm{dist}(a,H) = 0$ donc la relation est vraie. Supposons que $L(a) \neq 0$. Alors on a $X = H+\Rr.a$.
...
Ce qui était l'égalité demandée.}
  \item \reponse{Si $H$ est fermé alors $\mathrm{dist}(a,H)>0$ si $a\notin H$ (voir les exercices sur les compacts), par l'égalité démontrée ci-dessus on a $\| L \|$ finie
donc $L$ est continue.}
  \item \reponse{Soit $X = \Rr[x]$. Pour $P(x) = \sum _{k=0}^p a_k x^k$
