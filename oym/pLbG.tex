...
\organisation{exo7}
\contenu{
  \texte{On considère l'équation différentielle $(E)$~:~$ax^2y''+bxy'+cy= 0$ ($a$, $b$, $c$ réels, $a\neq0$)
pour $x\in]0,+\infty[$.}
\begin{enumerate}
...
\end{enumerate}
\begin{enumerate}
  \item \reponse{Supposons $y$ deux fois dérivable sur $]0,+\infty[$. La fonction $t\mapsto e^t$ est deux fois dérivable sur
...
$]0,+\infty[$.}
  \item \reponse{Pour $t$ réel, posons donc $x=e^t$ puis, $z(t)=y(x)=y(e^t)$. Alors, $z'(t)=e^ty'(e^t)=xy'(x)$ puis
...
$$\forall x>0,\;ax^2y''(x)+bxy'(x)+cy(x)=0\Leftrightarrow\forall t\in\Rr,\;az''(t)+(b-a)z'(t)+cz(t)=0.$$}
  \item \reponse{On applique le 2) avec $a=1$, $b=-1$ et $c=1$. L'équation à résoudre sur $\Rr$ est alors $z''-2z'+z=0$. Les
