\uuid{3695}
\titre{Division vectorielle}
\theme{Exercices de Michel Quercia, Espace vectoriel euclidien orienté de dimension 3}
\auteur{quercia}
\date{2010/03/11}
\organisation{exo7}
\contenu{
  \texte{}
  \question{Soit $E$ un espace vectoriel euclidien orienté de dimension 3 et $\vec a,\vec b$ deux vecteurs
donnés, $\vec a \ne \vec 0$.
\'Etudier l'équation : $\vec a\wedge\vec x = \vec b$.
On cherchera une solution particulière de la forme
$\vec x = \vec a\wedge\vec y$.}
  \reponse{$\vec x = -\frac{\vec a\wedge\vec b}{\|\vec a\|^2} + \lambda \vec a$.}
}