\uuid{3195}
\titre{Polyn{\^o}mes {\`a} coefficients entiers, ULM-Lyon-Cachan MP$^*$ 2004}
...
\contenu{
  \texte{}
  \question{On donne un entier $n\ge 0$.

Montrer qu'il existe des polyn{\^o}mes
$P_0,\dots,P_n$ dans $\Z_n[X]$ tels que
$\forall\ i,j\in{[[0,n]]},\  \int_{t=0}^1 t^iP_j(t)\,dt = \delta_{ij}$.}
  \reponse{Analyse~: on pose $P_j = a_0 + a_1X + \dots + a_nX^n$ et on consid{\`e}re la fraction rationnelle
