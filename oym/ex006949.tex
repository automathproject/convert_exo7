\uuid{6949}
\titre{Fonctions caractéristiques}
\theme{}
\auteur{ruette}
\date{2013/01/24}
\organisation{exo7}
...
\end{enumerate}
\begin{enumerate}
  \item \reponse{$\displaystyle\varphi_X(t)=\sum_{k=0}^{+\infty}e^{itk}e^{-\lambda}
\frac{\lambda^k}{k!}=e^{-\lambda}\sum_{k=0}^{+\infty}
\frac{(\lambda e^{it})^k}{k!}=e^{-\lambda}\exp(\lambda e^{it})$, donc
$\varphi_X(t)=\exp(\lambda(e^{it}-1))$.}
  \item \reponse{$\varphi_Y(t)=\exp(\lambda'(e^{it}-1))$ par 1. Par indépendance,
