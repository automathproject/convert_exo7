\exo7id{5033}
\titre{Normales à une cardioïde}
\theme{}
\auteur{quercia}
\date{2010/03/17}
\organisation{exo7}
\contenu{
  \texte{On considère la courbe $\mathcal{C}$ d'équation polaire $\rho = 1 + \cos\theta$
(cardioïde).}
\begin{enumerate}
  \item \question{Dessiner $\mathcal{C}$.}
  \item \question{Une droite $D$ passant par $O$ coupe $\mathcal{C}$ en deux points $M_1$ et $M_2$.
    Soient $\Delta_1$, $\Delta_2$ les normales à $\mathcal{C}$ en ces points et $P$
    le point d'intersection de $\Delta_1$ et $\Delta_2$. Quelle est la courbe
    décrite par $P$ lorsque $D$ tourne autour de $O$~?}
\end{enumerate}
\begin{enumerate}
  \item \reponse{Cercle de centre $(\frac12,0)$ et de rayon $\frac12$.}
\end{enumerate}
}