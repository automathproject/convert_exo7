\uuid{7822}
\titre{Complété non singulier}
\theme{}
\auteur{mourougane}
\date{2021/08/11}
\organisation{exo7}
\contenu{
  \texte{Soit $(E,q)$ un espace vectoriel muni d'une forme quadratique non
...
de $F^\perp\cap F$.}
\begin{enumerate}
  \item \question{Montrer que $G$ n'est pas isotrope.}
  \item \question{Montrer que $G^\perp$ contient strictement $F^\perp$.}
  \item \question{On suppose que $s=1$. Montrer qu'il existe un plan d'Artin $P_1$
 contenant $x_1$ et tel que $G\oplus ^\perp P_1$ soit non singulier.}
  \item \question{Montrer qu'il existe des plans d'Artin
...
(On pourra considérer $G':=G\oplus^\perp Vect(x_1,x_2,\cdots ,x_{s-1}$)}
  \item \question{Soit $F$ un sous espace singulier de $E$.
 Montrer que si $H$ est un sous espace
 non singulier qui contient $F$ alors $\dim H\geq \dim F+\dim
 F^\perp\cap F$.}
  \item \question{Montrer que tout espace quadratique non dégénéré 
qui contient un sous-espace totalement isotrope de dimension moitié
est un espace d'Artin.}
  \item \question{Montrer que toute isométrie $u~:F\to F'$ se prolonge en une
