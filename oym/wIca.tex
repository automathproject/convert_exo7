...
\organisation{exo7}
\contenu{
  \texte{On définit une suite de fonctions sur $[0,1]$ de la manière suivante~:
...
\end{enumerate}
\begin{enumerate}
  \item \reponse{$f_{n+1}(x) - f_n(x) =  \int_{t=0}^x (f_n-f_{n-1})(t-t^2)\,d t$
    ...
    convergente.}
  \item \reponse{Par passage à la limite uniforme sous le signe intégral
    ...
    récurrence.
    $f'(0) = f'(1) = f(0) = 1$.}
  \item \reponse{$f'$ est positive d'après l'équation différentielle vérifiée par~$f$
    et $f''(x) = (1-2x)f'(x-x^2)$ est du signe de $1-2x$, c'est-à-dire que $f$
    est convexe sur $[0,\frac12]$ et concave sur $[\frac12,1]$.}
  \item \reponse{$1+x = f_1(x) \le f(x) = f_1(x) + \sum_{k=1}^\infty(f_{k+1}(x) - f_k(x))$.
