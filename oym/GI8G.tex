...
\organisation{exo7}
\contenu{
  \texte{Soit $a\in {]-1,1[}$ et $g$ : $x \mapsto\frac{1-a\cos x}{ 1-2a\cos x +a^2}$.}
\begin{enumerate}
...
\end{enumerate}
\begin{enumerate}
  \item \reponse{$\sum_{n=0}^\infty a^n\cos nx = \Re\Bigl(\sum_{n=0}^\infty (ae^{ix})^n\Bigr)
    =\Re\Bigl(\frac1{1-ae^{ix}}\Bigr) = g(x)$.}
  \item \reponse{Il y a convergence normale.}
  \item \reponse{\begin{align*} h(x)
  ...
  \item \reponse{Les coefficients de Fourier des deux membres doivent être égaux, ce qui
