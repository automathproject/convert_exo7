\uuid{2447}
\titre{Exercice 2447}
\theme{}
\auteur{matexo1}
\date{2002/02/01}
\organisation{exo7}
\contenu{
  \texte{Soit $E$ l'espace vectoriel des fonctions r\'eelles
ind\'efiniment d\'erivables \`a valeurs dans $\R$.}
\begin{enumerate}
  \item \question{Montrer que les quatre fonctions d\'efinies par
\begin{eqnarray*}
&&x_1(t) = \cos t \cosh t, \\
&&x_2(t) = \sin t \cosh t, \\
&&x_3(t) = \cos t \sinh t, \\
&&x_4(t) =\sin t \sinh t
\end{eqnarray*}
appartiennent \`a $E$ et sont lin\'eairement ind\'ependantes.}
  \item \question{Soit $F$ le sous-espace vectoriel de $E$ engendr\'e par
ces quatres vecteurs, et $u$ l'endomor\-phisme de $E$ d\'efini par
$u(f) = f'$. Montrer que $F$ est stable par $u$ et d\'eterminer
la matrice $M$ de $u$ dans la bae $(x_1, x_2, x_3, x_4)$ de $F$.}
  \item \question{Calculer $M^n$.}
\end{enumerate}
\begin{enumerate}

\end{enumerate}
}