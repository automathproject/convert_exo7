...
\organisation{exo7}
\contenu{
  \texte{Soit $f : {\R^+} \to \R$. On pose, sous réserve de convergence,
...
\end{enumerate}
\begin{enumerate}
  \item \reponse{En supposant $f$ positive décroissante, $ \int_0^{+\infty}f \le tg(t) \le tf(0) +  \int_0^{+\infty}f$.}
  \item \reponse{$ \int_{u=Pt}^{Qt}f(u)\,d u - \sum_{n=P}^{Q-1}tf(nt) =  \int_{u=Pt}^{Qt}(f(u)-f(t[u/t]))\,d u =  \int_{u=Pt}^{Qt}t(1-\{u/t\})f'(u)\,d u\to 0$ lorsque $P,Q\to\infty$.
...
On a alors ${ \int_0^{+\infty}\!\!f} - tg(t) =  \int_{u=0}^{+\infty}t(1-\{u/t\})f'(u)\,d u\to 0$ lorsque $t\to0^+$.}
  \item \reponse{${ \int_0^{+\infty}\!\!2f} - 2tg(t) =  \int_{u=0}^{+\infty}tf'(u)\,d u +  \int_{u=0}^{+\infty}t(1-2\{u/t\})f'(u)\,d u = tf(0) -  \int_{u=0}^{+\infty}t^2\{u/t\}(1-\{u/t\})f''(u)\,d u$.}
\end{enumerate}
}