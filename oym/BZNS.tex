...
\organisation{exo7}
\contenu{
  \texte{Pour chacune des fonctions suivantes \'etudier la nature du point
critique donn\'e :}
\begin{enumerate}
...
\end{enumerate}
\begin{enumerate}
  \item \reponse{$df=(2x-y)\mathrm dx + (2y-x)\mathrm dy$ et
...
est positive et ce point pr\'esente donc un minimum local.}
  \item \reponse{$f(x,y)=x^2+2xy+y^2+6= (x+y)^2+6$ d'o\`u 
le point $(0,0)$
pr\'esente un minimum local.}
  \item \reponse{$df=(3x^2+2x+2y^2 +3y)\mathrm dx + (4xy-4y^3 +3x+2y)\mathrm dy$ et
\[
