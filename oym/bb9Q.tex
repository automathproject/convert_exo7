...
\organisation{exo7}
\contenu{
  \texte{Calculer les primitives suivantes, en précisant si nécessaire les
...
\end{enumerate}
\begin{enumerate}
  \item \reponse{$\int \frac{x+2}{x^2-3x-4}\,dx$
...
Cette primitive est définie sur $\Rr\setminus\left\{-1,4\right\} $}
  \item \reponse{$\int \frac{x-1}{x^2+x+1}\,dx$
...
Cette primitive est définie sur $\Rr$.}
  \item \reponse{$\int \sin ^8x\cos ^3x \, dx$
...
Cette primitive est définie sur $\Rr$.}
  \item \reponse{$\int \frac 1{\sin x} \, dx$
...
Un autre changement de variable possible aurait été $t=\tan \frac x2$.}
  \item \reponse{$\int \frac{3-\sin x}{2\cos x+3\tan x}\,dx$
