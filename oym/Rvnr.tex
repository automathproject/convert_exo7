...
\organisation{exo7}
\contenu{
  \texte{On se propose d'int\'egrer sur l'intervalle le plus grand possible contenu dans $]0,
...
\end{enumerate}
\begin{enumerate}
  \item \reponse{Trouvons $a \in ]0, \infty[ $ tel que $y_0(x)=ax$ soit une solution
particuli\`ere. Puisque
\[ y_0'(x) - \frac{y_0(x)}{x}-y_0(x)^2 = - a^2 x^2,\]
$y_0$ est solution si et seulement si $a = \pm 3$. On choisit $a=3$.}
  \item \reponse{Si $z$ est une fonction $\mathcal{C}^1$ ne s'annulant pas, on pose $y(x) = 3x-1/{z(x)}$. Alors $y$ est solution si et seulement si
...
\end{equation}}
  \item \reponse{On résout \eqref{eq1} sur $]0,\infty[$. Une primitive de $x \mapsto 6x + 1/x$ est $x \mapsto 3x^2 + \ln(x)$, donc les solutions de l'équation homogène sont les $x \mapsto A \exp(-3x^2 - \ln(x))$. On cherche une solution particulière de \eqref{eq1} sous la forme $z_p(x) = \alpha(x) \exp(-3x^2-\ln(x))$ ; alors $z_p$ est solution si
 ...
 \[ z(x) = \frac{1+A\exp(-3x^2)}{6x}, \qquad \text{ avec } A \in \R.\]}
  \item \reponse{On va maintenant en déduire les solutions de \eqref{eq} d\'efinies sur $]0,\infty[$. 
