\exo7id{197}
\titre{Exponentielle complexe}
\theme{}
\auteur{bodin}
\date{1998/09/01}
\organisation{exo7}
\contenu{
  \texte{Si $z=x+iy$, $(x,y)\in \Rr^2$, on pose $e^z=e^x \times e^{iy}$.}
\begin{enumerate}
...
\end{enumerate}
\begin{enumerate}
  \item \reponse{Pour $z=x+iy$, le  module de $e^z=e^{x+iy}=e^xe^{iy}$ est $e^x$ et
son argument est $y$.}
  \item \reponse{Les r\'esultats : $e^{z+z'} = e^ze^{z'}$, $e^{\overline{z}} =
\overline{e^z}$, $e^{-z}= \left( e^{z} \right)^{-1}$,
$(e^z)^n=e^{nz}$.}
  \item \reponse{La fonction $\exp$ n'est pas surjective car $|e^z| = e^x >0$ et
