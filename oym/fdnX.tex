...
\organisation{exo7}
\contenu{
  \texte{Soient $E$ et $F$ deux $\Kk$-espaces vectoriels et $f$ une application linéaire de $E$ vers $F$.}
\begin{enumerate}
...
Finalement, $f$ est bien un isomorphisme de $E$ sur $F$.}
  \item \reponse{Soit $A=\{g\in\mathcal{L}(F,E)/\;f\circ g\circ f = 0\}$. Tout d'abord $A$ est bien un sous-espace vectoriel de $\mathcal{L}(F,E)$ car contient l'application nulle et est stable par combinaison linéaire (ou bien $A$ est le noyau de l'application linéaire de $\mathcal{L}(F,E)$ dans $\mathcal{L}(E,F)$ qui à $g$ associe $f\circ g\circ f$).
