\uuid{9gEt}
\exo7id{2210}
\titre{Matrices triangulaires \'el\'ementaires}
\theme{Quelques compléments d'algèbre matricielle}
\auteur{matos}
\date{2008/04/23}
\organisation{exo7}
\contenu{
  \texte{Soit $n \in \Nn$ et on d\'efinit les matrices suivantes dans $\Rr^{n\times n}$:
\begin{itemize}
...
\end{enumerate}
\begin{enumerate}
  \item \reponse{$VA$ remplace la ligne $i$ par sa somme avec la ligne $j$ multipli\'ee par $\lambda$.

$AV$ remplace la colonne $j$ par sa somme avec la colonne $i$ multipli\'ee par $\lambda$.}
  \item \reponse{$V_{ij}(\lambda )V_{kj})=I+\lambda E_{ij} + \lambda ' E_{kj}$}
  \item \reponse{Il suffit de montrer que $(I+l_ie_i^T)(I-l_ie_i^T)=I$.}
  \item \reponse{$L(l_i)=V_{i+1,i}(l_{i+1,i}) \cdots V_{n,i}(l_{n,i})$}
  \item \reponse{$L^{-1}=L(-l_{n-1}) L(-l_{n-2}) \cdots L(-l_1)\neq I-l_1e_1^T - \cdots -l_{n-1}e_{n-1}^T$}
  \item \reponse{(a) algorithme en utilisant l'expression de $L^{-1}$
