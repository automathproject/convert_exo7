...
\organisation{exo7}
\contenu{
  \texte{$E$ est l'ensemble des fonctions de $\R$ dans $\R$ continues et born{\'e}es sur $\R$.
...
\begin{enumerate}
  ...
  \item \reponse{Si $p<q$ alors $N_p(x \mapsto e^{-n|x|})/N_q(x \mapsto e^{-n|x|}) \to \infty$ losrque $n\to \infty$.}
\end{enumerate}
}