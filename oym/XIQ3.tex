...
\organisation{exo7}
\contenu{
  \texte{Soit $E$ un espace vectoriel de dimension $n$. On cherche à déterminer une matrice $A\in{\cal M}_n(\R)$ telle que $A^2=-I_n$, où $I_n$ désigne la matrice identité d'ordre $n$. On notera $f$ l'endomorphisme de $E$ de matrice $A$ dans la base canonique.}
\begin{enumerate}
...
\end{enumerate}
\begin{enumerate}
  \item \reponse{{\it Démontrons que l'existence d'une telle matrice implique la parité de $n$.}
...
ce qui implique $n$ pair, car un carré est toujours positif.}
  \item \reponse{On suppose maintenant que $n=4$.
   \begin{enumerate}}
  \item \reponse{{\it Démontrons que pour tout $\vec x\in E$, $\vec x\neq 0$, les vecteurs $\vec x$ et $f(\vec x)$ sont linéairement indépendants.}
...
ce qui implique $a^2+b^2=0$ car $\vec x\neq \vec 0$, et, donc $a=b=0$. Ce qui prouve que les vecteurs $\vec x$ et $f(\vec x)$ sont linéairement indépendants.}
  \item \reponse{Soit $\vec x_1\neq0$, on note $F$ le sous-espace vectoriel de $E$ engendré par les vecteurs $\vec x_1$ et $f(\vec x_1)$.
            \begin{enumerate}}
  \item \reponse{{\it Démontrons que $F$ est stable par $f$.}
...
D'où la stabilité de $F$ par $f$.}
  \item \reponse{Soit $\vec x_2\in E$, on suppose que $\vec x_2\not\notin F$. 
