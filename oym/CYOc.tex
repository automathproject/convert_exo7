...
\exo7id{1268}
\titre{Exercice 1268}
\theme{Développements limités, Formules de Taylor}
\auteur{legall}
\date{1998/09/01}
\organisation{exo7}
\contenu{
  \texte{Soit $a$ un nombre réel et $f : ] a , +\infty [ \rightarrow \Rr$ une application de classe $C^2$. 
...
\end{enumerate}
\begin{enumerate}
  \item \reponse{La formule de Taylor-Lagrange à l'ordre $2$ entre $x$ et $x+h$ (avec $h>0$) donne :
...
$$|f'(x)|\le \frac{2}{h}{M_0} + \frac{h}{2}M_2.$$}
  \item \reponse{Soit $\phi : ]0,+\infty[ \rightarrow \Rr$ la fonction définie par $\phi(h) = \frac {h}{2}M_2+\frac{2}{h}M_0$.
...
$h=h_0$ on obtient $|f'(x)| \le \phi(h_0)= 2\sqrt{M_0M_2}$. Et donc $f'$ est bornée.}
  \item \reponse{Fixons $\epsilon >0$. $g''$ est bornée, notons $M_2 = \sup_{x> 0}\vert g''(x)\vert$. Comme $g(x)\to 0$ alors il 
