...
\organisation{exo7}
\contenu{
  \texte{Soient $\Kk$ un sous-corps de $\Cc$ et $E$ un $\Kk$-espace vectoriel de dimension finie notée $n$. Soit $u$ un endomorphisme de $E$.
...
\end{enumerate}
\begin{enumerate}
  \item \reponse{Soit $p(\in\Nn^*)$ l'indice de nilpotence de $u$.
...
Donc tous les coefficients $\lambda_k$ sont nuls et on a montré que la famille $(u^k(x))_{0\leq k\leq p-1}$ est libre.}
  \item \reponse{Le cardinal d'une famille libre est inférieur ou égal à la dimension de l'espace et donc $p\leq n$. 
Par suite, $u^n=u^p\circ u^{n-p}=0$.}
  \item \reponse{On applique l'exerice \ref{exo:suprou10}.
