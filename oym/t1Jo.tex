...
\organisation{exo7}
\contenu{
  \texte{Soit $a_1,\dots,a_n,b_1,\dots,b_n \in \R$ et
...
\end{enumerate}
\begin{enumerate}
  \item \reponse{$a_1\dots a_n + b_1a_2\dots a_n + a_1b_2a_3\dots a_n + \dots + a_1\dots a_{n-1}b_n$.}
  \item \reponse{$\frac{\chi_A(t)}{\prod_{i=1}^n(a_i-t)} = 1 + \sum_{i=1}^n\frac{b_i}{a_i-t}$ change de signe
    entre deux $a_i$ successifs et dans l'un des intervalles $]-\infty,a_1[$ ou $]a_n,+\infty[$
    donc $\chi_A$ admet $n$ racines distinctes.}
  \item \reponse{Oui. Supposons par exemple $a_1=\dots=a_p <a_{p+1}<\dots<a_n$~:
