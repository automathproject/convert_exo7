...
\organisation{exo7}
\contenu{
  \texte{Soit $E$ un espace vectoriel euclidien de dimension $p$ sur $\Rr$ ($p\geq2$).
...
\end{enumerate}
\begin{enumerate}
  \item \reponse{Soit $\mathcal{B}$ une base orthonormée de $E$ et $M=\mbox{Mat}_{\mathcal{B}}(x_1,...,x_n)$ ($M$ est une matrice de format $(p,n)$).
...
\end{center}}
  \item \reponse{Si la famille $(x_1,...,x_n)$ est liée, $\mbox{rg}(G)=\mbox{rg}(x_1,...,x_n)<n$, et donc, puisque $G$ est une matrice carrée de format $n$, $\gamma(x_1,...,x_n)=\mbox{det}(G)=0$.
...
$$\gamma(x_1,...,x_n)=\mbox{det}({^t}MM)=\mbox{det}({^t}M)\mbox{det}(M)=(\mbox{det}M)^2>0.$$}
  \item \reponse{On écrit $x=x-p_F(x)+p_F(x)$. La première colonne de $\gamma(x,x_1,...,x_n)$ s'écrit~:
