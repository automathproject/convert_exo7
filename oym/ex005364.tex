...
\end{enumerate}
\begin{enumerate}
  \item \reponse{Pour $n\geq2$, posons $\Delta_n=\left|
\begin{array}{ccccc}
0&1&2&\ldots&n-1\\
1&0&1& &n-2\\
2&1&0&\ddots&\vdots\\
...
\end{center}}
  \item \reponse{$\forall(i,j)\in\llbracket1,n\rrbracket^2,\;\sin(a_i+a_j)=\sin a_i\cos a_j+\cos a_i\sin a_j$ et donc si on pose $C=\left(
\begin{array}{c}
\cos a_1\\
\cos a_2\\
\vdots\\
...
Si $n=2$, $\mbox{det}(\sin(a_i+a_j))_{1\leq i,j\leq 2}=\sin(2a_1)\sin(2a_2)-\sin^2(a_1+a_2)$.}
  \item \reponse{L'exercice n'a de sens que si le format $n$ est pair. Posons $n=2p$ où $p$ est un entier naturel non nul.
...
\end{center}}
  \item \reponse{On retranche à la première colonne la somme de toutes les autres et on obtient
...
\end{array}
\right|=-(n-2).$$}
  \item \reponse{Pour $1\leq i\leq p$,
...
Par suite, $\mbox{det}(A_p)=\mbox{det}(A_{p-1})=...=\mbox{det}(A_1)=1$.}
  \item \reponse{En développant suivant la dernière ligne, on obtient~:
