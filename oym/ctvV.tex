...
\exo7id{5955}
\titre{Exercice 5955}
\theme{Espaces $L^{p, Théorème de complétude de Riesz}
\auteur{tumpach}
\date{2010/11/11}
\organisation{exo7}
\contenu{
...
\end{enumerate}
\begin{enumerate}
  \item \reponse{\emph{Cas de $L^{\infty}(\mu)$.}

\begin{enumerate}}
  \item \reponse{Soit $\{f_{n}\}_{n\in\mathbb{N}}$ une suite de Cauchy
...
$$}
  \item \reponse{Sur $\Omega\setminus E$, on a~:
$$
...
uniform\'ement vers $f$ sur $\Omega\setminus E$.}
  \item \reponse{\'Etendons la fonction $f$ \`a $\Omega$ en posant $f =
