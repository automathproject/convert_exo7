\uuid{4546}
\titre{Fonctions $\zeta$ et $\eta$}
\theme{}
\auteur{quercia}
\date{2010/03/14}
\organisation{exo7}
\contenu{
  \texte{Pour $x>1$ on pose $\zeta(x) = \sum_{n=1}^\infty \frac1{\strut n^x}$
...
\end{enumerate}
\begin{enumerate}
  \item \reponse{$\zeta(x) - \frac 1{x-1} =
  ...
  \item \reponse{$\sum_{n=1}^\infty \frac{(-1)^n\ln n}n = \eta'(1) = \gamma\ln 2 - \frac12\ln(2)^2$.}
\end{enumerate}
}