...
\organisation{exo7}
\contenu{
  \texte{Les polynômes suivants sont-ils irréductibles ?}
\begin{enumerate}
...
\end{enumerate}
\begin{enumerate}
  \item \reponse{Ce polynôme est unitaire donc primitif. $11$ est nombre premier qui
  ...
  \item \reponse{$f(X,Y)=(X^2+1)Y^3+(X-1)^{2}Y^2+(X-1)$. Regardons $f$ comme un
  ...
  \item \reponse{$f(X,Y)=Y^7+Y^6+7Y^4+XY^3+3X^2Y^2-5Y+X^2+X+1$. Considérons $f$ comme
