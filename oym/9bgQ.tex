...
\organisation{exo7}
\contenu{
  \texte{(un développement en série de fonctions de $\frac{\pi}{\sin(\pi z)}$ et $\mathrm{cotan}(\pi z)$).
...
\end{enumerate}
\begin{enumerate}
  \item \reponse{Soit $\alpha\in\Cc\setminus\Zz$. La fonction $f$ est $2\pi$-périodique, continue sur $\Rr$ et de classe $C^1$ par morceaux sur $\Rr$. Donc la série de \textsc{Fourier} de $f$ converge vers $f$ sur $\Rr$ d'après le théorème de \textsc{Dirichlet}.
...
\end{center}}
  \item \reponse{Soit $z\in\Cc\setminus\Zz$.
