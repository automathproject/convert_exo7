...
\exo7id{2081}
\titre{Exercice 2081}
\theme{Calculs d'intégrales, Utilisation de la définition}
\auteur{bodin}
\date{2008/02/04}
\organisation{exo7}
\contenu{
  \texte{Soit $f$ la fonction définie sur $[0,4]$ par 
\begin{equation*}
  f(x)=
  \begin{cases}
    -1 &\text{ si $x=0$}\\
...
\end{enumerate}
\begin{enumerate}
  \item \reponse{On trouve $\int_0^4 f(t) \, dt = +7$. Il faut tout d'abord tracer le graphe de cette fonction. 
...
$x=0$, $x=1$, $x=2$) dont on sait calculer l'intégrale.}
  \item \reponse{C'est la même chose pour $\int_0^x  f(t) \, dt$, mais au lieu d'aller jusqu'à $4$ on s'arrête à $x$, on trouve
\begin{equation*}
  F(x)=
  \begin{cases}
  ...
  \end{cases}
\end{equation*}}
  \item \reponse{Les seuls points à discuter pour la continuité sont les points $x=1$ et $x=2$,
