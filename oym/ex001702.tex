...
\organisation{exo7}
\contenu{
  \texte{Soit $E$ un $\R$-espace vectoriel de dimension finie $n$, et $u$ un
endomorphisme de $E$.
...
\end{enumerate}
\begin{enumerate}
  \item \reponse{$u\big(\sum_{i=1}^{N}\alpha_{i}x_{i}\big)=\sum_{i=1}^{N}\alpha_{i}u(u^{i}(x_{0}))=\sum_{i=1}^{N}\alpha_{i} x_{i+1}$. Donc $\forall x\in F,\ u(x)\in F$.}
  \item \reponse{Si à un rang $k$, $x_{k+1}$ est une combinaison linéaire des $x_{i}$
    ...
    $(x_{0},\dots,x_{k})$.}
  \item \reponse{$x_{k+1}-\sum_{i=0}^{k}a_{i}x_{i}=u^{k+1}(x_{0})-\sum_{i=0}^{k}a_{i}u^{i}(x_{0})=0$ donc $P_{0}(u)(x_{0})=0$.}
  \item \reponse{Si $x\in F$ alors $x=\sum_{i=0}^{N}\alpha_{i}u^{i}(x_{0})$. En posant
    $P=\sum_{i=0}^{N}\alpha_{i} X^{i}$, on a $x=P(u)(x_{0})$.}
  \item \reponse{Soit $P=QP_{0}+R$ la division euclidienne de $P$ par $P_{0}$, alors
    $\deg(R)<\deg(P_{0})=k+1$. Notons $R=\sum_{i=0}^{k}r_{i} X^{i}$. On a
    $x=P(u)(x_{0})=Q(u)P_{0}(u)(x_{0})+R(u)(x_{0})=R(u)(x_{0})$}
  \item \reponse{La famille $(x_{0},\dots,x_{k})$ est donc libre et génératrice dans
    $F$~: c'est une base.}
  \item \reponse{La matrice de $u_{|_{F}}$ dans cette base est la matrice compagnon
    associée au polynôme $P_{0}$, et $\chi_{u_{|_{F}}}=P_{0}$.}
  \item \reponse{On choisit un vecteur $y\in E\setminus F$, et on recommence le même
