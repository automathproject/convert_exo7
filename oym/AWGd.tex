\uuid{AWGd}
\exo7id{5767}
\titre{** I Un calcul de l'intégrale de \textsc{Gauss} $I=\int_{0}^{+\infty}e^{-t^2}\;dt$}
\theme{Intégrales dépendant d'un paramètre}
\auteur{rouget}
\date{2010/10/16}
\organisation{exo7}
\contenu{
  \texte{Pour $x\in\Rr$, on pose $F(x)=\int_{0}^{1}\frac{e^{-x^2(1+t^2)}}{1+t^2}\;dt$ et $G(x)=\left(\int_{0}^{x}e^{-t^2}\;dt\right)^2$.}
\begin{enumerate}
...
\end{enumerate}
\begin{enumerate}
  \item \reponse{Soit $A>0$. Soit $\begin{array}[t]{cccc}
...
\end{center}}
  \item \reponse{La fonction $x\mapsto e^{-x^2}$ est continue sur $\Rr$. On en déduit que la fonction $x\mapsto\int_{0}^{x}e^{-t^2}\;dt$ est de classe $C^1$ sur $\Rr$. Il en est de même de la fonction $G$ et pour tout réel $x$,
...
\end{center}}
  \item \reponse{Soit $x\in\Rr^*$. En posant $u=xt$, on obtient
...
\end{center}}
  \item \reponse{Pour $x>0$, on a $\int_{0}^{x}e^{-t^2}\;dt\geqslant0$ et donc d'après la question 3),
