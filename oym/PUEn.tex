\uuid{PUEn}
\exo7id{3380}
\titre{$M$ antisymétrique $ \Rightarrow  I+M$ est inversible}
\theme{Calculs sur les matrices, Inverse}
\auteur{quercia}
\date{2010/03/09}
\organisation{exo7}
\contenu{
  \texte{Soit $M \in \mathcal{M}_n(\R)$ antisymétrique.}
\begin{enumerate}
...
\end{enumerate}
\begin{enumerate}
  \item \reponse{Nous allons montrer que $I+M$ est inversible en montrant que si un vecteur $X$ vérifie
$(I+M)X = 0$ alors $X=0$.
...
\end{align*}}
  \item \reponse{Montrons que $I+M$ et $(I-M) ^{-1}$ commutent.
