\uuid{5Rr3}
\exo7id{5665}
\titre{***I Matrices stochastiques}
\theme{Réduction}
\auteur{rouget}
\date{2010/10/16}
\organisation{exo7}
\contenu{
  \texte{Soit $A=(a_{i,j})_{1\leqslant i,j\leqslant n}\in\mathcal{M}_n(\Rr)$ telle que $\forall(i,j)\in\llbracket1,n\rrbracket^2,\;a_{i,j}\in[0,1]$ et $\forall i\in\llbracket1,n\rrbracket$, $\sum_{j=1}^{n}a_{i,j}= 1$.}
\begin{enumerate}
...
\end{enumerate}
\begin{enumerate}
  \item \reponse{Les hypothèses fournissent $AU= U$ où $U=\left(
\begin{array}{c}
1\\
\vdots\\
1
...
\right)$ et donc $1$ est valeur propre de $A$.}
  ...
  \item \reponse{Soient $\lambda$ une valeur propre de $A$ et $X =\left(
\begin{array}{c}
x_1\\
\vdots\\
x_n
