...
\organisation{exo7}
\contenu{
  \texte{Soit la courbe d'équation polaire $r=a(1+\cos\theta)$, $a>0$.}
\begin{enumerate}
  \item \question{Construire la courbe.}
  \item \question{Longueur et développée.}
\end{enumerate}
\begin{enumerate}
  \item \reponse{\textbf{Domaine d'étude.} La fonction $r$ est $2\pi$-périodique et paire. Donc on étudie et on construit la courbe quand $\theta$ décrit $[0,\pi]$ et on obtient la courbe complète par réflexion d'axe $(Ox)$.
...
$$\includegraphics{../images/img005532-1}$$}
  \item \reponse{Soient $\theta\in[-\pi,\pi]$ puis $M=O+a(1+\cos\theta)\overrightarrow{u}_{\theta}$ le point de $\mathcal{C}$ de paramètre $\theta$. 
