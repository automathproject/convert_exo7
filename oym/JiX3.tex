...
\exo7id{5738}
\titre{** I}
\theme{Suites et séries d'intégrales}
\auteur{rouget}
\date{2010/10/16}
\organisation{exo7}
\contenu{
...
\end{enumerate}
\begin{enumerate}
  \item \reponse{Soit $x\in[0,+\infty[$. Pour $n> x^2$, $f_n(x)=\text{exp}\left(n\ln\left(1-\frac{x^2}{n}\right)\right)$ et donc $f_n(x)\underset{n\rightarrow+\infty}{=}\text{exp}(-x^2+o(1))$. Donc la suite $(f_n)_{n\in\Nn^*}$ converge simplement sur $\Rr^+$ vers la fonction $f~:~x\mapsto e^{-x^2}$.}
  \item \reponse{Chaque fonction $f_n$, $n\in\Nn^*$, est continue par morceaux sur $[0,+\infty[$ et nulle au voisinage de $+\infty$. Donc chaque fonction $f_n$, $n\in\Nn^*$, est intégrable sur $[0,+\infty[$.
