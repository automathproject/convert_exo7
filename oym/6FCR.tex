...
\exo7id{6974}
\titre{Exercice 6974}
\theme{Fonctions circulaires et hyperboliques inverses, Fonctions circulaires inverses}
\auteur{blanc-centi}
\date{2014/05/06}
\organisation{exo7}
\contenu{
  \texte{Soit $z=x+iy$ un nombre complexe, où $x=\Re z$ et $y=\Im z$. 
...
\end{enumerate}
\begin{enumerate}
  \item \reponse{Si $x>0$, alors $\frac{y}{x}$ est bien défini et $\Arctan\frac{y}{x}$ aussi. 
...
$\Arctan(\tan\theta) = \theta$.)}
  \item \reponse{Si $\theta\in]-\pi,\pi[$ alors $\frac{\theta}{2}\in]-\frac{\pi}{2},\frac{\pi}{2}[$, 
...
d'où $\frac\theta2=\Arctan\left(\tan\frac{\theta}{2}\right)=\Arctan\left(\frac{\sin\theta}{1+\cos\theta}\right)$.}
  \item \reponse{Remarquons que $z=x+iy$, supposé non nul, est un nombre réel négatif si et seulement si 
