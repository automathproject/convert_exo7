...
\organisation{exo7}
\contenu{
  \texte{On consid\`ere la fonction $ f : \R \longrightarrow \R$ d\'efinie
par
...
\end{enumerate}
\begin{enumerate}
  \item \reponse{La fonction polynomiale $P (x) := x^{3} - 3 x + 1$ est continue et
...
0.$}
  \item \reponse{Comme $ f (x) - x = (x^{3} - 3 x + 1) \slash 9$ il en
r\'esulte que $\alpha$ est l'unique solution de l'\'equation $f
(x) = x$ dans $]0,1\slash 2[.$}
  \item \reponse{Comme $f' (x) = (x^{2} +2)\slash 3 > 0$ pour tout $x \in \R,$ on
...
prouve que la suite $(x_{n})$ est croissante.}
  \item \reponse{Un calcul simple montre que  $f (1 \slash 2) < 1
...
$x_{n+1} = f(x_n) < f(1/2) < 1/2$).}
  \item \reponse{D'apr\`es les questions
