\uuid{5750}
\titre{**** Inverse d'une série entière}
\theme{}
\auteur{rouget}
\date{2010/10/16}
\organisation{exo7}
\contenu{
  \texte{Soit  $\sum_{n=0}^{+\infty}a_nz^n$ une série entière de rayon $R > 0$ et telle que $a_0 = 1$ (ou plus généralement $a_0\neq 0$).}
\begin{enumerate}
...
\end{enumerate}
\begin{enumerate}
  \item \reponse{Montrons par récurrence que $\forall n\in\Nn$, $b_n$ existe et est unique.
...
On a montré par récurrence que la suite $(b_n)$ existe et est unique.}
  \item \reponse{Il faut alors vérifier que la série entière associée à la suite $(b_n)_{n\in\Nn}$ a un rayon de convergence strictement positif .
