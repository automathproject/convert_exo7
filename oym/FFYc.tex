...
\exo7id{96}
\titre{Exercice 96}
\theme{Nombres complexes, Divers}
\auteur{bodin}
\date{1998/09/01}
\organisation{exo7}
\contenu{
  \texte{Soit $\Zz[i] = \{ a+ib \  ; \ a,b \in \Zz \}$.}
\begin{enumerate}
...
\end{enumerate}
\begin{enumerate}
  \item \reponse{Soit $\alpha,\beta\in\Z[i]$. Notons $\alpha=a+ib$ et $\beta=c+id$ avec $a,b,c,d\in\Z$. Alors
...
$ac-bd\in\Z$, $ad+bc\in\Z$ donc $\alpha\beta\in\Z[i]$.}
  \item \reponse{Soit $\alpha\in\Z[i]$ inversible. Il existe donc $\beta\in\Z[i]$
...
$-i$.}
  \item \reponse{Soit $\omega\in\C$. Notons $\omega=x+iy$ avec $x,y\in\R$. soit
...
% $$}
  \item \reponse{Soit $\alpha,\beta\in\Z[i]$, avec $\beta\neq0$. Soit alors
