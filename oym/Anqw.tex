...
\organisation{exo7}
\contenu{
  \texte{Soit $n \geq 2$ un entier fix\'e et $f:\R^{+} = [0,
...
\begin{enumerate}
  ...
  \item \reponse{Il est clair que la fonction $f$ est d\'erivable sur $\R^{+}$
...
$$ f' (x) = \frac{n (x^{n-1} - 1)}{(1 + x)^{n + 1}}.$$}
  \item \reponse{Par l'expression pr\'ec\'edente $f' (x)$ est du signe de $x^{n - 1} - 1$ sur $\R^{+}.$ Par
