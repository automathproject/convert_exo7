\uuid{3799}
\titre{Centrale MP 2000 (avec Maple)}
\theme{}
\auteur{quercia}
\date{2010/03/11}
\organisation{exo7}
\contenu{
  \texte{Soit~$E$ un espace euclidien, $u$ et~$v$ deux endomorphismes auto-adjoints
...
\end{enumerate}
\begin{enumerate}
  \item \reponse{On se place dans une base propre pour~$u$, soient
...
$ \Rightarrow  C=\frac1{140}\begin{pmatrix}11&-11&-33\cr -11&11&33\cr -33&33&69\cr\end{pmatrix}$.}
  \item \reponse{Si $v$ est défini positif~: on a $(v(x)\mid x) = 2(u(x)\mid w(x))$ donc si $\lambda$ est
