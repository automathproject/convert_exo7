...
\contenu{
  \texte{}
  \question{Soit $f$ une fonction continue sur $[a,b]$. Pour $x$ réel, on pose $F(x)=\int_{a}^{b}|t-x|f(t)\;dt$. Etudier la dérivabilité de $F$ sur $\Rr$.}
  \reponse{Pour $x$ réel donné, la fonction $t\mapsto|t-x|f(t)$ est continue sur $[a,b]$ et donc $F(x)$ existe. Pour $x\leq a$,
