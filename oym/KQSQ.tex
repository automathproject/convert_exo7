...
\organisation{exo7}
\contenu{
  \texte{Soit $(a_n)$ une suite réelle avec $a_0=0$ et $a_1=1$. On note 
...
\end{enumerate}
\begin{enumerate}
  \item \reponse{$f(\overline z) = \overline{f(z)}$.}
  \item \reponse{$\Im(f)$ est de signe constant sur le connexe $D\cap \Omega ^+$
    et $f(z)\sim z$ au voisinage de~$0$.}
  \item \reponse{Intégrer terme à terme.}
  \item \reponse{$\Im (f(re^{i\theta})) \sin\theta \ge 0$ par la question {2.}.}
\end{enumerate}
}