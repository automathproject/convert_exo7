...
\end{enumerate}
\begin{enumerate}
  \item \reponse{Soit $P=X^2+X+1$. Le polynôme $P$ est sans racine, donc s'il
est réductible, il se factorise en deux polynômes de degré 2. Or
$(X^2+X+1)^2=X^4+X^2+1$, qui est différent de $P$. Donc $P$ est
irréductible.}
  \item \reponse{L'idéal $(X^4+X^3+1)$ est premier car $X^4+X^3+1$ est
...
possède 16 éléments.}
  \item \reponse{Puisque $A$ est un corps, $A^*$ contient 15 éléments, donc
l'ordre de tout élément de $A^*$ est un diviseur de 15. En
particulier, $\alpha^{15}=1$.}
  \item \reponse{$$\begin{array}{l}
\alpha^1=\alpha\\
\alpha^2=\alpha^2\\
\alpha^3=\alpha^3\\
\alpha^4=\alpha^3+1\\
...
\alpha^{15}=1
\end{array}$$}
  \item \reponse{$\alpha^7+\alpha^8+\alpha^9=\alpha^7(1+\alpha+\alpha^2)=\alpha^{7+7}=\alpha^{14}=\alpha^3+\alpha^2$.}
  \item \reponse{$(1+\alpha+\alpha^3)^{-1}=(\alpha^5)^{-1}=\alpha^{10}$.}
\end{enumerate}
}