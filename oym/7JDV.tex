...
\exo7id{6987}
\titre{Exercice 6987}
\theme{Courbes planes, Courbes paramétrées en coordonnées cartésiennes}
\auteur{blanc-centi}
\date{2015/07/04}
\organisation{exo7}
\contenu{
  \texte{On considère la courbe paramétrée définie par 
...
\end{enumerate}
\begin{enumerate}
  \item \reponse{Les fonctions $x$ et $y$ sont de classe $\mathcal{C}^1$ sur $\mathcal{D}$. Soit $t\in\mathcal{D}$:
$$\begin{array}{lcl}
...
y'(t)&& + &0 &-&&-&&-&&-&0&+&\\
\end{array}$$}
  \item \reponse{Le tableau de variations conjointes indique:
\begin{itemize}}
  \item \reponse{$t=-4$: tangente horizontale, au point de coordonnées $(-5,-\frac{1}{3})$;}
  \item \reponse{$t=-2$: tangente verticale, au point de coordonnées $(-4,-\frac{5}{3})$;}
  \item \reponse{$t=-1$: une asymptote verticale, d'équation $x=-5$;}
  \item \reponse{$t=0$: une asymptote horizontale, d'équation $y=5$;}
  \item \reponse{$t=2$: il y a un point singulier en $(4,\frac{11}{3})$ (voir après).
\end{itemize}
