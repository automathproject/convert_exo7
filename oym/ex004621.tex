...
\end{enumerate}
\begin{enumerate}
  \item \reponse{$\frac z{(1-z)^2} = \sum_{n=1}^\infty nz^n$ ($R=1$).
  ...
  \item \reponse{$f(z)\in\R \Leftrightarrow f(z) = \overline{f(z)} = f(\overline z) \Leftrightarrow z=\overline z \Leftrightarrow z\in\R$.
...
limité par $]-1,1[$, et comme $f(z) = z + o_{z\to0}(z)$, ce signe est celui de~$\Im z$.}
  \item \reponse{$ \int_{t=0}^{\pi }\Im(f(re^{it}))\sin nt\,d t = \frac{\pi a_nr^n}2$.
