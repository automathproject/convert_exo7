\uuid{618n}
\exo7id{4614}
\titre{Formules de Cauchy\label{Cauchy}}
\theme{Exercices de Michel Quercia, Séries entières}
\auteur{quercia}
\date{2010/03/14}
\organisation{exo7}
\contenu{
  \texte{Soit $U$ un ouvert de~$\C$ contenant $0$ et $f : U \to \C$ analytique.
...
\begin{enumerate}
  \item \reponse{Calcul.}
  \item \reponse{Soit $0<r_0<d$ et $R(\theta)$ le rayon de la série de Taylor de~$f$ en $r_0e^{i\theta}$.
  ...
  \item \reponse{$g(z)= \frac1{2\pi} \int_{\theta=0}^{2\pi}\frac{f(re^{i\theta})}{re^{i\theta}-z}re^{i\theta}\, d\theta
  ...
  \item \reponse{résulte de la question {3.}.}
  \item \reponse{D'après la question {1.}, $|a_n| \le \|f\|_\infty/r^n$ pour tout $r>0$ donc $a_n = 0$ si $n\ge 1$.}
  \item \reponse{$1/P$ est analytique bornée sur~$\C$.}
  \item \reponse{On peut passer à la limite uniforme (ou dominée) dans la question {3.}.}
  \item \reponse{$f(z) = \sum_{n=0}^\infty a_nz^n  \Rightarrow  f\circ g(z) = \sum_{n=0}^\infty a_ng^n(z)$
