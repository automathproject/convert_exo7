...
\organisation{exo7}
\contenu{
  \texte{Soient $(E,d)$ un espace m\'etrique compact et $f:E\rightarrow E$
...
\end{enumerate}
\begin{enumerate}
  \item \reponse{Comme $E\times E$ est compact alors de la suite $(a_n,b_n)$ on peut
...
Donc $d(a_{\phi(n+1)-\phi(n)}, a_0) \le d(a_{\phi(n+1)-\phi(n)+1}, a_1) \le \ldots \le d(a_{\phi(n+1)-1}, a_{\phi(n)-1}) \le  d(a_{\phi(n+1)}, a_{\phi(n)}) < \epsilon$. Donc pour $k = \phi(n+1)-\phi(n)$, sachant que $a_0=a$ alors $d(a_{k}, a) < \epsilon$. Même chose avec $(b_n)$.}
  ...
  \item \reponse{Soit $a \in E$ et $\epsilon >0$ alors il existe $k\ge 1$ tel que $a_k = f^k(a) \in f(E)$     avec $d(a,a_k) < \epsilon $. Donc $f(E)$ est dense dans $E$.}
  \item \reponse{Soit $u_n = d(a_n,b_n)$. Alors par la propriété pour $f$, $(u_n)$ est une suite croissante de $\Rr$. Comme $E$ est compact alors son diamètre est borné, donc $(u_n)$ est majorée. La suite $(u_n)$ est croissante et majorée donc converge vers $u$.
...
En particulier $u_1=u_0$ donc $d(a_1,b_1)=d(a_0,b_0)$ soit $d(f(a),f(b))=d(a,b)$. Donc $f$ est une isométrie.}
  \item \reponse{$f$ est une isométrie donc continue (elle est $1$ lipschitzi\`enne !). $E$ est compact donc
