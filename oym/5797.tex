...
\contenu{
  \texte{}
  \question{Trouver tous les endomorphismes de $\Rr^3$ vérifiant $\forall(x,y)\in(\Rr^3)^2$, $f(x\wedge y) = f(x)\wedge f(y)$.}
  \reponse{Soit $(i,j,k)$ une base orthonormée directe de $\Rr^3$ euclidien orienté. Posons $u = f(i)$, $v = f(j)$ et $w = f(k)$. Nécessairement, $u\wedge v =f(i)\wedge f(j) = f(i\wedge j) = f(k) = w$ et de même $v\wedge w = u$ et $w\wedge u = v$.
