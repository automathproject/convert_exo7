...
\exo7id{4037}
\titre{tan$(x) = x$}
\theme{Développements limités, DL implicite}
\auteur{quercia}
\date{2010/03/11}
\organisation{exo7}
\contenu{
...
\end{enumerate}
\begin{enumerate}
  \item \reponse{Notons $I_n$ l'intervalle  $\left]n\pi-\frac \pi2, n\pi+\frac \pi2\right[$.
...
tel que $f(x_n)=0$ c'est-à-dire $\tan x_n=x_n$.}
  \item \reponse{$x \mapsto \arctan x$ est la bijection réciproque de la restriction de la tangente  
...
$x \in ]-\frac\pi2,+\frac\pi2[$ !}
  \item \reponse{Comme $x_n \in I_n$ alors $x_n \to +\infty$ lorsque $n\to +\infty$.
...
$$x_n = n\pi + \arctan x_n = n\pi +\frac \pi 2 - \arctan \frac{1}{x_n} = n\pi +\frac \pi 2 + o(1).$$}
  \item \reponse{On va utiliser le dl obtenu précédemment pour obtenir un dl à un ordre plus grand :
