\exo7id{5586}
\titre{**I Noyaux itérés}
\theme{}
\auteur{rouget}
\date{2010/10/16}
\organisation{exo7}
\contenu{
  \texte{Soient $E$ un espace vectoriel et $f$ un endomorphisme de $E$. Pour $k\in\Nn$, on pose $N_k= \text{Ker}(f^k)$ et $I_k=\text{Im}(f^k)$ puis $N=\displaystyle\bigcup_{k\in\Nn}N_k$ et $I=\displaystyle\bigcap_{k\in\Nn}I_k$. ($N$ est le nilespace de $f$ et $I$ le c\oe ur de $f$)}
\begin{enumerate}
...
\begin{enumerate}
  ...
  \item \reponse{Soient $k\in\Nn$ et $x\in E$. $x\in N_k\Rightarrow f^k(x)=0\Rightarrow f(f^k(x)) = 0\Rightarrow x\in N_{k+1}$. 
...
\end{center}}
  \item \reponse{Soit $x\in N$. Il existe un entier $k$ tel que $x$ est dans $N_k$ ou encore tel que $f^k(x)=0$. Mais alors $f^k(f(x)) = f(f^k(x))=0$ et $f(x)$ est dans $N_k$ et donc dans $N$. Ainsi, $N$ est stable par $f$.
...
Soit $y\in I$. Alors, pour tout naturel $k$, il existe $x_k\in E$ tel que $y=f^k(x_k)$. Mais alors, pour tout entier $k$, $f(y)=f(f^k(x_k))=f^k(f(x))$ est dans $I_k$, et donc $f(y)$ est dans $I$. $I$ est stable par $f$.}
  \item \reponse{Si $N_k=N_{k+1}$, on a déjà $N_{k+1}\subset N_{k+2}$. Montrons que $N_{k+2}\subset N_{k+1}$.
