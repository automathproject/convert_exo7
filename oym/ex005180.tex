...
\organisation{exo7}
\contenu{
  \texte{Dans $E=\Rr^\Rr$, étudier la liberté des familles suivantes $A$ de vecteurs de $E$~:}
\begin{enumerate}
...
\end{enumerate}
\begin{enumerate}
  \item \reponse{Notons respectivement $g$ et $h$, les fonctions sinus et cosinus.
...
de $p$ vecteurs donnés constituent une famille liée).}
  \item \reponse{$f_0$, $f_1$ et $f_2$ sont trois combinaisons linéaires des deux fonctions $x\mapsto1$ et $x\mapsto x$. Donc,
la famille $(f_0,f_1,f_2)$ est une famille liée puis la famille $(f_n)_{n\in\Zz}$ est liée en tant que sur-famille
d'une famille liée.}
  \item \reponse{Pour $\alpha$ réel donné et $x>0$, posons $f_\alpha(x)=x^\alpha$.
...
$(f_\alpha)_{\alpha\in\Rr}$ est libre.}
  \item \reponse{Pour $a$ réel donné et $x$ réel, posons $f_a(x)=|x-a|$. Soient $n$ un
