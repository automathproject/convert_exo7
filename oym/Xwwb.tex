...
\organisation{exo7}
\contenu{
  \texte{On considère le polynôme défini par :
...
\end{enumerate}
\begin{enumerate}
  \item \reponse{La fonction $f_n : x  \mapsto \frac{P_n(x)}{x^n}$ croît strictement
             de $-\infty$ à 1 quand $x$ varie de $0$ à $+\infty$.}
  \item \reponse{$\chi_A(x) = (-1)^n\Bigl(x^n - \sum_{k=1}^n kx^{n-k}\Bigr)$.}
\end{enumerate}
}