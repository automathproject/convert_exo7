...
\exo7id{5925}
\titre{Exercice 5925}
\theme{Intégrales généralisées et théorie de la mesure , Intégrales généralisées}
\auteur{tumpach}
\date{2010/11/11}
\organisation{exo7}
\contenu{
...
\end{enumerate}
\begin{enumerate}
  \item \reponse{Pour $n\in\mathbb{N}$, on pose~:
\begin{eqnarray*}
u_{n} = \int_{n\pi}^{(n+1)\pi} \frac{|\sin t|}{t} \, dt.
\end{eqnarray*}
...
\sum_{n=0}^{+\infty} \frac{1}{n+1}$$ est divergente.}
  \item \reponse{\emph{Deuxi\`eme formule de la moyenne.}
...
permettent de conclure.}
  \item \reponse{D'apr\`es le crit\`ere de Cauchy (voir la proposition des rappels), pour
...
convergente.}
  ...
  \item \reponse{Posons pour $t > 0$, $U(t) = \frac{e^{-\lambda t}}{t}$.
...
e^{-\lambda x}}{x}.
\end{equation*}}
  \item \reponse{On remarque que, pour tout $x>0$, la fonction $t \mapsto f(t,
...
$ convient.}
  \item \reponse{Pour $x \in [0, +\infty[$ et $\lambda \in ]0, +\infty[$,
posons
$$
...
$$}
  \item \reponse{De la question pr\'ec\'edente, il d\'ecoule que
$$
