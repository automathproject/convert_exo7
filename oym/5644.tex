...
\contenu{
  \texte{}
  \question{Soient $a_0$,...,$a_{n-1}$ $n$ nombres complexes. Calculer $\left|
\begin{array}{ccccc}
...
Pour cela, on calculera d'abord $A\Omega$ où $\Omega=(\omega^{(j-1)(k-1)})_{1\leqslant j,k\leqslant n}$ avec $\omega=e^{2i\pi/n}$.}
  \reponse{Le coefficient ligne $j$, colonne $k$, $(j,k)\in\llbracket1,n\rrbracket^2$, de la matrice $A$ vaut $a_{k-j}$ avec la convention : si $-(n-1)\leqslant u\leqslant -1$, $a_u = a_{n+u}$.
