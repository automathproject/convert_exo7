...
\exo7id{5483}
\titre{***}
\theme{Produit scalaire, espaces euclidiens}
\auteur{rouget}
\date{2010/07/10}
\organisation{exo7}
\contenu{
  \texte{Soit $E$ un $\Rr$ espace vectoriel de dimension finie. Soit $||\;||$ une norme sur $E$ vérifiant l'identité du parallèlogramme, c'est-à-dire~:~$\forall(x,y)\in E^2,\;||x+y||^2+||x-y||^2=2(||x||^2+||y||^2)$. On se propose de démontrer que $||\;||$ est associée à un produit scalaire.
...
\end{enumerate}
\begin{enumerate}
  \item \reponse{Soit $(x,y,z)\in\Rr^3$.
...
\end{align*}}
  \item \reponse{$2f(x,y)= f(x+x,y)+f(x-x,y)=f(2x,y)+f(0,y)$ mais $f(0,y)=(||y||^2-||-y||^2)=0$ (définition d'une norme).}
  \item \reponse{\textbullet~Montrons par récurrence que $\forall n\in\Nn,\;f(nx,y)=nf(x,y)$.
...
\end{center}}
  \item \reponse{On pose $x=\frac{1}{2}(u+v)$ et $y=\frac{1}{2}(u-v)$.

$$f(u,w)+f(v,w)=f(x+y,w)+f(x-y,w)=2f(x,w)=2f\left(\frac{1}{2}(u+v),w\right)=f(u+v,w).$$}
  \item \reponse{f est symétrique (définition d'une norme) et linéaire par rapport à sa première variable (d'après 3) et 4)).
Donc f est bilinéaire.}
  \item \reponse{f est une forme bilinéaire symétrique.
