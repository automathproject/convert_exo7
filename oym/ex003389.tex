\exo7id{3389}
\titre{Rayon spectral d'une matrice à coefficients positifs}
\theme{}
\auteur{quercia}
\date{2010/03/09}
\organisation{exo7}
\contenu{
  \texte{Soit $A = (a_{ij}) \in \mathcal{M}_n(\R)$ avec : $\forall\ i,j,\ a_{ij} > 0$.
...
\end{enumerate}
\begin{enumerate}
  \item \reponse{Compacité.}
  \item \reponse{Si $x_1 = 0$, on pose $Y = \begin{pmatrix}\alpha \cr x_2 \cr \vdots \cr x_n \cr\end{pmatrix}$ :
  ...
  \item \reponse{Si $y_1 > 0$, on pose $X = X_0 + \begin{pmatrix}\alpha\cr0\cr\vdots\cr0\cr\end{pmatrix}$ :
