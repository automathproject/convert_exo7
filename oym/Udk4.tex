\uuid{Udk4}
\exo7id{3354}
\titre{Automorphismes de $\mathcal{L}(E)$}
...
\end{enumerate}
\begin{enumerate}
  \item \reponse{$\psi_{ij} \circ \psi_{k\ell} = \delta_{jk}\psi_{i\ell}$.}
  \item \reponse{$\psi_{11}$ est un projecteur non trivial.}
  \item \reponse{Si $\sum \lambda_k\vec u_k = \vec 0$, alors en appliquant
              $\psi_{1j}$ : $\lambda_j\vec u_1 = \vec 0  \Rightarrow  \lambda_j = 0$.}
  \item \reponse{Décomposer $g$ sur la base $(\varphi_{ij})$.}
\end{enumerate}
}