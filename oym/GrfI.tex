...
\exo7id{5088}
\titre{**}
\theme{Trigonométrie hyperbolique}
\auteur{rouget}
\date{2010/06/30}
\organisation{exo7}
\contenu{
...
\end{enumerate}
\begin{enumerate}
  \item \reponse{\textbf{1ère solution.} Pour tout réel $x$, $\sqrt{x^2+1}>\sqrt{x^2}=|x|$ et donc $-1<\frac{x}{\sqrt{x^2+1}}<
...
f_1(x)&=\Arcsin(\sin\theta)=\theta\;(\mbox{car}\;\theta\;\mbox{est dans}\;\left]-\frac{\pi}{2},\frac{\pi}{2}\right[)\\
 &=\Arctan x.
\end{align*}}
  \item \reponse{\textbf{1ère solution.} Pour tout réel $x$, $-1<-1+\frac{2}{1+x^2}=\frac{1-x^2}{1+x^2}\leq-1+2=1$ (avec égalité
...
\end{array}
\right.=2\Arctan|x|.$$}
  \item \reponse{La fonction $x\mapsto\Arcsin\sqrt{1-x^2}$ est définie et continue sur $[-1,1]$, dérivable sur $[-1,1]\setminus\{0\}$ car
...
\end{center}}
  \item \reponse{$f_4$ est dérivable sur $\mathcal{D}=\Rr\setminus\{-1,0\}$ et pour $x$ élément de $\mathcal{D}$, on a~:~
